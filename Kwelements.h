...
double cxpentanejpmole(void);
double cxpentanejpkg(void);
double cxpentanejpkgtotal(void);
...
double cxsucrosejpmole(void);
double cxsucrosejpkg(void);
double cxsucrosejpkgtotal(void);
...
#define HYDROGENJPMOLE (2.86E+5 * JOULE / MOLE)
#define HYDROGENJPKG (HYDROGENJPMOLE / HYDROGEN_MASS)
#define HYDROGENJPKGTOTAL \
      (HYDROGENJPMOLE / (HYDROGEN_MASS + (2 / 4.0 + 0 - 0 /\
            2.0) * 2.0 * O_MASS))
#define METHANE_MASS \
      (4 * H_MASS + 1 * C_MASS + 0 * N_MASS + 0 * O_MASS)
#define METHANEMASS \
      (4 * HMASS + 1 * CMASS + 0 * NMASS + 0 * OMASS)
#define METHANEJPMOLE (8.89E+5 * JOULE / MOLE)
#define METHANEJPKG (METHANEJPMOLE / METHANE_MASS)
#define METHANEJPKGTOTAL \
      (METHANEJPMOLE / (METHANE_MASS + (4 / 4.0 + 1 - 0 /\
            2.0) * 2.0 * O_MASS))
#define ETHANE_MASS \
      (6 * H_MASS + 2 * C_MASS + 0 * N_MASS + 0 * O_MASS)
#define ETHANEMASS \
      (6 * HMASS + 2 * CMASS + 0 * NMASS + 0 * OMASS)
#define ETHANEJPMOLE (1.560E+6 * JOULE / MOLE)
#define ETHANEJPKG (ETHANEJPMOLE / ETHANE_MASS)
#define ETHANEJPKGTOTAL \
      (ETHANEJPMOLE / (ETHANE_MASS + (6 / 4.0 + 2 - 0 / 2.0)\
            * 2.0 * O_MASS))
#define PROPANE_MASS \
      (8 * H_MASS + 3 * C_MASS + 0 * N_MASS + 0 * O_MASS)
#define PROPANEMASS \
      (8 * HMASS + 3 * CMASS + 0 * NMASS + 0 * OMASS)
#define PROPANEJPMOLE (2.220E+6 * JOULE / MOLE)
#define PROPANEJPKG (PROPANEJPMOLE / PROPANE_MASS)
#define PROPANEJPKGTOTAL \
      (PROPANEJPMOLE / (PROPANE_MASS + (8 / 4.0 + 3 - 0 /\
            2.0) * 2.0 * O_MASS))
#define BUTANE_MASS \
      (10 * H_MASS + 4 * C_MASS + 0 * N_MASS + 0 * O_MASS)
#define BUTANEMASS \
      (10 * HMASS + 4 * CMASS + 0 * NMASS + 0 * OMASS)
#define BUTANEJPMOLE (2.877E+6 * JOULE / MOLE)
#define BUTANEJPKG (BUTANEJPMOLE / BUTANE_MASS)
#define BUTANEJPKGTOTAL \
      (BUTANEJPMOLE / (BUTANE_MASS + (10 / 4.0 + 4 - 0 / 2.0)\
            * 2.0 * O_MASS))
#define PENTANE_MASS \
      (12 * H_MASS + 5 * C_MASS + 0 * N_MASS + 0 * O_MASS)
#define PENTANEMASS \
      (12 * HMASS + 5 * CMASS + 0 * NMASS + 0 * OMASS)
#define PENTANEJPMOLE (3.507E+6 * JOULE / MOLE)
#define PENTANEJPKG (PENTANEJPMOLE / PENTANE_MASS)
#define PENTANEJPKGTOTAL \
      (PENTANEJPMOLE / (PENTANE_MASS + (12 / 4.0 + 5 - 0 /\
            2.0) * 2.0 * O_MASS))
#define ETHANOL_MASS \
      (6 * H_MASS + 2 * C_MASS + 0 * N_MASS + 1 * O_MASS)
#define ETHANOLMASS \
      (6 * HMASS + 2 * CMASS + 0 * NMASS + 1 * OMASS)
#define ETHANOLJPMOLE (1.300E+6 * JOULE / MOLE)
#define ETHANOLJPKG (ETHANOLJPMOLE / ETHANOL_MASS)
#define ETHANOLJPKGTOTAL \
      (ETHANOLJPMOLE / (ETHANOL_MASS + (6 / 4.0 + 2 - 1 /\
            2.0) * 2.0 * O_MASS))
#define SUGAR_MASS \
      (22 * H_MASS + 12 * C_MASS + 0 * N_MASS + 11 * O_MASS)
#define SUGARMASS \
...
#define SUGARJPKGTOTAL \
      (SUGARJPMOLE / (SUGAR_MASS + (22 / 4.0 + 12 - 11 / 2.0)\
            * 2.0 * O_MASS))
#define SUCROSE_MASS \
      (22 * H_MASS + 12 * C_MASS + 0 * N_MASS + 11 * O_MASS)
#define SUCROSEMASS \
      (22 * HMASS + 12 * CMASS + 0 * NMASS + 11 * OMASS)
#define SUCROSEJPMOLE (5.6467E+6 * JOULE / MOLE)
#define SUCROSEJPKG (SUCROSEJPMOLE / SUCROSE_MASS)
#define SUCROSEJPKGTOTAL \
      (SUCROSEJPMOLE / (SUCROSE_MASS + (22 / 4.0 + 12 - 11 /\
            2.0) * 2.0 * O_MASS))
#define COAL_MASS \
...
#define COALJPKGTOTAL \
      (COALJPMOLE / (COAL_MASS + (0 / 4.0 + 1 - 0 / 2.0) *\
            2.0 * O_MASS))
...
#define PARAFFINJPKGTOTAL \
      (PARAFFINJPMOLE / (PARAFFIN_MASS + (62 / 4.0 + 30 - 0 /\
            2.0) * 2.0 * O_MASS))
#define FAT_MASS \
...
#define FATJPKGTOTAL \
      (FATJPMOLE / (FAT_MASS + (100 / 4.0 + 60 - 6 / 2.0) *\
            2.0 * O_MASS))
#define DIESEL_MASS \
...
#define DIESELJPKGTOTAL \
      (DIESELJPMOLE / (DIESEL_MASS + (26 / 4.0 + 12 - 0 /\
            2.0) * 2.0 * O_MASS))
...
#define GASOLINEJPKGTOTAL \
      (GASOLINEJPMOLE / (GASOLINE_MASS + (18 / 4.0 + 8 - 0 /\
            2.0) * 2.0 * O_MASS))
...
#define Kwelements_DESCRIPTION \
   ...
   "ASS Ag_MELT AgMASS AgZ AIR_MASS AIRMASS Al_BOIL Al_Cp Al_DE"\
   "NSITY Al_En Al_MASS Al_MELT AlMASS AlZ Am_BOIL Am_Cp Am_DEN"\
   "SITY Am_En Am_MASS Am_MELT AmMASS AmZ Ar_BOIL Ar_Cp Ar_DENS"\
   "ITY Ar_En Ar_MASS Ar_MELT ArMASS ArZ As_BOIL As_Cp As_DENSI"\
   "TY As_En As_MASS As_MELT AsMASS AsZ At_BOIL At_Cp At_DENSIT"\
   "Y At_En At_MASS At_MELT AtMASS AtZ Au_BOIL Au_Cp Au_DENSITY"\
   " Au_En Au_MASS Au_MELT AuMASS AuZ B_BOIL B_Cp B_DENSITY B_E"\
   "n B_MASS B_MELT Ba_BOIL Ba_Cp Ba_DENSITY Ba_En Ba_MASS Ba_M"\
   "ELT BaMASS BaZ Be_BOIL Be_Cp Be_DENSITY Be_En Be_MASS Be_ME"\
   "LT BeMASS BeZ Bh_BOIL Bh_Cp Bh_DENSITY Bh_En Bh_MASS Bh_MEL"\
   "T BhMASS BhZ Bi_BOIL Bi_Cp Bi_DENSITY Bi_En Bi_MASS Bi_MELT"\
   " BiMASS BiZ Bk_BOIL Bk_Cp Bk_DENSITY Bk_En Bk_MASS Bk_MELT "\
   "BkMASS BkZ BMASS Br_BOIL Br_Cp Br_DENSITY Br_En Br_MASS Br_"\
   "MELT BrMASS BrZ BUTANE_MASS BUTANEJPKG BUTANEJPKGTOTAL BUTA"\
   "NEJPMOLE BUTANEMASS BZ C12_MASS C12_RATIO C12H22O11_MASS C1"\
   "2H22O11MASS C12MASS C13_MASS C13_RATIO C13MASS C14_MASS C14"\
   "_RATIO C14MASS C2H60_MASS C2H60MASS C2H6_MASS C2H6MASS C3H8"\
   "_MASS C3H8MASS C4H10_MASS C4H10MASS C_BOIL C_Cp C_DENSITY C"\
   "_En C_MASS C_MEAN_MASS C_MELT Ca_BOIL Ca_Cp Ca_DENSITY Ca_E"\
   "n Ca_MASS Ca_MELT CaMASS CaZ Cd_BOIL Cd_Cp Cd_DENSITY Cd_En"\
   " Cd_MASS Cd_MELT CdMASS CdZ Ce_BOIL Ce_Cp Ce_DENSITY Ce_En "\
   "Ce_MASS Ce_MELT CeMASS CeZ Cf_BOIL Cf_Cp Cf_DENSITY Cf_En C"\
   "f_MASS Cf_MELT CfMASS CfZ CH3CH2OH_MASS CH3CH2OHMASS CH4_MA"\
   "SS CH4MASS Cl_BOIL Cl_Cp Cl_DENSITY Cl_En Cl_MASS Cl_MELT C"\
   "lMASS ClZ Cm_BOIL Cm_Cp Cm_DENSITY Cm_En Cm_MASS Cm_MELT CM"\
   "ASS CmMASS CmZ Cn_BOIL Cn_Cp Cn_DENSITY Cn_En Cn_MASS Cn_ME"\
   "LT CnMASS CnZ Co_BOIL Co_Cp Co_DENSITY Co_En Co_MASS Co_MEL"\
   "T COAL_MASS COALJPKG COALJPKGTOTAL COALJPMOLE COALMASS CoMA"\
   "SS CoZ Cr_BOIL Cr_Cp Cr_DENSITY Cr_En Cr_MASS Cr_MELT CrMAS"\
   "S CrZ Cs_BOIL Cs_Cp Cs_DENSITY Cs_En Cs_MASS Cs_MELT CsMASS"\
   " CsZ Cu_BOIL Cu_Cp Cu_DENSITY Cu_En Cu_MASS Cu_MELT CuMASS "\
   "CuZ CZ Db_BOIL Db_Cp Db_DENSITY Db_En Db_MASS Db_MELT DbMAS"\
   "S DbZ DIESEL_MASS DIESELDENSITY DIESELJPKG DIESELJPKGTOTAL "\
   "DIESELJPMOLE DIESELMASS Ds_BOIL Ds_Cp Ds_DENSITY Ds_En Ds_M"\
   "ASS Ds_MELT DsMASS DsZ Dy_BOIL Dy_Cp Dy_DENSITY Dy_En Dy_MA"\
   "SS Dy_MELT DyMASS DyZ Er_BOIL Er_Cp Er_DENSITY Er_En Er_MAS"\
   "S Er_MELT ErMASS ErZ Es_BOIL Es_Cp Es_DENSITY Es_En Es_MASS"\
   " Es_MELT EsMASS EsZ ETHANE_MASS ETHANEJPKG ETHANEJPKGTOTAL "\
   "ETHANEJPMOLE ETHANEMASS ETHANOL_MASS ETHANOLDENSITY ETHANOL"\
   "JPKG ETHANOLJPKGTOTAL ETHANOLJPMOLE ETHANOLMASS Eu_BOIL Eu_"\
   "Cp Eu_DENSITY Eu_En Eu_MASS Eu_MELT EuMASS EuZ F_BOIL F_Cp "\
   "F_DENSITY F_En F_MASS F_MELT FAT_MASS FATDENSITY FATJPKG FA"\
   "TJPKGTOTAL FATJPMOLE FATMASS Fe_BOIL Fe_Cp Fe_DENSITY Fe_En"\
   " Fe_MASS Fe_MELT FeMASS FeZ Fl_BOIL Fl_Cp Fl_DENSITY Fl_En "\
   "Fl_MASS Fl_MELT FlMASS FlZ Fm_BOIL Fm_Cp Fm_DENSITY Fm_En F"\
   "m_MASS Fm_MELT FMASS FmMASS FmZ Fr_BOIL Fr_Cp Fr_DENSITY Fr"\
   "_En Fr_MASS Fr_MELT FrMASS FrZ FZ Ga_BOIL Ga_Cp Ga_DENSITY "\
   "Ga_En Ga_MASS Ga_MELT GaMASS GASOLINE_MASS GASOLINEDENSITY "\
   "GASOLINEJPKG GASOLINEJPKGTOTAL GASOLINEJPMOLE GASOLINEMASS "\
   "GaZ Gd_BOIL Gd_Cp Gd_DENSITY Gd_En Gd_MASS Gd_MELT GdMASS G"\
   "dZ Ge_BOIL Ge_Cp Ge_DENSITY Ge_En Ge_MASS Ge_MELT GeMASS Ge"\
   "Z H1_MASS H1MASS H2_MASS H2MASS H3_MASS H3MASS H_BOIL H_Cp "\
   "H_DENSITY H_En H_MASS H_MELT He3_MASS He3MASS He4_MASS He4M"\
   "ASS He_BOIL He_Cp He_DENSITY He_En He_MASS He_MELT HeMASS H"\
   "eZ Hf_BOIL Hf_Cp Hf_DENSITY Hf_En Hf_MASS Hf_MELT HfMASS Hf"\
   "Z Hg_BOIL Hg_Cp Hg_DENSITY Hg_En Hg_MASS Hg_MELT HgMASS HgZ"\
   " HMASS Ho_BOIL Ho_Cp Ho_DENSITY Ho_En Ho_MASS Ho_MELT HoMAS"\
   "S HoZ Hs_BOIL Hs_Cp Hs_DENSITY Hs_En Hs_MASS Hs_MELT HsMASS"\
   " HsZ HYDROGEN_MASS HYDROGENJPKG HYDROGENJPKGTOTAL HYDROGENJ"\
   "PMOLE HYDROGENMASS HZ I_BOIL I_Cp I_DENSITY I_En I_MASS I_M"\
   "ELT ICE_Cp IMASS In_BOIL In_Cp In_DENSITY In_En In_MASS In_"\
   "MELT InMASS InZ Ir_BOIL Ir_Cp Ir_DENSITY Ir_En Ir_MASS Ir_M"\
   "ELT IrMASS IrZ IZ K_BOIL K_Cp K_DENSITY K_En K_MASS K_MELT "\
   "KMASS Kr_BOIL Kr_Cp Kr_DENSITY Kr_En Kr_MASS Kr_MELT KrMASS"\
   " KrZ KZ La_BOIL La_Cp La_DENSITY La_En La_MASS La_MELT LaMA"\
   "SS LaZ Li_BOIL Li_Cp Li_DENSITY Li_En Li_MASS Li_MELT LiMAS"\
   "S LiZ Lr_BOIL Lr_Cp Lr_DENSITY Lr_En Lr_MASS Lr_MELT LrMASS"\
   " LrZ Lu_BOIL Lu_Cp Lu_DENSITY Lu_En Lu_MASS Lu_MELT LuMASS "\
   "LuZ Lv_BOIL Lv_Cp Lv_DENSITY Lv_En Lv_MASS Lv_MELT LvMASS L"\
   "vZ Mc_BOIL Mc_Cp Mc_DENSITY Mc_En Mc_MASS Mc_MELT McMASS Mc"\
   "Z Md_BOIL Md_Cp Md_DENSITY Md_En Md_MASS Md_MELT MdMASS MdZ"\
   " METHANE_MASS METHANEJPKG METHANEJPKGTOTAL METHANEJPMOLE ME"\
   "THANEMASS Mg_BOIL Mg_Cp Mg_DENSITY Mg_En Mg_MASS Mg_MELT Mg"\
   "MASS MgZ Mn_BOIL Mn_Cp Mn_DENSITY Mn_En Mn_MASS Mn_MELT MnM"\
   "ASS MnZ Mo_BOIL Mo_Cp Mo_DENSITY Mo_En Mo_MASS Mo_MELT MoMA"\
   "SS MoZ Mt_BOIL Mt_Cp Mt_DENSITY Mt_En Mt_MASS Mt_MELT MtMAS"\
   "S MtZ N_BOIL N_Cp N_DENSITY N_En N_MASS N_MELT Na_BOIL Na_C"\
   "p Na_DENSITY Na_En Na_MASS Na_MELT NaMASS NaZ Nb_BOIL Nb_Cp"\
   " Nb_DENSITY Nb_En Nb_MASS Nb_MELT NbMASS NbZ Nd_BOIL Nd_Cp "\
   "Nd_DENSITY Nd_En Nd_MASS Nd_MELT NdMASS NdZ Ne_BOIL Ne_Cp N"\
   "e_DENSITY Ne_En Ne_MASS Ne_MELT NeMASS NeZ Nh_BOIL Nh_Cp Nh"\
   "_DENSITY Nh_En Nh_MASS Nh_MELT NhMASS NhZ Ni_BOIL Ni_Cp Ni_"\
   "DENSITY Ni_En Ni_MASS Ni_MELT NiMASS NiZ NMASS No_BOIL No_C"\
   "p No_DENSITY No_En No_MASS No_MELT NoMASS NoZ Np_BOIL Np_Cp"\
   " Np_DENSITY Np_En Np_MASS Np_MELT NpMASS NpZ NZ O16_MASS O1"\
   "6MASS O17_MASS O17MASS O18_MASS O18MASS O_BOIL O_Cp O_DENSI"\
   "TY O_En O_MASS O_MELT Og_BOIL Og_Cp Og_DENSITY Og_En Og_MAS"\
   "S Og_MELT OgMASS OgZ OMASS Os_BOIL Os_Cp Os_DENSITY Os_En O"\
   "s_MASS Os_MELT OsMASS OsZ OZ P_BOIL P_Cp P_DENSITY P_En P_M"\
   "ASS P_MELT Pa_BOIL Pa_Cp Pa_DENSITY Pa_En Pa_MASS Pa_MELT P"\
   "aMASS PARAFFIN_MASS PARAFFINDENSITY PARAFFINJPKG PARAFFINJP"\
   "KGTOTAL PARAFFINJPMOLE PARAFFINMASS PaZ Pb_BOIL Pb_Cp Pb_DE"\
   "NSITY Pb_En Pb_MASS Pb_MELT PbMASS PbZ Pd_BOIL Pd_Cp Pd_DEN"\
   "SITY Pd_En Pd_MASS Pd_MELT PdMASS PdZ PENTANE_MASS PENTANEJ"\
   "PKG PENTANEJPKGTOTAL PENTANEJPMOLE PENTANEMASS Pm_BOIL Pm_C"\
   "p Pm_DENSITY Pm_En Pm_MASS Pm_MELT PMASS PmMASS PmZ Po_BOIL"\
   " Po_Cp Po_DENSITY Po_En Po_MASS Po_MELT PoMASS PoZ Pr_BOIL "\
   "Pr_Cp Pr_DENSITY Pr_En Pr_MASS Pr_MELT PrMASS PROPANE_MASS "\
   "PROPANEJPKG PROPANEJPKGTOTAL PROPANEJPMOLE PROPANEMASS PrZ "\
   "Pt_BOIL Pt_Cp Pt_DENSITY Pt_En Pt_MASS Pt_MELT PtMASS PtZ P"\
   "u_BOIL Pu_Cp Pu_DENSITY Pu_En Pu_MASS Pu_MELT PuMASS PuZ PZ"\
   " Ra_BOIL Ra_Cp Ra_DENSITY Ra_En Ra_MASS Ra_MELT RaMASS RaZ "\
   "Rb_BOIL Rb_Cp Rb_DENSITY Rb_En Rb_MASS Rb_MELT RbMASS RbZ R"\
   "e_BOIL Re_Cp Re_DENSITY Re_En Re_MASS Re_MELT ReMASS ReZ Rf"\
   "_BOIL Rf_Cp Rf_DENSITY Rf_En Rf_MASS Rf_MELT RfMASS RfZ Rg_"\
   "BOIL Rg_Cp Rg_DENSITY Rg_En Rg_MASS Rg_MELT RgMASS RgZ Rh_B"\
   "OIL Rh_Cp Rh_DENSITY Rh_En Rh_MASS Rh_MELT RhMASS RhZ Rn_BO"\
   "IL Rn_Cp Rn_DENSITY Rn_En Rn_MASS Rn_MELT RnMASS RnZ Ru_BOI"\
   "L Ru_Cp Ru_DENSITY Ru_En Ru_MASS Ru_MELT RuMASS RuZ S_BOIL "\
   "S_Cp S_DENSITY S_En S_MASS S_MELT Sb_BOIL Sb_Cp Sb_DENSITY "\
   "Sb_En Sb_MASS Sb_MELT SbMASS SbZ Sc_BOIL Sc_Cp Sc_DENSITY S"\
   "c_En Sc_MASS Sc_MELT ScMASS ScZ Se_BOIL Se_Cp Se_DENSITY Se"\
   "_En Se_MASS Se_MELT SeMASS SeZ Sg_BOIL Sg_Cp Sg_DENSITY Sg_"\
   "En Sg_MASS Sg_MELT SgMASS SgZ Si_BOIL Si_Cp Si_DENSITY Si_E"\
   "n Si_MASS Si_MELT SiMASS SiZ Sm_BOIL Sm_Cp Sm_DENSITY Sm_En"\
   " Sm_MASS Sm_MELT SMASS SmMASS SmZ Sn_BOIL Sn_Cp Sn_DENSITY "\
   "Sn_En Sn_MASS Sn_MELT SnMASS SnZ Sr_BOIL Sr_Cp Sr_DENSITY S"\
   "r_En Sr_MASS Sr_MELT SrMASS SrZ STEAM_Cp SUCROSE_MASS SUCRO"\
   "SEJPKG SUCROSEJPKGTOTAL SUCROSEJPMOLE SUCROSEMASS SUGAR_MAS"\
   "S SUGARDENSITY SUGARJPKG SUGARJPKGTOTAL SUGARJPMOLE SUGARMA"\
   "SS SZ Ta_BOIL Ta_Cp Ta_DENSITY Ta_En Ta_MASS Ta_MELT TaMASS"\
   " TaZ Tb_BOIL Tb_Cp Tb_DENSITY Tb_En Tb_MASS Tb_MELT TbMASS "\
   "TbZ Tc_BOIL Tc_Cp Tc_DENSITY Tc_En Tc_MASS Tc_MELT TcMASS T"\
   "cZ Te_BOIL Te_Cp Te_DENSITY Te_En Te_MASS Te_MELT TeMASS Te"\
   "Z Th_BOIL Th_Cp Th_DENSITY Th_En Th_MASS Th_MELT ThMASS ThZ"\
   " Ti_BOIL Ti_Cp Ti_DENSITY Ti_En Ti_MASS Ti_MELT TiMASS TiZ "\
   "Tl_BOIL Tl_Cp Tl_DENSITY Tl_En Tl_MASS Tl_MELT TlMASS TlZ T"\
   "m_BOIL Tm_Cp Tm_DENSITY Tm_En Tm_MASS Tm_MELT TmMASS TmZ TN"\
   "TJPKG Ts_BOIL Ts_Cp Ts_DENSITY Ts_En Ts_MASS Ts_MELT TsMASS"\
   " TsZ U_BOIL U_Cp U_DENSITY U_En U_MASS U_MELT UMASS UZ V_BO"\
   "IL V_Cp V_DENSITY V_En V_MASS V_MELT VMASS VSMOW_H1_RATIO V"\
   "SMOW_H2_RATIO VSMOW_H3_RATIO VSMOW_H_MASS VSMOW_MASS VSMOW_"\
   "O16_RATIO VSMOW_O17_RATIO VSMOW_O18_RATIO VSMOW_O_MASS VZ W"\
   "_BOIL W_Cp W_DENSITY W_En W_MASS W_MELT WATER_Cp WATER_FUSI"\
   "ON WATER_MASS WATER_VAPORIZATION WATERMASS WMASS WZ Xe_BOIL"\
   " Xe_Cp Xe_DENSITY Xe_En Xe_MASS Xe_MELT XeMASS XeZ Y_BOIL Y"\
   "_Cp Y_DENSITY Y_En Y_MASS Y_MELT Yb_BOIL Yb_Cp Yb_DENSITY Y"\
   "b_En Yb_MASS Yb_MELT YbMASS YbZ YMASS YZ Zn_BOIL Zn_Cp Zn_D"\
   "ENSITY Zn_En Zn_MASS Zn_MELT ZnMASS ZnZ Zr_BOIL Zr_Cp Zr_DE"\
   "NSITY Zr_En Zr_MASS Zr_MELT ZrMASS ZrZ"\
   "\n"
#define Kwelements_DESC \
   ...
   "IR(_MASS|MASS) Al_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) AlMASS Al"\
   "Z Am_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) AmMASS AmZ Ar_(BOIL|Cp"\
   "|DENSITY|En|M(ASS|ELT)) ArMASS ArZ As_(BOIL|Cp|DENSITY|En|M"\
   "(ASS|ELT)) AsMASS AsZ At_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) At"\
   "MASS AtZ Au_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) AuMASS AuZ B_BO"\
   "IL B_Cp B_DENSITY B_En B_M(ASS|ELT) Ba_(BOIL|Cp|DENSITY|En|"\
   "M(ASS|ELT)) BaMASS BaZ Be_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) B"\
   "eMASS BeZ Bh_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) BhMASS BhZ Bi_"\
   "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) BiMASS BiZ Bk_(BOIL|Cp|DENS"\
   "ITY|En|M(ASS|ELT)) BkMASS BkZ BMASS Br_(BOIL|Cp|DENSITY|En|"\
   "M(ASS|ELT)) BrMASS BrZ BUTANE(_MASS|JP(KG(TOTAL)?|MOLE)|MAS"\
   "S) BZ C12(_(MASS|RATIO)|H22O11(_MASS|MASS)|MASS) C13(_(MASS"\
   "|RATIO)|MASS) C14(_(MASS|RATIO)|MASS) C2H6(0(_MASS|MASS)|_M"\
   "ASS|MASS) C3H8(_MASS|MASS) C4H10(_MASS|MASS) C_BOIL C_Cp C_"\
   "DENSITY C_En C_M(ASS|E(AN_MASS|LT)) Ca_(BOIL|Cp|DENSITY|En|"\
   "M(ASS|ELT)) CaMASS CaZ Cd_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) C"\
   "dMASS CdZ Ce_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CeMASS CeZ Cf_"\
   "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CfMASS CfZ CH3CH2OH(_MASS|M"\
   "ASS) CH4(_MASS|MASS) Cl_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) ClM"\
   "ASS ClZ Cm_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CMASS CmMASS CmZ"\
   " Cn_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CnMASS CnZ Co_(BOIL|Cp|"\
   "DENSITY|En|M(ASS|ELT)) COAL(_MASS|JP(KG(TOTAL)?|MOLE)|MASS)"\
   " CoMASS CoZ Cr_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CrMASS CrZ C"\
   "s_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CsMASS CsZ Cu_(BOIL|Cp|DE"\
   "NSITY|En|M(ASS|ELT)) CuMASS CuZ CZ Db_(BOIL|Cp|DENSITY|En|M"\
   "(ASS|ELT)) DbMASS DbZ DIESEL(_MASS|DENSITY|JP(KG(TOTAL)?|MO"\
   "LE)|MASS) Ds_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) DsMASS DsZ Dy_"\
   "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) DyMASS DyZ Er_(BOIL|Cp|DENS"\
   "ITY|En|M(ASS|ELT)) ErMASS ErZ Es_(BOIL|Cp|DENSITY|En|M(ASS|"\
   "ELT)) EsMASS EsZ ETHAN(E(_MASS|JP(KG(TOTAL)?|MOLE)|MASS)|OL"\
   "(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS)) Eu_(BOIL|Cp|DENSI"\
   "TY|En|M(ASS|ELT)) EuMASS EuZ F_BOIL F_Cp F_DENSITY F_En F_M"\
   "(ASS|ELT) FAT(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS) Fe_(B"\
   "OIL|Cp|DENSITY|En|M(ASS|ELT)) FeMASS FeZ Fl_(BOIL|Cp|DENSIT"\
   "Y|En|M(ASS|ELT)) FlMASS FlZ Fm_(BOIL|Cp|DENSITY|En|M(ASS|EL"\
   "T)) FMASS FmMASS FmZ Fr_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) FrM"\
   "ASS FrZ FZ Ga_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) GaMASS GASOLI"\
   "NE(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS) GaZ Gd_(BOIL|Cp|"\
   "DENSITY|En|M(ASS|ELT)) GdMASS GdZ Ge_(BOIL|Cp|DENSITY|En|M("\
   "ASS|ELT)) GeMASS GeZ H1_MASS H1MASS H2_MASS H2MASS H3_MASS "\
   "H3MASS H_BOIL H_Cp H_DENSITY H_En H_M(ASS|ELT) He3(_MASS|MA"\
   "SS) He4(_MASS|MASS) He_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HeMA"\
   "SS HeZ Hf_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HfMASS HfZ Hg_(BO"\
   "IL|Cp|DENSITY|En|M(ASS|ELT)) HgMASS HgZ HMASS Ho_(BOIL|Cp|D"\
   "ENSITY|En|M(ASS|ELT)) HoMASS HoZ Hs_(BOIL|Cp|DENSITY|En|M(A"\
   "SS|ELT)) HsMASS HsZ HYDROGEN(_MASS|JP(KG(TOTAL)?|MOLE)|MASS"\
   ") HZ I_BOIL I_Cp I_DENSITY I_En I_M(ASS|ELT) ICE_Cp IMASS I"\
   "n_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) InMASS InZ Ir_(BOIL|Cp|DE"\
   "NSITY|En|M(ASS|ELT)) IrMASS IrZ IZ K_BOIL K_Cp K_DENSITY K_"\
   "En K_M(ASS|ELT) KMASS Kr_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) Kr"\
   "MASS KrZ KZ La_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) LaMASS LaZ L"\
   "i_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) LiMASS LiZ Lr_(BOIL|Cp|DE"\
   "NSITY|En|M(ASS|ELT)) LrMASS LrZ Lu_(BOIL|Cp|DENSITY|En|M(AS"\
   "S|ELT)) LuMASS LuZ Lv_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) LvMAS"\
   "S LvZ Mc_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) McMASS McZ Md_(BOI"\
   "L|Cp|DENSITY|En|M(ASS|ELT)) MdMASS MdZ METHANE(_MASS|JP(KG("\
   "TOTAL)?|MOLE)|MASS) Mg_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) MgMA"\
   "SS MgZ Mn_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) MnMASS MnZ Mo_(BO"\
   "IL|Cp|DENSITY|En|M(ASS|ELT)) MoMASS MoZ Mt_(BOIL|Cp|DENSITY"\
   "|En|M(ASS|ELT)) MtMASS MtZ N_BOIL N_Cp N_DENSITY N_En N_M(A"\
   "SS|ELT) Na_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NaMASS NaZ Nb_(B"\
   "OIL|Cp|DENSITY|En|M(ASS|ELT)) NbMASS NbZ Nd_(BOIL|Cp|DENSIT"\
   "Y|En|M(ASS|ELT)) NdMASS NdZ Ne_(BOIL|Cp|DENSITY|En|M(ASS|EL"\
   "T)) NeMASS NeZ Nh_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NhMASS Nh"\
   "Z Ni_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NiMASS NiZ NMASS No_(B"\
   "OIL|Cp|DENSITY|En|M(ASS|ELT)) NoMASS NoZ Np_(BOIL|Cp|DENSIT"\
   "Y|En|M(ASS|ELT)) NpMASS NpZ NZ O16(_MASS|MASS) O17(_MASS|MA"\
   "SS) O18(_MASS|MASS) O_BOIL O_Cp O_DENSITY O_En O_M(ASS|ELT)"\
   " Og_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) OgMASS OgZ OMASS Os_(BO"\
   "IL|Cp|DENSITY|En|M(ASS|ELT)) OsMASS OsZ OZ P_BOIL P_Cp P_DE"\
   "NSITY P_En P_M(ASS|ELT) Pa_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) "\
   "PaMASS PARAFFIN(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS) PaZ"\
   " Pb_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PbMASS PbZ Pd_(BOIL|Cp|"\
   "DENSITY|En|M(ASS|ELT)) PdMASS PdZ PENTANE(_MASS|JP(KG(TOTAL"\
   ")?|MOLE)|MASS) Pm_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PMASS PmM"\
   "ASS PmZ Po_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PoMASS PoZ Pr_(B"\
   "OIL|Cp|DENSITY|En|M(ASS|ELT)) PrMASS PROPANE(_MASS|JP(KG(TO"\
   "TAL)?|MOLE)|MASS) PrZ Pt_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) Pt"\
   "MASS PtZ Pu_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PuMASS PuZ PZ R"\
   "a_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RaMASS RaZ Rb_(BOIL|Cp|DE"\
   "NSITY|En|M(ASS|ELT)) RbMASS RbZ Re_(BOIL|Cp|DENSITY|En|M(AS"\
   "S|ELT)) ReMASS ReZ Rf_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RfMAS"\
   "S RfZ Rg_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RgMASS RgZ Rh_(BOI"\
   "L|Cp|DENSITY|En|M(ASS|ELT)) RhMASS RhZ Rn_(BOIL|Cp|DENSITY|"\
   "En|M(ASS|ELT)) RnMASS RnZ Ru_(BOIL|Cp|DENSITY|En|M(ASS|ELT)"\
   ") RuMASS RuZ S_BOIL S_Cp S_DENSITY S_En S_M(ASS|ELT) Sb_(BO"\
   "IL|Cp|DENSITY|En|M(ASS|ELT)) SbMASS SbZ Sc_(BOIL|Cp|DENSITY"\
   "|En|M(ASS|ELT)) ScMASS ScZ Se_(BOIL|Cp|DENSITY|En|M(ASS|ELT"\
   ")) SeMASS SeZ Sg_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) SgMASS SgZ"\
   " Si_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) SiMASS SiZ Sm_(BOIL|Cp|"\
   "DENSITY|En|M(ASS|ELT)) SMASS SmMASS SmZ Sn_(BOIL|Cp|DENSITY"\
   "|En|M(ASS|ELT)) SnMASS SnZ Sr_(BOIL|Cp|DENSITY|En|M(ASS|ELT"\
   ")) SrMASS SrZ STEAM_Cp SUCROSE(_MASS|JP(KG(TOTAL)?|MOLE)|MA"\
   "SS) SUGAR(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS) SZ Ta_(BO"\
   "IL|Cp|DENSITY|En|M(ASS|ELT)) TaMASS TaZ Tb_(BOIL|Cp|DENSITY"\
   "|En|M(ASS|ELT)) TbMASS TbZ Tc_(BOIL|Cp|DENSITY|En|M(ASS|ELT"\
   ")) TcMASS TcZ Te_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TeMASS TeZ"\
   " Th_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) ThMASS ThZ Ti_(BOIL|Cp|"\
   "DENSITY|En|M(ASS|ELT)) TiMASS TiZ Tl_(BOIL|Cp|DENSITY|En|M("\
   "ASS|ELT)) TlMASS TlZ Tm_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TmM"\
   "ASS TmZ TNTJPKG Ts_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TsMASS T"\
   "sZ U_BOIL U_Cp U_DENSITY U_En U_M(ASS|ELT) UMASS UZ V_BOIL "\
   "V_Cp V_DENSITY V_En V_M(ASS|ELT) VMASS VSMOW_(H(1_RATIO|2_R"\
   "ATIO|3_RATIO|_MASS)|MASS|O(1[678]_RATIO|_MASS)) VZ W_BOIL W"\
   "_Cp W_DENSITY W_En W_M(ASS|ELT) WATER(_(Cp|FUSION|MASS|VAPO"\
   "RIZATION)|MASS) WMASS WZ Xe_(BOIL|Cp|DENSITY|En|M(ASS|ELT))"\
   " XeMASS XeZ Y_BOIL Y_Cp Y_DENSITY Y_En Y_M(ASS|ELT) Yb_(BOI"\
   "L|Cp|DENSITY|En|M(ASS|ELT)) YbMASS YbZ YMASS YZ Zn_(BOIL|Cp"\
   "|DENSITY|En|M(ASS|ELT)) ZnMASS ZnZ Zr_(BOIL|Cp|DENSITY|En|M"\
   "(ASS|ELT)) ZrMASS ZrZ"\
   "\n"
