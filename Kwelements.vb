...
Public Const HYDROGENMASS As Double = 2 * HMASS + 0 * CMASS + _
      0 * NMASS + 0 * OMASS
Public Const HYDROGENJPMOLE As Double = 2.86E+5 * Kw.JOULE / _
      Kw.MOLE
...
Public Const METHANEMASS As Double = 4 * HMASS + 1 * CMASS + _
      0 * NMASS + 0 * OMASS
Public Const METHANEJPMOLE As Double = 8.89E+5 * Kw.JOULE / _
      Kw.MOLE
...
Public Const ETHANEMASS As Double = 6 * HMASS + 2 * CMASS + 0 _
      * NMASS + 0 * OMASS
Public Const ETHANEJPMOLE As Double = 1.560E+6 * Kw.JOULE / _
      Kw.MOLE
...
Public Const PROPANEMASS As Double = 8 * HMASS + 3 * CMASS + _
      0 * NMASS + 0 * OMASS
Public Const PROPANEJPMOLE As Double = 2.220E+6 * Kw.JOULE / _
      Kw.MOLE
...
Public Const BUTANEMASS As Double = 10 * HMASS + 4 * CMASS + _
      0 * NMASS + 0 * OMASS
Public Const BUTANEJPMOLE As Double = 2.877E+6 * Kw.JOULE / _
      Kw.MOLE
...
Public Const PENTANEMASS As Double = 12 * HMASS + 5 * CMASS + _
      0 * NMASS + 0 * OMASS
Public Const PENTANEJPMOLE As Double = 3.507E+6 * Kw.JOULE / _
      Kw.MOLE
Public Const PENTANEJPKG As Double = PENTANEJPMOLE / PENTANE_MASS
Public Const PENTANEJPKGTOTAL As Double = PENTANEJPMOLE / _
...
Public Const ETHANOLMASS As Double = 6 * HMASS + 2 * CMASS + _
      0 * NMASS + 1 * OMASS
Public Const ETHANOLJPMOLE As Double = 1.300E+6 * Kw.JOULE / _
      Kw.MOLE
...
Public Const SUCROSEMASS As Double = 22 * HMASS + 12 * CMASS _
      + 0 * NMASS + 11 * OMASS
Public Const SUCROSEJPMOLE As Double = 5.6467E+6 * Kw.JOULE / _
      Kw.MOLE
Public Const SUCROSEJPKG As Double = SUCROSEJPMOLE / SUCROSE_MASS
Public Const SUCROSEJPKGTOTAL As Double = SUCROSEJPMOLE / _
...
Public Function kwelements_description() As String
   ...
   desc_string = desc_string _
         & "AIRMASS Al_BOIL Al_Cp Al_DENSITY Al_En Al_MASS Al_" _
         & "MELT AlMASS AlZ Am_BOIL Am_Cp Am_DENSITY Am_En Am_" _
         & "MASS Am_MELT AmMASS AmZ Ar_BOIL Ar_Cp Ar_DENSITY A" _
         & "r_En Ar_MASS Ar_MELT ArMASS ArZ As_BOIL As_Cp As_D"
   desc_string = desc_string _
         & "ENSITY As_En As_MASS As_MELT AsMASS AsZ At_BOIL At" _
         & "_Cp At_DENSITY At_En At_MASS At_MELT AtMASS AtZ Au" _
         & "_BOIL Au_Cp Au_DENSITY Au_En Au_MASS Au_MELT AuMAS" _
         & "S AuZ B_BOIL B_Cp B_DENSITY B_En B_MASS B_MELT Ba_"
   desc_string = desc_string _
         & "BOIL Ba_Cp Ba_DENSITY Ba_En Ba_MASS Ba_MELT BaMASS" _
         & " BaZ Be_BOIL Be_Cp Be_DENSITY Be_En Be_MASS Be_MEL" _
         & "T BeMASS BeZ Bh_BOIL Bh_Cp Bh_DENSITY Bh_En Bh_MAS" _
         & "S Bh_MELT BhMASS BhZ Bi_BOIL Bi_Cp Bi_DENSITY Bi_E"
   desc_string = desc_string _
         & "n Bi_MASS Bi_MELT BiMASS BiZ Bk_BOIL Bk_Cp Bk_DENS" _
         & "ITY Bk_En Bk_MASS Bk_MELT BkMASS BkZ BMASS Br_BOIL" _
         & " Br_Cp Br_DENSITY Br_En Br_MASS Br_MELT BrMASS BrZ" _
         & " BUTANE_MASS BUTANEJPKG BUTANEJPKGTOTAL BUTANEJPMO"
   desc_string = desc_string _
         & "LE BUTANEMASS BZ C12_MASS C12_RATIO C12H22O11_MASS" _
         & " C12H22O11MASS C12MASS C13_MASS C13_RATIO C13MASS " _
         & "C14_MASS C14_RATIO C14MASS C2H60_MASS C2H60MASS C2" _
         & "H6_MASS C2H6MASS C3H8_MASS C3H8MASS C4H10_MASS C4H"
   desc_string = desc_string _
         & "10MASS C_BOIL C_Cp C_DENSITY C_En C_MASS C_MEAN_MA" _
         & "SS C_MELT Ca_BOIL Ca_Cp Ca_DENSITY Ca_En Ca_MASS C" _
         & "a_MELT CaMASS CaZ Cd_BOIL Cd_Cp Cd_DENSITY Cd_En C" _
         & "d_MASS Cd_MELT CdMASS CdZ Ce_BOIL Ce_Cp Ce_DENSITY"
   desc_string = desc_string _
         & " Ce_En Ce_MASS Ce_MELT CeMASS CeZ Cf_BOIL Cf_Cp Cf" _
         & "_DENSITY Cf_En Cf_MASS Cf_MELT CfMASS CfZ CH3CH2OH" _
         & "_MASS CH3CH2OHMASS CH4_MASS CH4MASS Cl_BOIL Cl_Cp " _
         & "Cl_DENSITY Cl_En Cl_MASS Cl_MELT ClMASS ClZ Cm_BOI"
   desc_string = desc_string _
         & "L Cm_Cp Cm_DENSITY Cm_En Cm_MASS Cm_MELT CMASS CmM" _
         & "ASS CmZ Cn_BOIL Cn_Cp Cn_DENSITY Cn_En Cn_MASS Cn_" _
         & "MELT CnMASS CnZ Co_BOIL Co_Cp Co_DENSITY Co_En Co_" _
         & "MASS Co_MELT COAL_MASS COALJPKG COALJPKGTOTAL COAL"
   desc_string = desc_string _
         & "JPMOLE COALMASS CoMASS CoZ Cr_BOIL Cr_Cp Cr_DENSIT" _
         & "Y Cr_En Cr_MASS Cr_MELT CrMASS CrZ Cs_BOIL Cs_Cp C" _
         & "s_DENSITY Cs_En Cs_MASS Cs_MELT CsMASS CsZ Cu_BOIL" _
         & " Cu_Cp Cu_DENSITY Cu_En Cu_MASS Cu_MELT CuMASS CuZ"
   desc_string = desc_string _
         & " CZ Db_BOIL Db_Cp Db_DENSITY Db_En Db_MASS Db_MELT" _
         & " DbMASS DbZ DIESEL_MASS DIESELDENSITY DIESELJPKG D" _
         & "IESELJPKGTOTAL DIESELJPMOLE DIESELMASS Ds_BOIL Ds_" _
         & "Cp Ds_DENSITY Ds_En Ds_MASS Ds_MELT DsMASS DsZ Dy_"
   desc_string = desc_string _
         & "BOIL Dy_Cp Dy_DENSITY Dy_En Dy_MASS Dy_MELT DyMASS" _
         & " DyZ Er_BOIL Er_Cp Er_DENSITY Er_En Er_MASS Er_MEL" _
         & "T ErMASS ErZ Es_BOIL Es_Cp Es_DENSITY Es_En Es_MAS" _
         & "S Es_MELT EsMASS EsZ ETHANE_MASS ETHANEJPKG ETHANE"
   desc_string = desc_string _
         & "JPKGTOTAL ETHANEJPMOLE ETHANEMASS ETHANOL_MASS ETH" _
         & "ANOLDENSITY ETHANOLJPKG ETHANOLJPKGTOTAL ETHANOLJP" _
         & "MOLE ETHANOLMASS Eu_BOIL Eu_Cp Eu_DENSITY Eu_En Eu" _
         & "_MASS Eu_MELT EuMASS EuZ F_BOIL F_Cp F_DENSITY F_E"
   desc_string = desc_string _
         & "n F_MASS F_MELT FAT_MASS FATDENSITY FATJPKG FATJPK" _
         & "GTOTAL FATJPMOLE FATMASS Fe_BOIL Fe_Cp Fe_DENSITY " _
         & "Fe_En Fe_MASS Fe_MELT FeMASS FeZ Fl_BOIL Fl_Cp Fl_" _
         & "DENSITY Fl_En Fl_MASS Fl_MELT FlMASS FlZ Fm_BOIL F"
   desc_string = desc_string _
         & "m_Cp Fm_DENSITY Fm_En Fm_MASS Fm_MELT FMASS FmMASS" _
         & " FmZ Fr_BOIL Fr_Cp Fr_DENSITY Fr_En Fr_MASS Fr_MEL" _
         & "T FrMASS FrZ FZ Ga_BOIL Ga_Cp Ga_DENSITY Ga_En Ga_" _
         & "MASS Ga_MELT GaMASS GASOLINE_MASS GASOLINEDENSITY "
   desc_string = desc_string _
         & "GASOLINEJPKG GASOLINEJPKGTOTAL GASOLINEJPMOLE GASO" _
         & "LINEMASS GaZ Gd_BOIL Gd_Cp Gd_DENSITY Gd_En Gd_MAS" _
         & "S Gd_MELT GdMASS GdZ Ge_BOIL Ge_Cp Ge_DENSITY Ge_E" _
         & "n Ge_MASS Ge_MELT GeMASS GeZ H1_MASS H1MASS H2_MAS"
   desc_string = desc_string _
         & "S H2MASS H3_MASS H3MASS H_BOIL H_Cp H_DENSITY H_En" _
         & " H_MASS H_MELT He3_MASS He3MASS He4_MASS He4MASS H" _
         & "e_BOIL He_Cp He_DENSITY He_En He_MASS He_MELT HeMA" _
         & "SS HeZ Hf_BOIL Hf_Cp Hf_DENSITY Hf_En Hf_MASS Hf_M"
   desc_string = desc_string _
         & "ELT HfMASS HfZ Hg_BOIL Hg_Cp Hg_DENSITY Hg_En Hg_M" _
         & "ASS Hg_MELT HgMASS HgZ HMASS Ho_BOIL Ho_Cp Ho_DENS" _
         & "ITY Ho_En Ho_MASS Ho_MELT HoMASS HoZ Hs_BOIL Hs_Cp" _
         & " Hs_DENSITY Hs_En Hs_MASS Hs_MELT HsMASS HsZ HYDRO"
   desc_string = desc_string _
         & "GEN_MASS HYDROGENJPKG HYDROGENJPKGTOTAL HYDROGENJP" _
         & "MOLE HYDROGENMASS HZ I_BOIL I_Cp I_DENSITY I_En I_" _
         & "MASS I_MELT ICE_Cp IMASS In_BOIL In_Cp In_DENSITY " _
         & "In_En In_MASS In_MELT InMASS InZ Ir_BOIL Ir_Cp Ir_"
   desc_string = desc_string _
         & "DENSITY Ir_En Ir_MASS Ir_MELT IrMASS IrZ IZ K_BOIL" _
         & " K_Cp K_DENSITY K_En K_MASS K_MELT KMASS Kr_BOIL K" _
         & "r_Cp Kr_DENSITY Kr_En Kr_MASS Kr_MELT KrMASS KrZ K" _
         & "Z La_BOIL La_Cp La_DENSITY La_En La_MASS La_MELT L"
   desc_string = desc_string _
         & "aMASS LaZ Li_BOIL Li_Cp Li_DENSITY Li_En Li_MASS L" _
         & "i_MELT LiMASS LiZ Lr_BOIL Lr_Cp Lr_DENSITY Lr_En L" _
         & "r_MASS Lr_MELT LrMASS LrZ Lu_BOIL Lu_Cp Lu_DENSITY" _
         & " Lu_En Lu_MASS Lu_MELT LuMASS LuZ Lv_BOIL Lv_Cp Lv"
   desc_string = desc_string _
         & "_DENSITY Lv_En Lv_MASS Lv_MELT LvMASS LvZ Mc_BOIL " _
         & "Mc_Cp Mc_DENSITY Mc_En Mc_MASS Mc_MELT McMASS McZ " _
         & "Md_BOIL Md_Cp Md_DENSITY Md_En Md_MASS Md_MELT MdM" _
         & "ASS MdZ METHANE_MASS METHANEJPKG METHANEJPKGTOTAL "
   desc_string = desc_string _
         & "METHANEJPMOLE METHANEMASS Mg_BOIL Mg_Cp Mg_DENSITY" _
         & " Mg_En Mg_MASS Mg_MELT MgMASS MgZ Mn_BOIL Mn_Cp Mn" _
         & "_DENSITY Mn_En Mn_MASS Mn_MELT MnMASS MnZ Mo_BOIL " _
         & "Mo_Cp Mo_DENSITY Mo_En Mo_MASS Mo_MELT MoMASS MoZ "
   desc_string = desc_string _
         & "Mt_BOIL Mt_Cp Mt_DENSITY Mt_En Mt_MASS Mt_MELT MtM" _
         & "ASS MtZ N_BOIL N_Cp N_DENSITY N_En N_MASS N_MELT N" _
         & "a_BOIL Na_Cp Na_DENSITY Na_En Na_MASS Na_MELT NaMA" _
         & "SS NaZ Nb_BOIL Nb_Cp Nb_DENSITY Nb_En Nb_MASS Nb_M"
   desc_string = desc_string _
         & "ELT NbMASS NbZ Nd_BOIL Nd_Cp Nd_DENSITY Nd_En Nd_M" _
         & "ASS Nd_MELT NdMASS NdZ Ne_BOIL Ne_Cp Ne_DENSITY Ne" _
         & "_En Ne_MASS Ne_MELT NeMASS NeZ Nh_BOIL Nh_Cp Nh_DE" _
         & "NSITY Nh_En Nh_MASS Nh_MELT NhMASS NhZ Ni_BOIL Ni_"
   desc_string = desc_string _
         & "Cp Ni_DENSITY Ni_En Ni_MASS Ni_MELT NiMASS NiZ NMA" _
         & "SS No_BOIL No_Cp No_DENSITY No_En No_MASS No_MELT " _
         & "NoMASS NoZ Np_BOIL Np_Cp Np_DENSITY Np_En Np_MASS " _
         & "Np_MELT NpMASS NpZ NZ O16_MASS O16MASS O17_MASS O1"
   desc_string = desc_string _
         & "7MASS O18_MASS O18MASS O_BOIL O_Cp O_DENSITY O_En " _
         & "O_MASS O_MELT Og_BOIL Og_Cp Og_DENSITY Og_En Og_MA" _
         & "SS Og_MELT OgMASS OgZ OMASS Os_BOIL Os_Cp Os_DENSI" _
         & "TY Os_En Os_MASS Os_MELT OsMASS OsZ OZ P_BOIL P_Cp"
   desc_string = desc_string _
         & " P_DENSITY P_En P_MASS P_MELT Pa_BOIL Pa_Cp Pa_DEN" _
         & "SITY Pa_En Pa_MASS Pa_MELT PaMASS PARAFFIN_MASS PA" _
         & "RAFFINDENSITY PARAFFINJPKG PARAFFINJPKGTOTAL PARAF" _
         & "FINJPMOLE PARAFFINMASS PaZ Pb_BOIL Pb_Cp Pb_DENSIT"
   desc_string = desc_string _
         & "Y Pb_En Pb_MASS Pb_MELT PbMASS PbZ Pd_BOIL Pd_Cp P" _
         & "d_DENSITY Pd_En Pd_MASS Pd_MELT PdMASS PdZ PENTANE" _
         & "_MASS PENTANEJPKG PENTANEJPKGTOTAL PENTANEJPMOLE P" _
         & "ENTANEMASS Pm_BOIL Pm_Cp Pm_DENSITY Pm_En Pm_MASS "
   desc_string = desc_string _
         & "Pm_MELT PMASS PmMASS PmZ Po_BOIL Po_Cp Po_DENSITY " _
         & "Po_En Po_MASS Po_MELT PoMASS PoZ Pr_BOIL Pr_Cp Pr_" _
         & "DENSITY Pr_En Pr_MASS Pr_MELT PrMASS PROPANE_MASS " _
         & "PROPANEJPKG PROPANEJPKGTOTAL PROPANEJPMOLE PROPANE"
   desc_string = desc_string _
         & "MASS PrZ Pt_BOIL Pt_Cp Pt_DENSITY Pt_En Pt_MASS Pt" _
         & "_MELT PtMASS PtZ Pu_BOIL Pu_Cp Pu_DENSITY Pu_En Pu" _
         & "_MASS Pu_MELT PuMASS PuZ PZ Ra_BOIL Ra_Cp Ra_DENSI" _
         & "TY Ra_En Ra_MASS Ra_MELT RaMASS RaZ Rb_BOIL Rb_Cp "
   desc_string = desc_string _
         & "Rb_DENSITY Rb_En Rb_MASS Rb_MELT RbMASS RbZ Re_BOI" _
         & "L Re_Cp Re_DENSITY Re_En Re_MASS Re_MELT ReMASS Re" _
         & "Z Rf_BOIL Rf_Cp Rf_DENSITY Rf_En Rf_MASS Rf_MELT R" _
         & "fMASS RfZ Rg_BOIL Rg_Cp Rg_DENSITY Rg_En Rg_MASS R"
   desc_string = desc_string _
         & "g_MELT RgMASS RgZ Rh_BOIL Rh_Cp Rh_DENSITY Rh_En R" _
         & "h_MASS Rh_MELT RhMASS RhZ Rn_BOIL Rn_Cp Rn_DENSITY" _
         & " Rn_En Rn_MASS Rn_MELT RnMASS RnZ Ru_BOIL Ru_Cp Ru" _
         & "_DENSITY Ru_En Ru_MASS Ru_MELT RuMASS RuZ S_BOIL S"
   desc_string = desc_string _
         & "_Cp S_DENSITY S_En S_MASS S_MELT Sb_BOIL Sb_Cp Sb_" _
         & "DENSITY Sb_En Sb_MASS Sb_MELT SbMASS SbZ Sc_BOIL S" _
         & "c_Cp Sc_DENSITY Sc_En Sc_MASS Sc_MELT ScMASS ScZ S" _
         & "e_BOIL Se_Cp Se_DENSITY Se_En Se_MASS Se_MELT SeMA"
   desc_string = desc_string _
         & "SS SeZ Sg_BOIL Sg_Cp Sg_DENSITY Sg_En Sg_MASS Sg_M" _
         & "ELT SgMASS SgZ Si_BOIL Si_Cp Si_DENSITY Si_En Si_M" _
         & "ASS Si_MELT SiMASS SiZ Sm_BOIL Sm_Cp Sm_DENSITY Sm" _
         & "_En Sm_MASS Sm_MELT SMASS SmMASS SmZ Sn_BOIL Sn_Cp"
   desc_string = desc_string _
         & " Sn_DENSITY Sn_En Sn_MASS Sn_MELT SnMASS SnZ Sr_BO" _
         & "IL Sr_Cp Sr_DENSITY Sr_En Sr_MASS Sr_MELT SrMASS S" _
         & "rZ STEAM_Cp SUCROSE_MASS SUCROSEJPKG SUCROSEJPKGTO" _
         & "TAL SUCROSEJPMOLE SUCROSEMASS SUGAR_MASS SUGARDENS"
   desc_string = desc_string _
         & "ITY SUGARJPKG SUGARJPKGTOTAL SUGARJPMOLE SUGARMASS" _
         & " SZ Ta_BOIL Ta_Cp Ta_DENSITY Ta_En Ta_MASS Ta_MELT" _
         & " TaMASS TaZ Tb_BOIL Tb_Cp Tb_DENSITY Tb_En Tb_MASS" _
         & " Tb_MELT TbMASS TbZ Tc_BOIL Tc_Cp Tc_DENSITY Tc_En"
   desc_string = desc_string _
         & " Tc_MASS Tc_MELT TcMASS TcZ Te_BOIL Te_Cp Te_DENSI" _
         & "TY Te_En Te_MASS Te_MELT TeMASS TeZ Th_BOIL Th_Cp " _
         & "Th_DENSITY Th_En Th_MASS Th_MELT ThMASS ThZ Ti_BOI" _
         & "L Ti_Cp Ti_DENSITY Ti_En Ti_MASS Ti_MELT TiMASS Ti"
   desc_string = desc_string _
         & "Z Tl_BOIL Tl_Cp Tl_DENSITY Tl_En Tl_MASS Tl_MELT T" _
         & "lMASS TlZ Tm_BOIL Tm_Cp Tm_DENSITY Tm_En Tm_MASS T" _
         & "m_MELT TmMASS TmZ TNTJPKG Ts_BOIL Ts_Cp Ts_DENSITY" _
         & " Ts_En Ts_MASS Ts_MELT TsMASS TsZ U_BOIL U_Cp U_DE"
   desc_string = desc_string _
         & "NSITY U_En U_MASS U_MELT UMASS UZ V_BOIL V_Cp V_DE" _
         & "NSITY V_En V_MASS V_MELT VMASS VSMOW_H1_RATIO VSMO" _
         & "W_H2_RATIO VSMOW_H3_RATIO VSMOW_H_MASS VSMOW_MASS " _
         & "VSMOW_O16_RATIO VSMOW_O17_RATIO VSMOW_O18_RATIO VS"
   desc_string = desc_string _
         & "MOW_O_MASS VZ W_BOIL W_Cp W_DENSITY W_En W_MASS W_" _
         & "MELT WATER_Cp WATER_FUSION WATER_MASS WATER_VAPORI" _
         & "ZATION WATERMASS WMASS WZ Xe_BOIL Xe_Cp Xe_DENSITY" _
         & " Xe_En Xe_MASS Xe_MELT XeMASS XeZ Y_BOIL Y_Cp Y_DE"
   desc_string = desc_string _
         & "NSITY Y_En Y_MASS Y_MELT Yb_BOIL Yb_Cp Yb_DENSITY " _
         & "Yb_En Yb_MASS Yb_MELT YbMASS YbZ YMASS YZ Zn_BOIL " _
         & "Zn_Cp Zn_DENSITY Zn_En Zn_MASS Zn_MELT ZnMASS ZnZ " _
         & "Zr_BOIL Zr_Cp Zr_DENSITY Zr_En Zr_MASS Zr_MELT ZrM" _
         & "ASS ZrZ"
...
Public Function kwelements_desc() As String
   ...
   desc_string = desc_string _
         ...
         & "ELT)) AgMASS AgZ AIR(_MASS|MASS) Al_(BOIL|Cp|DENSI"
   desc_string = desc_string _
         & "TY|En|M(ASS|ELT)) AlMASS AlZ Am_(BOIL|Cp|DENSITY|E" _
         & "n|M(ASS|ELT)) AmMASS AmZ Ar_(BOIL|Cp|DENSITY|En|M(" _
         & "ASS|ELT)) ArMASS ArZ As_(BOIL|Cp|DENSITY|En|M(ASS|" _
         & "ELT)) AsMASS AsZ At_(BOIL|Cp|DENSITY|En|M(ASS|ELT)"
   desc_string = desc_string _
         & ") AtMASS AtZ Au_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) Au" _
         & "MASS AuZ B_BOIL B_Cp B_DENSITY B_En B_M(ASS|ELT) B" _
         & "a_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) BaMASS BaZ Be_(B" _
         & "OIL|Cp|DENSITY|En|M(ASS|ELT)) BeMASS BeZ Bh_(BOIL|"
   desc_string = desc_string _
         & "Cp|DENSITY|En|M(ASS|ELT)) BhMASS BhZ Bi_(BOIL|Cp|D" _
         & "ENSITY|En|M(ASS|ELT)) BiMASS BiZ Bk_(BOIL|Cp|DENSI" _
         & "TY|En|M(ASS|ELT)) BkMASS BkZ BMASS Br_(BOIL|Cp|DEN" _
         & "SITY|En|M(ASS|ELT)) BrMASS BrZ BUTANE(_MASS|JP(KG("
   desc_string = desc_string _
         & "TOTAL)?|MOLE)|MASS) BZ C12(_(MASS|RATIO)|H22O11(_M" _
         & "ASS|MASS)|MASS) C13(_(MASS|RATIO)|MASS) C14(_(MASS" _
         & "|RATIO)|MASS) C2H6(0(_MASS|MASS)|_MASS|MASS) C3H8(" _
         & "_MASS|MASS) C4H10(_MASS|MASS) C_BOIL C_Cp C_DENSIT"
   desc_string = desc_string _
         & "Y C_En C_M(ASS|E(AN_MASS|LT)) Ca_(BOIL|Cp|DENSITY|" _
         & "En|M(ASS|ELT)) CaMASS CaZ Cd_(BOIL|Cp|DENSITY|En|M" _
         & "(ASS|ELT)) CdMASS CdZ Ce_(BOIL|Cp|DENSITY|En|M(ASS" _
         & "|ELT)) CeMASS CeZ Cf_(BOIL|Cp|DENSITY|En|M(ASS|ELT"
   desc_string = desc_string _
         & ")) CfMASS CfZ CH3CH2OH(_MASS|MASS) CH4(_MASS|MASS)" _
         & " Cl_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) ClMASS ClZ Cm_" _
         & "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CMASS CmMASS CmZ C" _
         & "n_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) CnMASS CnZ Co_(B"
   desc_string = desc_string _
         & "OIL|Cp|DENSITY|En|M(ASS|ELT)) COAL(_MASS|JP(KG(TOT" _
         & "AL)?|MOLE)|MASS) CoMASS CoZ Cr_(BOIL|Cp|DENSITY|En" _
         & "|M(ASS|ELT)) CrMASS CrZ Cs_(BOIL|Cp|DENSITY|En|M(A" _
         & "SS|ELT)) CsMASS CsZ Cu_(BOIL|Cp|DENSITY|En|M(ASS|E"
   desc_string = desc_string _
         & "LT)) CuMASS CuZ CZ Db_(BOIL|Cp|DENSITY|En|M(ASS|EL" _
         & "T)) DbMASS DbZ DIESEL(_MASS|DENSITY|JP(KG(TOTAL)?|" _
         & "MOLE)|MASS) Ds_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) DsM" _
         & "ASS DsZ Dy_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) DyMASS "
   desc_string = desc_string _
         & "DyZ Er_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) ErMASS ErZ " _
         & "Es_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) EsMASS EsZ ETHA" _
         & "N(E(_MASS|JP(KG(TOTAL)?|MOLE)|MASS)|OL(_MASS|DENSI" _
         & "TY|JP(KG(TOTAL)?|MOLE)|MASS)) Eu_(BOIL|Cp|DENSITY|"
   desc_string = desc_string _
         & "En|M(ASS|ELT)) EuMASS EuZ F_BOIL F_Cp F_DENSITY F_" _
         & "En F_M(ASS|ELT) FAT(_MASS|DENSITY|JP(KG(TOTAL)?|MO" _
         & "LE)|MASS) Fe_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) FeMAS" _
         & "S FeZ Fl_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) FlMASS Fl"
   desc_string = desc_string _
         & "Z Fm_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) FMASS FmMASS " _
         & "FmZ Fr_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) FrMASS FrZ " _
         & "FZ Ga_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) GaMASS GASOL" _
         & "INE(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|MASS) GaZ Gd"
   desc_string = desc_string _
         & "_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) GdMASS GdZ Ge_(BO" _
         & "IL|Cp|DENSITY|En|M(ASS|ELT)) GeMASS GeZ H1_MASS H1" _
         & "MASS H2_MASS H2MASS H3_MASS H3MASS H_BOIL H_Cp H_D" _
         & "ENSITY H_En H_M(ASS|ELT) He3(_MASS|MASS) He4(_MASS"
   desc_string = desc_string _
         & "|MASS) He_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HeMASS H" _
         & "eZ Hf_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HfMASS HfZ H" _
         & "g_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HgMASS HgZ HMASS" _
         & " Ho_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HoMASS HoZ Hs_"
   desc_string = desc_string _
         & "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) HsMASS HsZ HYDROGE" _
         & "N(_MASS|JP(KG(TOTAL)?|MOLE)|MASS) HZ I_BOIL I_Cp I" _
         & "_DENSITY I_En I_M(ASS|ELT) ICE_Cp IMASS In_(BOIL|C" _
         & "p|DENSITY|En|M(ASS|ELT)) InMASS InZ Ir_(BOIL|Cp|DE"
   desc_string = desc_string _
         & "NSITY|En|M(ASS|ELT)) IrMASS IrZ IZ K_BOIL K_Cp K_D" _
         & "ENSITY K_En K_M(ASS|ELT) KMASS Kr_(BOIL|Cp|DENSITY" _
         & "|En|M(ASS|ELT)) KrMASS KrZ KZ La_(BOIL|Cp|DENSITY|" _
         & "En|M(ASS|ELT)) LaMASS LaZ Li_(BOIL|Cp|DENSITY|En|M"
   desc_string = desc_string _
         & "(ASS|ELT)) LiMASS LiZ Lr_(BOIL|Cp|DENSITY|En|M(ASS" _
         & "|ELT)) LrMASS LrZ Lu_(BOIL|Cp|DENSITY|En|M(ASS|ELT" _
         & ")) LuMASS LuZ Lv_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) L" _
         & "vMASS LvZ Mc_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) McMAS"
   desc_string = desc_string _
         & "S McZ Md_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) MdMASS Md" _
         & "Z METHANE(_MASS|JP(KG(TOTAL)?|MOLE)|MASS) Mg_(BOIL" _
         & "|Cp|DENSITY|En|M(ASS|ELT)) MgMASS MgZ Mn_(BOIL|Cp|" _
         & "DENSITY|En|M(ASS|ELT)) MnMASS MnZ Mo_(BOIL|Cp|DENS"
   desc_string = desc_string _
         & "ITY|En|M(ASS|ELT)) MoMASS MoZ Mt_(BOIL|Cp|DENSITY|" _
         & "En|M(ASS|ELT)) MtMASS MtZ N_BOIL N_Cp N_DENSITY N_" _
         & "En N_M(ASS|ELT) Na_(BOIL|Cp|DENSITY|En|M(ASS|ELT))" _
         & " NaMASS NaZ Nb_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NbM"
   desc_string = desc_string _
         & "ASS NbZ Nd_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NdMASS " _
         & "NdZ Ne_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NeMASS NeZ " _
         & "Nh_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NhMASS NhZ Ni_(" _
         & "BOIL|Cp|DENSITY|En|M(ASS|ELT)) NiMASS NiZ NMASS No"
   desc_string = desc_string _
         & "_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) NoMASS NoZ Np_(BO" _
         & "IL|Cp|DENSITY|En|M(ASS|ELT)) NpMASS NpZ NZ O16(_MA" _
         & "SS|MASS) O17(_MASS|MASS) O18(_MASS|MASS) O_BOIL O_" _
         & "Cp O_DENSITY O_En O_M(ASS|ELT) Og_(BOIL|Cp|DENSITY"
   desc_string = desc_string _
         & "|En|M(ASS|ELT)) OgMASS OgZ OMASS Os_(BOIL|Cp|DENSI" _
         & "TY|En|M(ASS|ELT)) OsMASS OsZ OZ P_BOIL P_Cp P_DENS" _
         & "ITY P_En P_M(ASS|ELT) Pa_(BOIL|Cp|DENSITY|En|M(ASS" _
         & "|ELT)) PaMASS PARAFFIN(_MASS|DENSITY|JP(KG(TOTAL)?"
   desc_string = desc_string _
         & "|MOLE)|MASS) PaZ Pb_(BOIL|Cp|DENSITY|En|M(ASS|ELT)" _
         & ") PbMASS PbZ Pd_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) Pd" _
         & "MASS PdZ PENTANE(_MASS|JP(KG(TOTAL)?|MOLE)|MASS) P" _
         & "m_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PMASS PmMASS PmZ"
   desc_string = desc_string _
         & " Po_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PoMASS PoZ Pr_" _
         & "(BOIL|Cp|DENSITY|En|M(ASS|ELT)) PrMASS PROPANE(_MA" _
         & "SS|JP(KG(TOTAL)?|MOLE)|MASS) PrZ Pt_(BOIL|Cp|DENSI" _
         & "TY|En|M(ASS|ELT)) PtMASS PtZ Pu_(BOIL|Cp|DENSITY|E"
   desc_string = desc_string _
         & "n|M(ASS|ELT)) PuMASS PuZ PZ Ra_(BOIL|Cp|DENSITY|En" _
         & "|M(ASS|ELT)) RaMASS RaZ Rb_(BOIL|Cp|DENSITY|En|M(A" _
         & "SS|ELT)) RbMASS RbZ Re_(BOIL|Cp|DENSITY|En|M(ASS|E" _
         & "LT)) ReMASS ReZ Rf_(BOIL|Cp|DENSITY|En|M(ASS|ELT))"
   desc_string = desc_string _
         & " RfMASS RfZ Rg_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RgM" _
         & "ASS RgZ Rh_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RhMASS " _
         & "RhZ Rn_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RnMASS RnZ " _
         & "Ru_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) RuMASS RuZ S_BO"
   desc_string = desc_string _
         & "IL S_Cp S_DENSITY S_En S_M(ASS|ELT) Sb_(BOIL|Cp|DE" _
         & "NSITY|En|M(ASS|ELT)) SbMASS SbZ Sc_(BOIL|Cp|DENSIT" _
         & "Y|En|M(ASS|ELT)) ScMASS ScZ Se_(BOIL|Cp|DENSITY|En" _
         & "|M(ASS|ELT)) SeMASS SeZ Sg_(BOIL|Cp|DENSITY|En|M(A"
   desc_string = desc_string _
         & "SS|ELT)) SgMASS SgZ Si_(BOIL|Cp|DENSITY|En|M(ASS|E" _
         & "LT)) SiMASS SiZ Sm_(BOIL|Cp|DENSITY|En|M(ASS|ELT))" _
         & " SMASS SmMASS SmZ Sn_(BOIL|Cp|DENSITY|En|M(ASS|ELT" _
         & ")) SnMASS SnZ Sr_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) S"
   desc_string = desc_string _
         & "rMASS SrZ STEAM_Cp SUCROSE(_MASS|JP(KG(TOTAL)?|MOL" _
         & "E)|MASS) SUGAR(_MASS|DENSITY|JP(KG(TOTAL)?|MOLE)|M" _
         & "ASS) SZ Ta_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TaMASS " _
         & "TaZ Tb_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TbMASS TbZ "
   desc_string = desc_string _
         & "Tc_(BOIL|Cp|DENSITY|En|M(ASS|ELT)) TcMASS TcZ Te_(" _
         & "BOIL|Cp|DENSITY|En|M(ASS|ELT)) TeMASS TeZ Th_(BOIL" _
         & "|Cp|DENSITY|En|M(ASS|ELT)) ThMASS ThZ Ti_(BOIL|Cp|" _
         & "DENSITY|En|M(ASS|ELT)) TiMASS TiZ Tl_(BOIL|Cp|DENS"
   desc_string = desc_string _
         & "ITY|En|M(ASS|ELT)) TlMASS TlZ Tm_(BOIL|Cp|DENSITY|" _
         & "En|M(ASS|ELT)) TmMASS TmZ TNTJPKG Ts_(BOIL|Cp|DENS" _
         & "ITY|En|M(ASS|ELT)) TsMASS TsZ U_BOIL U_Cp U_DENSIT" _
         & "Y U_En U_M(ASS|ELT) UMASS UZ V_BOIL V_Cp V_DENSITY"
   desc_string = desc_string _
         & " V_En V_M(ASS|ELT) VMASS VSMOW_(H(1_RATIO|2_RATIO|" _
         & "3_RATIO|_MASS)|MASS|O(1[678]_RATIO|_MASS)) VZ W_BO" _
         & "IL W_Cp W_DENSITY W_En W_M(ASS|ELT) WATER(_(Cp|FUS" _
         & "ION|MASS|VAPORIZATION)|MASS) WMASS WZ Xe_(BOIL|Cp|"
   desc_string = desc_string _
         & "DENSITY|En|M(ASS|ELT)) XeMASS XeZ Y_BOIL Y_Cp Y_DE" _
         & "NSITY Y_En Y_M(ASS|ELT) Yb_(BOIL|Cp|DENSITY|En|M(A" _
         & "SS|ELT)) YbMASS YbZ YMASS YZ Zn_(BOIL|Cp|DENSITY|E" _
         & "n|M(ASS|ELT)) ZnMASS ZnZ Zr_(BOIL|Cp|DENSITY|En|M(" _
         & "ASS|ELT)) ZrMASS ZrZ"
