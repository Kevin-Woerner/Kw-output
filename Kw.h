...
#define Kw_DESCRIPTION \
   ...
   "liptic1__2 elliptic2 elliptic2__1 ellipticE ellipticK erf e"\
   "rf__1 erf__2 erf__3 erf__4 erfc erfcinv erfcx erfcxinv erfi"\
   "nv eta eulerphi eulerx exp exp__1 exp__2 exp_rand expintegr"\
   "al1 expintegrali expm1 fac facinv factorial factorialdouble"\
   " factorialinv fibo fibox fiboxinv flat2ecc floor frac fubin"\
   "i fusc g2jewish gamma_rand gau_cdf gau_pdf gau_quantile gd "\
   "gdinv geo_rand ghmean1 ghmean1inv glmean1 gmean1 gqmean1 ha"\
   "s_primitiveroot haversin heaviside heronianmean1 hlmean1 hm"\
   "ean1 hn hn__1 hour2day hqmean1 hypot1 hypot1inv id int_rand"\
   " iround iseven isint isjewish8short isjewish9short isjewish"\
   "leap isleapyear ismagic isneg isnegint isodd ispos isposint"\
   " ispractical isprime issquarefree iszero j2k2dow j2k2jd j2k"\
   "2kwt j2k2uet j2k2ymdhms jacobicir jd2dow jd2j2k jd2kwt jd2u"\
   "et jd2ymdhms jewish2g jewish2jd jewish2jdx jewish_months_in"\
   "_year jewish_yearlength k2m k2tempc k2tempf k2tempr ks_a_cd"\
   "f kwt2j2k kwt2jd kwt2uet l10 lg lgamma lgamma__1 lgamma__2 "\
   "lgammainv lix lixinv lmean1 ln ln1p ln__1 ln__2 lns log log"\
   "10 logintegral logistic_rand lqmean1 lucas lucasx m2b m2k m"\
   "ertens minkowski mobius neg nom normal_cdf normal_cdf__1 no"\
   "rmal_pdf normal_quantile normal_quantile__1 parabola_get pa"\
   "rabola_roots partition partitionq pcf pfg pfl poi_rand pow1"\
   "0 pow2 prime0 prime1 primecount primecountx primenext prime"\
   "nth primenth__1 primeprev primitiveroot print_contfra print"\
   "_contfrac print_contfracd print_factor print_jd264 print_nu"\
   "m2char qmean1 rad2deg rad2dms rad2dms2 rad2sec rad2tur rand"\
   "_init randl reci reimann round rto125 rtoi rtoz sba search_"\
   "check search_search sec sec2deg sec2dhms sec2rad sec2tur se"\
   "cd secdinv sech sechinv secinv sect sectinv shc_shft2steps "\
   "shc_steps2shft shc_steps2shftinv sigma0 sigma1 sigmoid sigm"\
   "oidinv sign sin sin__1 sinc sinc__1 sincc sincinv sind sind"\
   "inv sinh sinhinv sinintegral sininv sint sintinv smooth sq "\
   "sqinv sqrt sqrt__1 sqrt__2 sqrt__3 sqrtx square squareroot "\
   "stair stts_delete stts_reset sttscount sttsmagic sttsmax st"\
   "tsmean sttsmin sttsmsq sttsspread sttsstdev sttsstdevmean s"\
   "ttssum sttsvar supercatalan tan tand tandinv tanh tanhinv t"\
   "aninv tant tantinv tempc tempc2k tempc2tempf tempf tempf2k "\
   "tempf2tempc tempr tempr2k tgamma tgamma__1 tgamma__2 tgamma"\
   "__3 tgamma__stirling tgammadouble tgammainv topyh1 totient "\
   "tri tri_rand triinv trip trunc tur2deg tur2rad tur2sec uet2"\
   "j2k uet2jd uet2kwt uni_rand versin wm wminv wp wpinv ymdhms"\
   "2j2k ymdhms2jd ymdhms_get zag zeta zig zigzag"\
   "\n"\
   ...
   "t isle islt ismult ismultl isneq ispractical_f isprimitiver"\
   "oot issignsame issquarefree_f isss jacobiphi jewish_monthbe"\
   "gin jewish_monthlength ks_b_cdf ks_cdf ks_quantile lbeta lc"\
   "am lcams lcantim lcm lcombx ligamma ligammainv lmean logist"\
   "ic_cdf logistic_pdf logistic_quantile lpermx lqmean max2 ma"\
   "xabs maxabs2 maxi mb_cdf mb_pdf mean min2 minabs minabs2 mi"\
   "ni mobius_f mods modulo nbd_rand nc nd nevillethetac nevill"\
   "ethetad nevillethetan nevillethetas ns nthroot__1 pareto_ra"\
   "nd perm permutation permx poi_cdf poi_pdf poi_quantile poly"\
   "a_rand polycnum polynum pow powi prime0_f print_base_out pr"\
   "int_base_outf qmean rand_init_array ratio repetendlen rliga"\
   "mma rms round2 rtomsd ruigamma sc sd shc_shftpoles2steps si"\
   "gma sigma0_f sigma1_f sn sn__1 spread2stdev stdev2spread st"\
   "irling1 stirling2 stts_copy stts_downdate stts_newx stts_sc"\
   "ale stts_shift stts_update stts_update2 t_cdf t_cdf__1 t_pd"\
   "f t_quantile t_quantile__1 tetracnum tetranum theta1 theta2"\
   " theta3 theta4 Tn topyh totient_f trap_rand tri_cdf tri_pdf"\
   " tri_quantile uigamma uigamma__1 uigammainv Un uni_cdf uni_"\
   "pdf uni_quantile variance zeta_cdf zeta_pdf zipf_rand znord"\
   "er znorder__1"\
   "\n"\
...
#define Kw_DESC \
   ...
   "elliptic(1(__[12])?|2(__1)?|[EK]) erf(|__([12]|[34])|c(inv|"\
   "x(inv)?)?|inv) eta euler(phi|x) exp(_(_[12]|rand)|integral["\
   "1i]|m1)? fac(inv|torial(double|inv)?)? fibo(x(inv)?)? flat2"\
   "ecc floor frac fubini fusc g2jewish gamma_rand gau_(cdf|pdf"\
   "|quantile) gd gdinv geo_rand ghmean1(inv)? glmean1 gmean1 g"\
   "qmean1 has_primitiveroot haversin heaviside heronianmean1 h"\
   "lmean1 hmean1 hn hn__1 hour2day hqmean1 hypot1(inv)? id int"\
   "_rand iround is(even|int|jewish8short|jewish9short|jewishle"\
   "ap|leapyear|magic|neg|negint|odd|pos|posint|practical|prime"\
   "|squarefree|zero) j2k2(dow|jd|kwt|uet|ymdhms) jacobicir jd2"\
   "(dow|j2k|kwt|uet|ymdhms) jewish(2(g|jdx?)|_(months_in_year|"\
   "yearlength)) k2m k2temp[cfr] ks_a_cdf kwt2(j(2k|d)|uet) l10"\
   " lg lgamma(__[12]|inv)? lix(inv)? lmean1 ln ln1p ln__[12] l"\
   "ns log(10|i(ntegral|stic_rand))? lqmean1 lucasx? m2b m2k me"\
   "rtens minkowski mobius neg nom normal_(cdf(__1)?|pdf|quanti"\
   "le(__1)?) par(abola_(get|roots)|titionq?) pcf pfg pfl poi_r"\
   "and pow(10|2) pri(m(e([01]|countx?|n(ext|th(__1)?)|prev)|it"\
   "iveroot)|nt_(contfra(cd?)?|factor|jd264|num2char)) qmean1 r"\
   "ad2(d(eg|ms2?)|sec|tur) rand(_init|l) reci reimann round rt"\
   "o(125|[iz]) sba search_(check|search) sec(|2(d(eg|hms)|rad|"\
   "tur)|[dht]?(inv)?) shc_s(hft2steps|teps2shft(inv)?) sig(m(a"\
   "[01]|oid(inv)?)|n) sin(|__1|c(__1|c|inv)?|d(inv)?|h(inv)?|i"\
   "n(tegral|v)|t(inv)?) smooth sq sqinv sqrt(__[123]|x)? squar"\
   "e(root)? stair stts(_(delete|reset)|count|m(a(gic|x)|ean|in"\
   "|sq)|s(pread|tdev(mean)?|um)|var) supercatalan tan([dht]?(i"\
   "nv)?)? temp(c(2(k|tempf))?|f(2(k|tempc))?|r(2k)?) tgamma(__"\
   "([123]|stirling)|double|inv)? topyh1 totient tri(_rand|inv|"\
   "p)? trunc tur2(deg|rad|sec) uet2(j(2k|d)|kwt) uni_rand vers"\
   ...
   "q|ge|gt|le|lt|mult|multl|neq|practical_f|primitiveroot|sign"\
   "same|squarefree_f|ss) jacobiphi jewish_month(begin|length) "\
   "ks_(b_cdf|cdf|quantile) lbeta lca(ms?|ntim) lcm lcombx liga"\
   "mma(inv)? lmean logistic_(cdf|pdf|quantile) lpermx lqmean m"\
   "ax(2|abs2?|i) mb_(cdf|pdf) mean min(2|abs2?|i) mobius_f mod"\
   "(s|ulo) nbd_rand nc nd nevilletheta([cd]|[ns]) ns nthroot__"\
   "1 pareto_rand perm(utation|x)? poi_(cdf|pdf|quantile) poly("\
   "a_rand|cnum|num) powi? pri(me0_f|nt_base_outf?) qmean rand_"\
   "init_array ratio repetendlen rligamma rms round2 rtomsd rui"\
   "gamma sc sd shc_shftpoles2steps sigma(0_f|1_f)? sn sn__1 sp"\
   "read2stdev stdev2spread stirling[12] stts_(copy|downdate|ne"\
   "wx|s(cale|hift)|update2?) t_cdf(__1)? t_pdf t_quantile(__1)"\
   "? tetra(cnum|num) theta([12]|[34]) Tn topyh totient_f trap_"\
   "rand tri_(cdf|pdf|quantile) uigamma(__1|inv)? Un uni_(cdf|p"\
   "df|quantile) variance zeta_(cdf|pdf) zipf_rand znorder(__1)"\
   "?"\
   "\n"\
