...
' 2020-08-03 kdw  For Changelog, See File Kw.varylog
Option Explicit On
...
End Function
Public Shared Function elliptic1__2(ByVal mm As Double) As Double
   Dim term As Double
   Dim sum_curr As Double
   Dim sum_prev As Double
   Dim ii As Long
...
Public Shared Function nom(ByVal mm As Double) As Double
   nom _
         = ((Math.Exp(-Kw.PI * Kw.elliptic1(1.0 - mm) / _
               Kw.elliptic1(mm))))
End Function
...
Public Shared Function nevillethetac(ByVal mm As Double, _
   ...
   km = Kw.elliptic1(mm)
   qq = Math.Exp(-Kw.PI * Kw.elliptic1(1.0 - mm) / km)
   sum_curr = 0
   cos_inc = Kw.PI * zz / km
...
Public Function kw_description() As String
   ...
   desc_string = desc_string _
         & "1__2 elliptic2 elliptic2__1 ellipticE ellipticK er" _
         & "f erf__1 erf__2 erf__3 erf__4 erfc erfcinv erfcx e" _
         & "rfcxinv erfinv eta eulerphi eulerx exp exp__1 exp_" _
         & "_2 exp_rand expintegral1 expintegrali expm1 fac fa"
   desc_string = desc_string _
         & "cinv factorial factorialdouble factorialinv fibo f" _
         & "ibox fiboxinv flat2ecc floor frac fubini fusc g2je" _
         & "wish gamma_rand gau_cdf gau_pdf gau_quantile gd gd" _
         & "inv geo_rand ghmean1 ghmean1inv glmean1 gmean1 gqm"
   desc_string = desc_string _
         & "ean1 has_primitiveroot haversin heaviside heronian" _
         & "mean1 hlmean1 hmean1 hn hn__1 hour2day hqmean1 hyp" _
         & "ot1 hypot1inv id int_rand iround iseven isint isje" _
         & "wish8short isjewish9short isjewishleap isleapyear "
   desc_string = desc_string _
         & "ismagic isneg isnegint isodd ispos isposint isprac" _
         & "tical isprime issquarefree iszero j2k2dow j2k2jd j" _
         & "2k2kwt j2k2uet j2k2ymdhms jacobicir jd2dow jd2j2k " _
         & "jd2kwt jd2uet jd2ymdhms jewish2g jewish2jd jewish2"
   desc_string = desc_string _
         & "jdx jewish_months_in_year jewish_yearlength k2m k2" _
         & "tempc k2tempf k2tempr ks_a_cdf kwt2j2k kwt2jd kwt2" _
         & "uet l10 lg lgamma lgamma__1 lgamma__2 lgammainv li" _
         & "x lixinv lmean1 ln ln1p ln__1 ln__2 lns log log10 "
   desc_string = desc_string _
         & "log2 logintegral logistic_rand lqmean1 lucas lucas" _
         & "x m2b m2k mertens minkowski mobius neg nom normal_" _
         & "cdf normal_cdf__1 normal_pdf normal_quantile norma" _
         & "l_quantile__1 parabola_get parabola_roots partitio"
   desc_string = desc_string _
         & "n partitionq pcf pfg pfl poi_rand pow10 pow2 prime" _
         & "0 prime1 primecount primecountx primenext primenth" _
         & " primenth__1 primeprev primitiveroot print_contfra" _
         & " print_contfrac print_contfracd print_factor print"
   desc_string = desc_string _
         & "_jd264 print_num2char qmean1 rad2deg rad2dms rad2d" _
         & "ms2 rad2sec rad2tur rand_init randl reci reimann r" _
         & "ound rto125 rtoi rtoz sba search_check search_sear" _
         & "ch sec sec2deg sec2dhms sec2rad sec2tur secd secdi"
   desc_string = desc_string _
         & "nv sech sechinv secinv sect sectinv shc_shft2steps" _
         & " shc_steps2shft shc_steps2shftinv sigma0 sigma1 si" _
         & "gmoid sigmoidinv sign sin sin__1 sinc sinc__1 sinc" _
         & "c sincinv sind sindinv sinh sinhinv sinintegral si"
   desc_string = desc_string _
         & "ninv sint sintinv smooth sq sqinv sqrt sqrt__1 sqr" _
         & "t__2 sqrt__3 sqrtx square squareroot stair stts_de" _
         & "lete stts_reset sttscount sttsmagic sttsmax sttsme" _
         & "an sttsmin sttsmsq sttsspread sttsstdev sttsstdevm"
   desc_string = desc_string _
         & "ean sttssum sttsvar supercatalan tan tand tandinv " _
         & "tanh tanhinv taninv tant tantinv tempc tempc2k tem" _
         & "pc2tempf tempf tempf2k tempf2tempc tempr tempr2k t" _
         & "gamma tgamma__1 tgamma__2 tgamma__3 tgamma__stirli"
   desc_string = desc_string _
         & "ng tgammadouble tgammainv topyh1 totient tri tri_r" _
         & "and triinv trip trunc tur2deg tur2rad tur2sec uet2" _
         & "j2k uet2jd uet2kwt uni_rand versin wm wminv wp wpi" _
         & "nv ymdhms2j2k ymdhms2jd ymdhms_get zag zeta zig zi" _
         & "gzag" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "hbegin jewish_monthlength ks_b_cdf ks_cdf ks_quant"
   desc_string = desc_string _
         & "ile lbeta lcam lcams lcantim lcm lcombx ligamma li" _
         & "gammainv lmean logistic_cdf logistic_pdf logistic_" _
         & "quantile lpermx lqmean max2 maxabs maxabs2 maxi mb" _
         & "_cdf mb_pdf mean min2 minabs minabs2 mini mobius_f"
   desc_string = desc_string _
         & " modf mods modulo nbd_rand nc nd nevillethetac nev" _
         & "illethetad nevillethetan nevillethetas ns nthroot_" _
         & "_1 pareto_rand perm permutation permx poi_cdf poi_" _
         & "pdf poi_quantile polya_rand polycnum polynum pow p"
   desc_string = desc_string _
         & "owi prime0_f print_base_out print_base_outf qmean " _
         & "rand_init_array ratio repetendlen rligamma rms rou" _
         & "nd2 rtomsd ruigamma sc sd shc_shftpoles2steps sigm" _
         & "a sigma0_f sigma1_f sn sn__1 spread2stdev stdev2sp"
   desc_string = desc_string _
         & "read stirling1 stirling2 stts_copy stts_downdate s" _
         & "tts_newx stts_scale stts_shift stts_update stts_up" _
         & "date2 t_cdf t_cdf__1 t_pdf t_quantile t_quantile__" _
         & "1 tetracnum tetranum theta1 theta2 theta3 theta4 T"
   desc_string = desc_string _
         & "n topyh totient_f trap_rand tri_cdf tri_pdf tri_qu" _
         & "antile uigamma uigamma__1 uigammainv Un uni_cdf un" _
         & "i_pdf uni_quantile variance zeta_cdf zeta_pdf zipf" _
         & "_rand znorder znorder__1" _
         & vbNewLine
   desc_string = desc_string _
...
Public Function kw_desc() As String
   ...
   desc_string = desc_string _
         ...
         & "lliptic(1(__[12])?|2(__1)?|[EK]) erf(|__([12]|[34]" _
         & ")|c(inv|x(inv)?)?|inv) eta euler(phi|x) exp(_(_[12"
   desc_string = desc_string _
         & "]|rand)|integral[1i]|m1)? fac(inv|torial(double|in" _
         & "v)?)? fibo(x(inv)?)? flat2ecc floor frac fubini fu" _
         & "sc g2jewish gamma_rand gau_(cdf|pdf|quantile) gd g" _
         & "dinv geo_rand ghmean1(inv)? glmean1 gmean1 gqmean1"
   desc_string = desc_string _
         & " has_primitiveroot haversin heaviside heronianmean" _
         & "1 hlmean1 hmean1 hn hn__1 hour2day hqmean1 hypot1(" _
         & "inv)? id int_rand iround is(even|int|jewish8short|" _
         & "jewish9short|jewishleap|leapyear|magic|neg|negint|"
   desc_string = desc_string _
         & "odd|pos|posint|practical|prime|squarefree|zero) j2" _
         & "k2(dow|jd|kwt|uet|ymdhms) jacobicir jd2(dow|j2k|kw" _
         & "t|uet|ymdhms) jewish(2(g|jdx?)|_(months_in_year|ye" _
         & "arlength)) k2m k2temp[cfr] ks_a_cdf kwt2(j(2k|d)|u"
   desc_string = desc_string _
         & "et) l10 lg lgamma(__[12]|inv)? lix(inv)? lmean1 ln" _
         & " ln1p ln__[12] lns log(10|2|i(ntegral|stic_rand))?" _
         & " lqmean1 lucasx? m2b m2k mertens minkowski mobius " _
         & "neg nom normal_(cdf(__1)?|pdf|quantile(__1)?) par("
   desc_string = desc_string _
         & "abola_(get|roots)|titionq?) pcf pfg pfl poi_rand p" _
         & "ow(10|2) pri(m(e([01]|countx?|n(ext|th(__1)?)|prev" _
         & ")|itiveroot)|nt_(contfra(cd?)?|factor|jd264|num2ch" _
         & "ar)) qmean1 rad2(d(eg|ms2?)|sec|tur) rand(_init|l)"
   desc_string = desc_string _
         & " reci reimann round rto(125|[iz]) sba search_(chec" _
         & "k|search) sec(|2(d(eg|hms)|rad|tur)|[dht]?(inv)?) " _
         & "shc_s(hft2steps|teps2shft(inv)?) sig(m(a[01]|oid(i" _
         & "nv)?)|n) sin(|__1|c(__1|c|inv)?|d(inv)?|h(inv)?|in"
   desc_string = desc_string _
         & "(tegral|v)|t(inv)?) smooth sq sqinv sqrt(__[123]|x" _
         & ")? square(root)? stair stts(_(delete|reset)|count|" _
         & "m(a(gic|x)|ean|in|sq)|s(pread|tdev(mean)?|um)|var)" _
         & " supercatalan tan([dht]?(inv)?)? temp(c(2(k|tempf)"
   desc_string = desc_string _
         & ")?|f(2(k|tempc))?|r(2k)?) tgamma(__([123]|stirling" _
         & ")|double|inv)? topyh1 totient tri(_rand|inv|p)? tr" _
         & "unc tur2(deg|rad|sec) uet2(j(2k|d)|kwt) uni_rand v" _
         & "ersin wm(inv)? wp(inv)? ymdhms(2j(2k|d)|_get) zag " _
         & "zeta zig(zag)?" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "t|mult|multl|neq|practical_f|primitiveroot|signsam"
   desc_string = desc_string _
         & "e|squarefree_f|ss) jacobiphi jewish_month(begin|le" _
         & "ngth) ks_(b_cdf|cdf|quantile) lbeta lca(ms?|ntim) " _
         & "lcm lcombx ligamma(inv)? lmean logistic_(cdf|pdf|q" _
         & "uantile) lpermx lqmean max(2|abs2?|i) mb_(cdf|pdf)"
   desc_string = desc_string _
         & " mean min(2|abs2?|i) mobius_f mod([fs]|ulo) nbd_ra" _
         & "nd nc nd nevilletheta([cd]|[ns]) ns nthroot__1 par" _
         & "eto_rand perm(utation|x)? poi_(cdf|pdf|quantile) p" _
         & "oly(a_rand|cnum|num) powi? pri(me0_f|nt_base_outf?"
   desc_string = desc_string _
         & ") qmean rand_init_array ratio repetendlen rligamma" _
         & " rms round2 rtomsd ruigamma sc sd shc_shftpoles2st" _
         & "eps sigma(0_f|1_f)? sn sn__1 spread2stdev stdev2sp" _
         & "read stirling[12] stts_(copy|downdate|newx|s(cale|"
   desc_string = desc_string _
         & "hift)|update2?) t_cdf(__1)? t_pdf t_quantile(__1)?" _
         & " tetra(cnum|num) theta([12]|[34]) Tn topyh totient" _
         & "_f trap_rand tri_(cdf|pdf|quantile) uigamma(__1|in" _
         & "v)? Un uni_(cdf|pdf|quantile) variance zeta_(cdf|p" _
         & "df) zipf_rand znorder(__1)?" _
         & vbNewLine
   desc_string = desc_string _
