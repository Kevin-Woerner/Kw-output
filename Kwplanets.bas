...
Public Const SUNRADIUS As Double = (SUN_A + SUN_B + SUN_C) / 3.0
...
Public Const SUNSURFACE As Double = 2.0 * Kw.TAU * SUNRADIUS _
      * SUNRADIUS
...
Public Const MERCURY_B As Double = MERCURY_A * (1.0 - _
      MERCURYFLATNESS)
Public Const MERCURYRADIUS As Double = (MERCURY_A + MERCURY_B _
      + MERCURY_C) / 3.0
...
Public Const MERCURYSURFACE As Double = 2.0 * Kw.TAU * _
      MERCURYRADIUS * MERCURYRADIUS
Public Const MERCURYDENSITY As Double = MERCURYMASS / _
      MERCURYVOLUME
Public Const MERCURYSYNODIC As Double = MERCURYYEAR * _
      EARTHSIDEREALYEAR / (MERCURYYEAR - EARTHSIDEREALYEAR)
...
Public Const VENUSRADIUS As Double = (VENUS_A + VENUS_B + _
      VENUS_C) / 3.0
...
Public Const VENUSSURFACE As Double = 2.0 * Kw.TAU * _
      VENUSRADIUS * VENUSRADIUS
Public Const VENUSDENSITY As Double = VENUSMASS / VENUSVOLUME
Public Const VENUSSYNODIC As Double = VENUSYEAR * _
      EARTHSIDEREALYEAR / (VENUSYEAR - EARTHSIDEREALYEAR)
...
Public Const EARTHRADIUS As Double = (EARTH_A + EARTH_B + _
      EARTH_C) / 3.0
...
Public Const EARTHSURFACE As Double = 2.0 * Kw.TAU * _
...
Public Const MARSRADIUS As Double = (MARS_A + MARS_B + _
      MARS_C) / 3.0
...
Public Const MARSSURFACE As Double = 2.0 * Kw.TAU * _
      MARSRADIUS * MARSRADIUS
Public Const MARSDENSITY As Double = MARSMASS / MARSVOLUME
Public Const MARSSYNODIC As Double = MARSYEAR * _
      EARTHSIDEREALYEAR / (MARSYEAR - EARTHSIDEREALYEAR)
...
Public Const JUPITER_B As Double = JUPITER_A * (1.0 - _
      JUPITERFLATNESS)
Public Const JUPITERRADIUS As Double = (JUPITER_A + JUPITER_B _
      + JUPITER_C) / 3.0
...
Public Const JUPITERSURFACE As Double = 2.0 * Kw.TAU * _
      JUPITERRADIUS * JUPITERRADIUS
Public Const JUPITERDENSITY As Double = JUPITERMASS / _
      JUPITERVOLUME
Public Const JUPITERSYNODIC As Double = JUPITERYEAR * _
      EARTHSIDEREALYEAR / (JUPITERYEAR - EARTHSIDEREALYEAR)
...
Public Const SATURN_B As Double = SATURN_A * (1.0 - _
      SATURNFLATNESS)
Public Const SATURNRADIUS As Double = (SATURN_A + SATURN_B + _
      SATURN_C) / 3.0
...
Public Const SATURNSURFACE As Double = 2.0 * Kw.TAU * _
      SATURNRADIUS * SATURNRADIUS
Public Const SATURNDENSITY As Double = SATURNMASS / SATURNVOLUME
Public Const SATURNSYNODIC As Double = SATURNYEAR * _
      EARTHSIDEREALYEAR / (SATURNYEAR - EARTHSIDEREALYEAR)
...
Public Const URANUS_B As Double = URANUS_A * (1.0 - _
      URANUSFLATNESS)
Public Const URANUSRADIUS As Double = (URANUS_A + URANUS_B + _
      URANUS_C) / 3.0
...
Public Const URANUSSURFACE As Double = 2.0 * Kw.TAU * _
      URANUSRADIUS * URANUSRADIUS
Public Const URANUSDENSITY As Double = URANUSMASS / URANUSVOLUME
Public Const URANUSSYNODIC As Double = URANUSYEAR * _
      EARTHSIDEREALYEAR / (URANUSYEAR - EARTHSIDEREALYEAR)
...
Public Const NEPTUNE_B As Double = NEPTUNE_A * (1.0 - _
      NEPTUNEFLATNESS)
Public Const NEPTUNERADIUS As Double = (NEPTUNE_A + NEPTUNE_B _
      + NEPTUNE_C) / 3.0
...
Public Const NEPTUNESURFACE As Double = 2.0 * Kw.TAU * _
      NEPTUNERADIUS * NEPTUNERADIUS
Public Const NEPTUNEDENSITY As Double = NEPTUNEMASS / _
      NEPTUNEVOLUME
Public Const NEPTUNESYNODIC As Double = NEPTUNEYEAR * _
      EARTHSIDEREALYEAR / (NEPTUNEYEAR - EARTHSIDEREALYEAR)
...
Public Const PLUTORADIUS As Double = (PLUTO_A + PLUTO_B + _
      PLUTO_C) / 3.0
...
Public Const PLUTOSURFACE As Double = 2.0 * Kw.TAU * _
      PLUTORADIUS * PLUTORADIUS
Public Const PLUTODENSITY As Double = PLUTOMASS / PLUTOVOLUME
Public Const PLUTOSYNODIC As Double = PLUTOYEAR * _
      EARTHSIDEREALYEAR / (PLUTOYEAR - EARTHSIDEREALYEAR)
...
Public Const MOONRADIUS As Double = (MOON_A + MOON_B + _
      MOON_C) / 3.0
...
Public Const MOONSURFACE As Double = 2.0 * Kw.TAU * _
      MOONRADIUS * MOONRADIUS
Public Const MOONDENSITY As Double = MOONMASS / MOONVOLUME
Public Const MOONSYNODIC As Double = MOONYEAR * _
      EARTHSIDEREALYEAR / (MOONYEAR - EARTHSIDEREALYEAR)
...
Public Const CERESRADIUS As Double = (CERES_A + CERES_B + _
      CERES_C) / 3.0
...
Public Const CERESSURFACE As Double = 2.0 * Kw.TAU * _
      CERESRADIUS * CERESRADIUS
Public Const CERESDENSITY As Double = CERESMASS / CERESVOLUME
Public Const CERESSYNODIC As Double = CERESYEAR * _
      EARTHSIDEREALYEAR / (CERESYEAR - EARTHSIDEREALYEAR)
...
Public Const ERISRADIUS As Double = (ERIS_A + ERIS_B + _
      ERIS_C) / 3.0
...
Public Const ERISSURFACE As Double = 2.0 * Kw.TAU * _
      ERISRADIUS * ERISRADIUS
Public Const ERISDENSITY As Double = ERISMASS / ERISVOLUME
Public Const ERISSYNODIC As Double = ERISYEAR * _
      EARTHSIDEREALYEAR / (ERISYEAR - EARTHSIDEREALYEAR)
...
Public Const MAKEMAKE_B As Double = MAKEMAKE_A * (1.0 - _
      MAKEMAKEFLATNESS)
Public Const MAKEMAKERADIUS As Double = (MAKEMAKE_A + _
      MAKEMAKE_B + MAKEMAKE_C) / 3.0
...
Public Const MAKEMAKESURFACE As Double = 2.0 * Kw.TAU * _
      MAKEMAKERADIUS * MAKEMAKERADIUS
Public Const MAKEMAKEDENSITY As Double = MAKEMAKEMASS / _
      MAKEMAKEVOLUME
Public Const MAKEMAKESYNODIC As Double = MAKEMAKEYEAR * _
      EARTHSIDEREALYEAR / (MAKEMAKEYEAR - EARTHSIDEREALYEAR)
...
Public Const HAUMEA_B As Double = HAUMEA_A * (1.0 - _
      HAUMEAFLATNESS)
Public Const HAUMEARADIUS As Double = (HAUMEA_A + HAUMEA_B + _
      HAUMEA_C) / 3.0
...
Public Const HAUMEASURFACE As Double = 2.0 * Kw.TAU * _
      HAUMEARADIUS * HAUMEARADIUS
Public Const HAUMEADENSITY As Double = HAUMEAMASS / HAUMEAVOLUME
Public Const HAUMEASYNODIC As Double = HAUMEAYEAR * _
      EARTHSIDEREALYEAR / (HAUMEAYEAR - EARTHSIDEREALYEAR)
...
Private Const LC_PREM_DENSITY As Double = 0
...
End Function
Public Function cxsunsurface() As Double
...
End Function
Public Function cxearthsurface() As Double
...
Public Function pl_surface(ByVal plnt As Long) As Double
   ...
   If (plnt < LC_P1_PREV_PL Or LC_P1_COUNT <= plnt) Then
      retvalu = LC_P1_ERR
   Else
      retvalu = Kw.ellipsoid_surface(pl_a(plnt), pl_b(plnt), _
            pl_c(plnt))
   End If
   pl_surface = (retvalu)
End Function
...
Private Function lf_isa_calc(ByVal height_m As Double, ByVal _
   ...
   Else
      ...
      If (Kw.iszero(lv_isa_mtrx(h_ind, LC_ISA_XLAPSERATE))) Then
         retvalu = Math.Exp(-air_k_per_m * delta_h / _
      ...
      Else
         ...
         If (state_x = LC_ISA_DENSITY) Then
            expon = expon + 1
         End If
         retvalu = Pow(lv_isa_mtrx(h_ind, _
               LC_ISA_XTEMPERATURE) / this_temp, expon)
      End If
      If (state_x = LC_ISA_PRESSURE) Then
         retvalu = retvalu * lv_isa_mtrx(h_ind, LC_ISA_XPRESSURE)
      Else
         retvalu = retvalu * lv_isa_mtrx(h_ind, LC_ISA_XDENSITY)
      End If
   End If
   lf_isa_calc = (retvalu)
End Function
...
End Function
Private Function lf_prem(ByVal height_m As Double, ByVal _
      state_x As Long) As Double
   Dim coec As Double
   Dim coeb As Double
   Dim coea As Double
   ...
   If (Not Kw.ismagic(lv_prem_magic)) Then
      ...
      For ii = 1 To 11
         ...
         mass_b = 2.0 * Kw.TAU * hght_i * hght_i * hght_i * _
               ((lv_prem_mtrx(ii, LC_PREM_XCOEC) / 5.0 * _
               hght_i + lv_prem_mtrx(ii, LC_PREM_XCOEB) / _
               4.0) * hght_i + lv_prem_mtrx(ii, _
               LC_PREM_XCOEA) / 3.0)
         hght_j = lv_prem_mtrx(ii, LC_PREM_XHEIGHT)
         mass_a = 2.0 * Kw.TAU * hght_j * hght_j * hght_j * _
               ((lv_prem_mtrx(ii, LC_PREM_XCOEC) / 5.0 * _
               hght_j + lv_prem_mtrx(ii, LC_PREM_XCOEB) / _
               4.0) * hght_j + lv_prem_mtrx(ii, _
               LC_PREM_XCOEA) / 3.0)
         mass_ip = mass_i
   ...
   If (height_m <= 0.0) Then
      retvalu = 0.0
   ElseIf (EARTHRADIUS < height_m) Then
      If (state_x = 0) Then
         retvalu = 0.0
      Else
         retvalu = EARTHMASS
   ...
   Else
      ...
      ElseIf (state_x = 1) Then
         ...
         mass_a = 2.0 * Kw.TAU * height_m * height_m * _
               height_m * ((coec / 5.0 * height_m + coeb / _
...
Public Function prem_density(ByVal height_m As Double) As Double
   prem_density = ((lf_prem(height_m, LC_PREM_DENSITY)))
End Function
...
Public Function prem_g(ByVal height_m As Double) As Double
   ...
   If (height_m <= 0.0) Then
      retvalu = 0.0
   Else
      retvalu = Kw.GRAVITATION * prem_mass(height_m) / _
            (height_m * height_m)
   End If
   prem_g = (retvalu)
End Function
...
Public Function kwplanets_description() As String
   ...
   desc_string = desc_string _
         ...
         & "RESPERIAPSIS CERESPERIAPSIS_DT CERESRADIUS CERESSU"
   desc_string = desc_string _
         & "RFACE CERESSYNODIC CERESVOLUME CERESYEAR EARTH_A E" _
         & "ARTH_B EARTH_C EARTHANOMALISTICYEAR EARTHASCENDING" _
         & " EARTHASCENDING_DT EARTHDAY EARTHDENSITY EARTHECCE" _
         & "NTRICITY EARTHECCENTRICITY_DT EARTHFLATNESS EARTHG"
   desc_string = desc_string _
         & "M EARTHGRAVITY EARTHINCLINATION EARTHINCLINATION_D" _
         & "T EARTHLONGITUDE EARTHLONGITUDE_DT EARTHMASS EARTH" _
         & "ORBIT EARTHORBIT_DT EARTHPERIAPSIS EARTHPERIAPSIS_" _
         & "DT EARTHPRECESSION EARTHRADIUS EARTHSIDEREALDAY EA"
   desc_string = desc_string _
         & "RTHSIDEREALYEAR EARTHSTELLARDAY EARTHSURFACE EARTH" _
         & "THETA_0 EARTHTROPICALYEAR EARTHVOLUME EARTHYEAR ER" _
         & "IS_A ERIS_B ERIS_C ERISASCENDING ERISASCENDING_DT " _
         & "ERISDAY ERISDENSITY ERISECCENTRICITY ERISECCENTRIC"
   desc_string = desc_string _
         & "ITY_DT ERISFLATNESS ERISGM ERISGRAVITY ERISINCLINA" _
         & "TION ERISINCLINATION_DT ERISLONGITUDE ERISLONGITUD" _
         & "E_DT ERISMASS ERISORBIT ERISORBIT_DT ERISPERIAPSIS" _
         & " ERISPERIAPSIS_DT ERISRADIUS ERISSURFACE ERISSYNOD"
   desc_string = desc_string _
         & "IC ERISVOLUME ERISYEAR HAUMEA_A HAUMEA_B HAUMEA_C " _
         & "HAUMEAASCENDING HAUMEAASCENDING_DT HAUMEADAY HAUME" _
         & "ADENSITY HAUMEAECCENTRICITY HAUMEAECCENTRICITY_DT " _
         & "HAUMEAFLATNESS HAUMEAGM HAUMEAGRAVITY HAUMEAINCLIN"
   desc_string = desc_string _
         & "ATION HAUMEAINCLINATION_DT HAUMEALONGITUDE HAUMEAL" _
         & "ONGITUDE_DT HAUMEAMASS HAUMEAORBIT HAUMEAORBIT_DT " _
         & "HAUMEAPERIAPSIS HAUMEAPERIAPSIS_DT HAUMEARADIUS HA" _
         & "UMEASURFACE HAUMEASYNODIC HAUMEAVOLUME HAUMEAYEAR "
   desc_string = desc_string _
         & "JUPITER_A JUPITER_B JUPITER_C JUPITERASCENDING JUP" _
         & "ITERASCENDING_DT JUPITERDAY JUPITERDENSITY JUPITER" _
         & "ECCENTRICITY JUPITERECCENTRICITY_DT JUPITERFLATNES" _
         & "S JUPITERGM JUPITERGRAVITY JUPITERINCLINATION JUPI"
   desc_string = desc_string _
         & "TERINCLINATION_DT JUPITERLONGITUDE JUPITERLONGITUD" _
         & "E_DT JUPITERMASS JUPITERORBIT JUPITERORBIT_DT JUPI" _
         & "TERPERIAPSIS JUPITERPERIAPSIS_DT JUPITERRADIUS JUP" _
         & "ITERSURFACE JUPITERSYNODIC JUPITERVOLUME JUPITERYE"
   desc_string = desc_string _
         & "AR LC_ISA_DENSITY LC_ISA_PRESSURE LC_ISA_TEMPERATU" _
         & "RE LC_ISA_XDENSITY LC_ISA_XHEIGHT LC_ISA_XLAPSERAT" _
         & "E LC_ISA_XPRESSURE LC_ISA_XTEMPERATURE LC_P1_CERES" _
         & " LC_P1_COUNT LC_P1_EARTH LC_P1_ERIS LC_P1_ERR LC_P"
   desc_string = desc_string _
         & "1_HAUMEA LC_P1_JUPITER LC_P1_MAKEMAKE LC_P1_MARS L" _
         & "C_P1_MERCURY LC_P1_NEPTUNE LC_P1_PLUTO LC_P1_PREV_" _
         & "PL LC_P1_SATURN LC_P1_SUN LC_P1_URANUS LC_P1_VENUS" _
         & " LC_P2_A LC_P2_ASC LC_P2_ASC_DT LC_P2_C LC_P2_COUN"
   desc_string = desc_string _
         & "T LC_P2_DAY LC_P2_ECC LC_P2_ECC_DT LC_P2_FLATNESS " _
         & "LC_P2_GM LC_P2_INC LC_P2_INC_DT LC_P2_LON LC_P2_LO" _
         & "N_DT LC_P2_ORB LC_P2_ORB_DT LC_P2_PER LC_P2_PER_DT" _
         & " LC_P2_YEAR LC_PREM_DENSITY LC_PREM_MASS LC_PREM_X"
   desc_string = desc_string _
         & "COEA LC_PREM_XCOEB LC_PREM_XCOEC LC_PREM_XHEIGHT L" _
         & "C_PREM_XMASSD MAKEMAKE_A MAKEMAKE_B MAKEMAKE_C MAK" _
         & "EMAKEASCENDING MAKEMAKEASCENDING_DT MAKEMAKEDAY MA" _
         & "KEMAKEDENSITY MAKEMAKEECCENTRICITY MAKEMAKEECCENTR"
   desc_string = desc_string _
         & "ICITY_DT MAKEMAKEFLATNESS MAKEMAKEGM MAKEMAKEGRAVI" _
         & "TY MAKEMAKEINCLINATION MAKEMAKEINCLINATION_DT MAKE" _
         & "MAKELONGITUDE MAKEMAKELONGITUDE_DT MAKEMAKEMASS MA" _
         & "KEMAKEORBIT MAKEMAKEORBIT_DT MAKEMAKEPERIAPSIS MAK"
   desc_string = desc_string _
         & "EMAKEPERIAPSIS_DT MAKEMAKERADIUS MAKEMAKESURFACE M" _
         & "AKEMAKESYNODIC MAKEMAKEVOLUME MAKEMAKEYEAR MARS_A " _
         & "MARS_B MARS_C MARSASCENDING MARSASCENDING_DT MARSD" _
         & "AY MARSDENSITY MARSECCENTRICITY MARSECCENTRICITY_D"
   desc_string = desc_string _
         & "T MARSFLATNESS MARSGM MARSGRAVITY MARSINCLINATION " _
         & "MARSINCLINATION_DT MARSLONGITUDE MARSLONGITUDE_DT " _
         & "MARSMASS MARSORBIT MARSORBIT_DT MARSPERIAPSIS MARS" _
         & "PERIAPSIS_DT MARSRADIUS MARSSURFACE MARSSYNODIC MA"
   desc_string = desc_string _
         & "RSVOLUME MARSYEAR MERCURY_A MERCURY_B MERCURY_C ME" _
         & "RCURYASCENDING MERCURYASCENDING_DT MERCURYDAY MERC" _
         & "URYDENSITY MERCURYECCENTRICITY MERCURYECCENTRICITY" _
         & "_DT MERCURYFLATNESS MERCURYGM MERCURYGRAVITY MERCU"
   desc_string = desc_string _
         & "RYINCLINATION MERCURYINCLINATION_DT MERCURYLONGITU" _
         & "DE MERCURYLONGITUDE_DT MERCURYMASS MERCURYORBIT ME" _
         & "RCURYORBIT_DT MERCURYPERIAPSIS MERCURYPERIAPSIS_DT" _
         & " MERCURYRADIUS MERCURYSURFACE MERCURYSYNODIC MERCU"
   desc_string = desc_string _
         & "RYVOLUME MERCURYYEAR MOON_A MOON_B MOON_C MOONDAY " _
         & "MOONDENSITY MOONECCENTRICITY MOONFLATNESS MOONGM M" _
         & "OONGRAVITY MOONINCLINATION MOONMASS MOONORBIT MOON" _
         & "RADIUS MOONSURFACE MOONSYNODIC MOONVOLUME MOONYEAR"
   desc_string = desc_string _
         & " NEPTUNE_A NEPTUNE_B NEPTUNE_C NEPTUNEASCENDING NE" _
         & "PTUNEASCENDING_DT NEPTUNEDAY NEPTUNEDENSITY NEPTUN" _
         & "EECCENTRICITY NEPTUNEECCENTRICITY_DT NEPTUNEFLATNE" _
         & "SS NEPTUNEGM NEPTUNEGRAVITY NEPTUNEINCLINATION NEP"
   desc_string = desc_string _
         & "TUNEINCLINATION_DT NEPTUNELONGITUDE NEPTUNELONGITU" _
         & "DE_DT NEPTUNEMASS NEPTUNEORBIT NEPTUNEORBIT_DT NEP" _
         & "TUNEPERIAPSIS NEPTUNEPERIAPSIS_DT NEPTUNERADIUS NE" _
         & "PTUNESURFACE NEPTUNESYNODIC NEPTUNEVOLUME NEPTUNEY"
   desc_string = desc_string _
         & "EAR PLUTO_A PLUTO_B PLUTO_C PLUTOASCENDING PLUTOAS" _
         & "CENDING_DT PLUTODAY PLUTODENSITY PLUTOECCENTRICITY" _
         & " PLUTOECCENTRICITY_DT PLUTOFLATNESS PLUTOGM PLUTOG" _
         & "RAVITY PLUTOINCLINATION PLUTOINCLINATION_DT PLUTOL"
   desc_string = desc_string _
         & "ONGITUDE PLUTOLONGITUDE_DT PLUTOMASS PLUTOORBIT PL" _
         & "UTOORBIT_DT PLUTOPERIAPSIS PLUTOPERIAPSIS_DT PLUTO" _
         & "RADIUS PLUTOSURFACE PLUTOSYNODIC PLUTOVOLUME PLUTO" _
         & "YEAR SATURN_A SATURN_B SATURN_C SATURNASCENDING SA"
   desc_string = desc_string _
         & "TURNASCENDING_DT SATURNDAY SATURNDENSITY SATURNECC" _
         & "ENTRICITY SATURNECCENTRICITY_DT SATURNFLATNESS SAT" _
         & "URNGM SATURNGRAVITY SATURNINCLINATION SATURNINCLIN" _
         & "ATION_DT SATURNLONGITUDE SATURNLONGITUDE_DT SATURN"
   desc_string = desc_string _
         & "MASS SATURNORBIT SATURNORBIT_DT SATURNPERIAPSIS SA" _
         & "TURNPERIAPSIS_DT SATURNRADIUS SATURNSURFACE SATURN" _
         & "SYNODIC SATURNVOLUME SATURNYEAR SIDEREALDAY SIDERE" _
         & "ALYEAR STELLARDAY SUN_A SUN_B SUN_C SUNASCENDING S"
   desc_string = desc_string _
         & "UNASCENDING_DT SUNDAY SUNDENSITY SUNECCENTRICITY S" _
         & "UNECCENTRICITY_DT SUNFLATNESS SUNGM SUNGMIERS SUNG" _
         & "MNASA SUNGRAVITY SUNINCLINATION SUNINCLINATION_DT " _
         & "SUNLONGITUDE SUNLONGITUDE_DT SUNMASS SUNORBIT SUNO"
   desc_string = desc_string _
         & "RBIT_DT SUNPERIAPSIS SUNPERIAPSIS_DT SUNRADIUS SUN" _
         & "SURFACE SUNVOLUME SUNYEAR TROPICALYEAR URANUS_A UR" _
         & "ANUS_B URANUS_C URANUSASCENDING URANUSASCENDING_DT" _
         & " URANUSDAY URANUSDENSITY URANUSECCENTRICITY URANUS"
   desc_string = desc_string _
         & "ECCENTRICITY_DT URANUSFLATNESS URANUSGM URANUSGRAV" _
         & "ITY URANUSINCLINATION URANUSINCLINATION_DT URANUSL" _
         & "ONGITUDE URANUSLONGITUDE_DT URANUSMASS URANUSORBIT" _
         & " URANUSORBIT_DT URANUSPERIAPSIS URANUSPERIAPSIS_DT"
   desc_string = desc_string _
         & " URANUSRADIUS URANUSSURFACE URANUSSYNODIC URANUSVO" _
         & "LUME URANUSYEAR VENUS_A VENUS_B VENUS_C VENUSASCEN" _
         & "DING VENUSASCENDING_DT VENUSDAY VENUSDENSITY VENUS" _
         & "ECCENTRICITY VENUSECCENTRICITY_DT VENUSFLATNESS VE"
   desc_string = desc_string _
         & "NUSGM VENUSGRAVITY VENUSINCLINATION VENUSINCLINATI" _
         & "ON_DT VENUSLONGITUDE VENUSLONGITUDE_DT VENUSMASS V" _
         & "ENUSORBIT VENUSORBIT_DT VENUSPERIAPSIS VENUSPERIAP" _
         & "SIS_DT VENUSRADIUS VENUSSURFACE VENUSSYNODIC VENUS" _
         & "VOLUME VENUSYEAR" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "helion plradius plsternlevison plsurface plsynodic" _
         & " plvolume plyear" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "rgot pl_mass pl_meananomaly pl_name pl_orbit pl_pe"
   desc_string = desc_string _
         & "riapsis pl_perihelion pl_radius pl_sternlevison pl" _
         & "_surface pl_synodic pl_volume pl_year plaradius pl" _
         & "geo plhelio plsettime prem_density prem_g prem_mas" _
         & "s" _
         & vbNewLine
   desc_string = desc_string _
...
Public Function kwplanets_desc() As String
   ...
   desc_string = desc_string _
         & "_DT)?|MASS|ORBIT(_DT)?|PERIAPSIS(_DT)?|RADIUS|S(UR" _
         & "FACE|YNODIC)|VOLUME|YEAR) EARTH(_[ABC]|A(NOMALISTI" _
         & "CYEAR|SCENDING(_DT)?)|D(AY|ENSITY)|ECCENTRICITY(_D" _
         & "T)?|FLATNESS|G(M|RAVITY)|INCLINATION(_DT)?|LONGITU"
   desc_string = desc_string _
         & "DE(_DT)?|MASS|ORBIT(_DT)?|P(ERIAPSIS(_DT)?|RECESSI" _
         & "ON)|RADIUS|S(IDEREAL(DAY|YEAR)|TELLARDAY|URFACE)|T" _
         & "(HETA_0|ROPICALYEAR)|VOLUME|YEAR) ERIS(_[ABC]|ASCE" _
         & "NDING(_DT)?|D(AY|ENSITY)|ECCENTRICITY(_DT)?|FLATNE"
   desc_string = desc_string _
         & "SS|G(M|RAVITY)|INCLINATION(_DT)?|LONGITUDE(_DT)?|M" _
         & "ASS|ORBIT(_DT)?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|YN" _
         & "ODIC)|VOLUME|YEAR) HAUMEA(_[ABC]|ASCENDING(_DT)?|D" _
         & "(AY|ENSITY)|ECCENTRICITY(_DT)?|FLATNESS|G(M|RAVITY"
   desc_string = desc_string _
         & ")|INCLINATION(_DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_DT" _
         & ")?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|YNODIC)|VOLUME|" _
         & "YEAR) JUPITER(_[ABC]|ASCENDING(_DT)?|D(AY|ENSITY)|" _
         & "ECCENTRICITY(_DT)?|FLATNESS|G(M|RAVITY)|INCLINATIO"
   desc_string = desc_string _
         & "N(_DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_DT)?|PERIAPSIS" _
         & "(_DT)?|RADIUS|S(URFACE|YNODIC)|VOLUME|YEAR) LC_(IS" _
         & "A_(DENSITY|PRESSURE|TEMPERATURE|X(DENSITY|HEIGHT|L" _
         & "APSERATE|PRESSURE|TEMPERATURE))|P(1_(C(ERES|OUNT)|"
   desc_string = desc_string _
         & "E(ARTH|R(IS|R))|HAUMEA|JUPITER|M(A(KEMAKE|RS)|ERCU" _
         & "RY)|NEPTUNE|P(LUTO|REV_PL)|S(ATURN|UN)|URANUS|VENU" _
         & "S)|2_(A(SC(_DT)?)?|C(OUNT)?|DAY|ECC(_DT)?|FLATNESS" _
         & "|GM|INC(_DT)?|LON(_DT)?|ORB(_DT)?|PER(_DT)?|YEAR)|"
   desc_string = desc_string _
         & "REM_(DENSITY|MASS|X(COE[ABC]|HEIGHT|MASSD)))) MAKE" _
         & "MAKE(_[ABC]|ASCENDING(_DT)?|D(AY|ENSITY)|ECCENTRIC" _
         & "ITY(_DT)?|FLATNESS|G(M|RAVITY)|INCLINATION(_DT)?|L" _
         & "ONGITUDE(_DT)?|MASS|ORBIT(_DT)?|PERIAPSIS(_DT)?|RA"
   desc_string = desc_string _
         & "DIUS|S(URFACE|YNODIC)|VOLUME|YEAR) MARS(_[ABC]|ASC" _
         & "ENDING(_DT)?|D(AY|ENSITY)|ECCENTRICITY(_DT)?|FLATN" _
         & "ESS|G(M|RAVITY)|INCLINATION(_DT)?|LONGITUDE(_DT)?|" _
         & "MASS|ORBIT(_DT)?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|Y"
   desc_string = desc_string _
         & "NODIC)|VOLUME|YEAR) MERCURY(_[ABC]|ASCENDING(_DT)?" _
         & "|D(AY|ENSITY)|ECCENTRICITY(_DT)?|FLATNESS|G(M|RAVI" _
         & "TY)|INCLINATION(_DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_" _
         & "DT)?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|YNODIC)|VOLUM"
   desc_string = desc_string _
         & "E|YEAR) MOON(_[ABC]|D(AY|ENSITY)|ECCENTRICITY|FLAT" _
         & "NESS|G(M|RAVITY)|INCLINATION|MASS|ORBIT|RADIUS|S(U" _
         & "RFACE|YNODIC)|VOLUME|YEAR) NEPTUNE(_[ABC]|ASCENDIN" _
         & "G(_DT)?|D(AY|ENSITY)|ECCENTRICITY(_DT)?|FLATNESS|G"
   desc_string = desc_string _
         & "(M|RAVITY)|INCLINATION(_DT)?|LONGITUDE(_DT)?|MASS|" _
         & "ORBIT(_DT)?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|YNODIC" _
         & ")|VOLUME|YEAR) PLUTO(_[ABC]|ASCENDING(_DT)?|D(AY|E" _
         & "NSITY)|ECCENTRICITY(_DT)?|FLATNESS|G(M|RAVITY)|INC"
   desc_string = desc_string _
         & "LINATION(_DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_DT)?|PE" _
         & "RIAPSIS(_DT)?|RADIUS|S(URFACE|YNODIC)|VOLUME|YEAR)" _
         & " SATURN(_[ABC]|ASCENDING(_DT)?|D(AY|ENSITY)|ECCENT" _
         & "RICITY(_DT)?|FLATNESS|G(M|RAVITY)|INCLINATION(_DT)"
   desc_string = desc_string _
         & "?|LONGITUDE(_DT)?|MASS|ORBIT(_DT)?|PERIAPSIS(_DT)?" _
         & "|RADIUS|S(URFACE|YNODIC)|VOLUME|YEAR) SIDEREAL(DAY" _
         & "|YEAR) STELLARDAY SUN(_[ABC]|ASCENDING(_DT)?|D(AY|" _
         & "ENSITY)|ECCENTRICITY(_DT)?|FLATNESS|G(M(IERS|NASA)"
   desc_string = desc_string _
         & "?|RAVITY)|INCLINATION(_DT)?|LONGITUDE(_DT)?|MASS|O" _
         & "RBIT(_DT)?|PERIAPSIS(_DT)?|RADIUS|SURFACE|VOLUME|Y" _
         & "EAR) TROPICALYEAR URANUS(_[ABC]|ASCENDING(_DT)?|D(" _
         & "AY|ENSITY)|ECCENTRICITY(_DT)?|FLATNESS|G(M|RAVITY)"
   desc_string = desc_string _
         & "|INCLINATION(_DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_DT)" _
         & "?|PERIAPSIS(_DT)?|RADIUS|S(URFACE|YNODIC)|VOLUME|Y" _
         & "EAR) VENUS(_[ABC]|ASCENDING(_DT)?|D(AY|ENSITY)|ECC" _
         & "ENTRICITY(_DT)?|FLATNESS|G(M|RAVITY)|INCLINATION(_" _
         & "DT)?|LONGITUDE(_DT)?|MASS|ORBIT(_DT)?|PERIAPSIS(_D" _
         & "T)?|RADIUS|S(URFACE|YNODIC)|VOLUME|YEAR)" _
         & vbNewLine
   desc_string = desc_string _
         ...
         & "vison|urface|ynodic) plvolume plyear" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "urface|ynodic)|volume|year) plaradius plgeo plheli" _
         & "o plsettime prem_(density|g|mass)" _
         & vbNewLine
   desc_string = desc_string _
         & "   Kwplanets 2-ary Functions: earth_gravity_height" _
