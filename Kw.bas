...
Public Const BOLTZMANNCONST As Double = BOLTZMANN
Public Const BOLTZMANNCONST_ERR As Double = 0.0
Public Const ELECTRONCHARGE As Double = 1.602176634E-19 * COULOMB
Public Const ELECTRONCHARGE_ERR As Double = 0.0
Public Const LIGHT As Double = 2.99792458E+8 * METER / SECOND
Public Const LIGHT_ERR As Double = 0.0
Public Const LIGHTCONST As Double = LIGHT
Public Const LIGHTCONST_ERR As Double = 0.0
Public Const AVOGADRO As Double = 6.02214076E+23 / MOLE
Public Const AVOGADRO_ERR As Double = 0.0
Public Const AVOGADROCONST As Double = AVOGADRO
Public Const AVOGADROCONST_ERR As Double = 0.0
Public Const PLANCK As Double = 6.62607015E-34 * JOULE * SECOND
Public Const PLANCK_ERR As Double = 0.0
Public Const PLANCKCONST As Double = PLANCK
Public Const PLANCKCONST_ERR As Double = 0.0
...
Public Const RYDBERGCONST As Double = RYDBERG
...
Public Const DBLMAXLN As Double = LNTWO * 1024.0
...
Public Const JULIANCENTURY As Double = 100.0 * JULIANYEAR
Public Const JULIANMILLENNIUM As Double = 1000.0 * JULIANYEAR
...
Public Const DBATTERYCHARGE As Double = 12.0 * AH
...
Public Const LFHz As Double = 1.0E+5 / SECOND
...
Public Const MFHz As Double = 1.0E+6 / SECOND
...
Public Const HFHz As Double = 1.0E+7 / SECOND
...
Public Const VHFHz As Double = 1.0E+8 / SECOND
...
Public Const UHFHz As Double = 1.0E+9 / SECOND
...
Public Const SHFHz As Double = 1.0E+10 / SECOND
...
Public Const EHFHz As Double = 1.0E+11 / SECOND
...
Public Const THFHz As Double = 1.0E+12 / SECOND
...
Public Const FIRHz As Double = 1.0E+12 / SECOND
...
Public Const MIRHz As Double = 1.0E+13 / SECOND
...
Public Const NIRHz As Double = 1.0E+14 / SECOND
...
Public Const REDWAVELENGTH As Double = 7.0E-7 * METER
...
Public Const ORANGEWAVELENGTH As Double = 6.2E-7 * METER
...
Public Const YELLOWWAVELENGTH As Double = 5.8E-7 * METER
...
Public Const YELLOWGREENHz As Double = 5.4E+14 / SECOND
...
Public Const GREENWAVELENGTH As Double = 5.3E-7 * METER
...
Public Const BLUEWAVELENGTH As Double = 5.0E-7 * METER
...
Public Const VIOLETWAVELENGTH As Double = 4.2E-7 * METER
...
Public Const NUVHz As Double = 1.0E+15 / SECOND
...
Public Const EUVHz As Double = 1.0E+16 / SECOND
...
Public Const UVAWAVELENGTH As Double = 3.6E-7 * METER
...
Public Const UVBWAVELENGTH As Double = 3.0E-7 * METER
...
Public Const UVCWAVELENGTH As Double = 1.9E-7 * METER
...
Public Const SXHz As Double = 3.0E+17 / SECOND
...
Public Const SXXWAVELENGTH As Double = 1.0E-9 * METER
...
Public Const HXHz As Double = 1.0E+19 / SECOND
...
Public Const GAMMAHz As Double = 1.0E+20 / SECOND
...
Public Const kgfm_PER_lbfin As Double = lbfin / (KG * GRAVITY _
      * METER)
Public Const lbfinrpm_PER_Hp As Double = HORSEPOWER / (lbfin _
      * RPM)
Public Const FINESTRUCTURE As Double = ALPHA
Public Const FINESTRUCTURE_ERR As Double = ALPHA_ERR
Public Const FINESTRUCTURECONST As Double = FINESTRUCTURE
...
Public Const ELECTRICCONST As Double = ELECTRIC
...
Public Const MOLARMASSCONST As Double = MOLARMASS
...
Public Const ELECTRONMASSA As Double = 2.0 * RYDBERGHz * _
      PLANCK / ((LIGHT * ALPHA) * (LIGHT * ALPHA))
...
Public Const PROTONENERGY As Double = PROTONMASS * (LIGHT * _
      LIGHT)
...
Public Const DEUTERONENERGY As Double = DEUTERONMASS * (LIGHT _
      * LIGHT)
Public Const DEUTERONHz As Double = DEUTERONENERGY / PLANCK
Public Const DEUTERONWAVELENGTH As Double = LIGHT / DEUTERONHz
...
Public Const HELIONENERGY As Double = HELIONMASS * (LIGHT * _
      LIGHT)
Public Const HELIONHz As Double = HELIONENERGY / PLANCK
Public Const HELIONWAVELENGTH As Double = LIGHT / HELIONHz
...
Public Const TRITONENERGY As Double = TRITONMASS * (LIGHT * _
      LIGHT)
Public Const TRITONHz As Double = TRITONENERGY / PLANCK
Public Const TRITONWAVELENGTH As Double = LIGHT / TRITONHz
...
Public Const ALPHAENERGY As Double = ALPHAMASS * (LIGHT * LIGHT)
Public Const ALPHAHz As Double = ALPHAENERGY / PLANCK
Public Const ALPHAWAVELENGTH As Double = LIGHT / ALPHAHz
...
Public Const PLANCKCHARGE As Double = 1.87554603814952214E-18 _
      * COULOMB
Public Const PLANCKENERGY As Double = PLANCKMASS * (LIGHT * _
      LIGHT)
...
Public Const STEFAN As Double = (TAU * TAU * TAU * TAU * TAU) _
      * (BOLTZMANN * BOLTZMANN * BOLTZMANN * BOLTZMANN) / _
      (240.0 * (LIGHT * LIGHT) * (PLANCK * PLANCK * PLANCK))
Public Const STEFAN_ERR As Double = 0.0
Public Const STEFANCONST As Double = STEFAN
Public Const STEFANCONST_ERR As Double = 0.0
...
Public Const SOFTBALLRADIUS As Double = 12.0 * INCH / TAU
Public Const SOCCERBALLRADIUS As Double = 0.69 * METER / TAU
Public Const BASKETBALLRADIUS As Double = 0.75 * METER / TAU
Public Const PINGPONGBALLMASS As Double = 0.0027 * KG
Public Const GOLFBALLMASS As Double = 0.04593 * KG
Public Const TENNISBALLMASS As Double = 0.0577 * KG
Public Const BASEBALLMASS As Double = 0.1465 * KG
Public Const SOFTBALLMASS As Double = 0.187 * KG
Public Const BASKETBALLMASS As Double = 0.620 * KG
Public Const SOCCERBALLMASS As Double = 0.430 * KG
...
Public Const PARSEC As Double = 1296000.0 * ASTRONOMICALUNIT _
      / TAU
...
Public Const UNIVERSERADIUS As Double = 14.26E+9 * PARSEC
Public Const UNIVERSEMASS As Double = 1.46E+53 * KILOGRAM
Public Const EINSTEIN As Double = 4.0 * TAU * GRAVITATION / _
...
Public Const HUBBLECONST As Double = HUBBLE
Public Const HUBBLECONST_ERR As Double = 6.0E-3
...
Public Const BHMASS3_LIFETIME As Double = (PLANCK * (LIGHT * _
      LIGHT * LIGHT * LIGHT)) / (2560.0 * (TAU * TAU) * _
      (GRAVITATION * GRAVITATION))
Public Const BHRADIUS2DENSITY As Double = 1.5 * BHMASS_RADIUS _
      / TAU
Public Const BHMASS2_ENTROPY As Double = (PLANCK * LIGHT) / _
...
Public Const BHMASS2POWER As Double = PLANCK * (LIGHT * LIGHT _
      * LIGHT * LIGHT * LIGHT * LIGHT) / (7680.0 * (TAU * _
      TAU) * (GRAVITATION * GRAVITATION))
Public Const BHMASS2_AREA As Double = BHMASS2_ENTROPY * _
      BHENTROPY_AREA
...
Private Const LC_HALAQIM As Double = HOUR / LC_HALAQIM_PER_HOUR
...
Private lv_sttx_mtrx(LC_STTS_MAX_KEY, LC_STTS_COUNT) As Double
...
Private lv_crx_mtrx(LC_STTS_MAX_KEY, LC_STTSC_COUNT) As Double
...
Public Const LC_SHC_SHIFTSHIFT As Double = 3.85514
Public Const LC_SHC_STEPSHIFT As Double = 8.5
Private Const LC_ELLIPTIC_EPS As Double = 1.0E-9
Private lv_mm As Double
...
Public Function cxapery() As Double
   cxapery = APERY
End Function
Public Function cxgammamin() As Double
   cxgammamin = GAMMAMIN
End Function
...
End Function
Public Function cxboltzmannconst() As Double
   cxboltzmannconst = BOLTZMANNCONST
End Function
Public Function cxboltzmannconst_err() As Double
   cxboltzmannconst_err = BOLTZMANNCONST_ERR
End Function
...
End Function
Public Function cxlightconst() As Double
   cxlightconst = LIGHTCONST
End Function
Public Function cxlightconst_err() As Double
   cxlightconst_err = LIGHTCONST_ERR
End Function
...
End Function
Public Function cxavogadroconst() As Double
   cxavogadroconst = AVOGADROCONST
End Function
Public Function cxavogadroconst_err() As Double
   cxavogadroconst_err = AVOGADROCONST_ERR
End Function
...
End Function
Public Function cxplanckconst() As Double
   cxplanckconst = PLANCKCONST
End Function
Public Function cxplanckconst_err() As Double
   cxplanckconst_err = PLANCKCONST_ERR
End Function
...
End Function
Public Function cxfaradayconst() As Double
   cxfaradayconst = FARADAYCONST
End Function
Public Function cxfaradayconst_err() As Double
   cxfaradayconst_err = FARADAYCONST_ERR
End Function
...
Public Function cxgas_err() As Double
   cxgas_err = GAS_ERR
End Function
Public Function cxgasconst() As Double
   cxgasconst = GASCONST
End Function
Public Function cxgasconst_err() As Double
   cxgasconst_err = GASCONST_ERR
End Function
...
End Function
Public Function cxgravitationconst() As Double
   cxgravitationconst = GRAVITATIONCONST
End Function
Public Function cxgravitationconst_err() As Double
   cxgravitationconst_err = GRAVITATIONCONST_ERR
End Function
...
End Function
Public Function cxrydbergconst() As Double
   cxrydbergconst = RYDBERGCONST
End Function
Public Function cxrydbergconst_err() As Double
   cxrydbergconst_err = RYDBERGCONST_ERR
End Function
...
Public Function cxounce() As Double
   cxounce = OUNCE
End Function
Public Function cxoz() As Double
   cxoz = OZ
End Function
...
Public Function cxinch() As Double
   cxinch = INCH
End Function
Public Function cxhand() As Double
   cxhand = HAND
End Function
Public Function cxfoot() As Double
   cxfoot = FOOT
End Function
Public Function cxfeet() As Double
   cxfeet = FEET
End Function
...
Public Function cxyard() As Double
   cxyard = YARD
End Function
Public Function cxhorse() As Double
   cxhorse = HORSE
End Function
...
Public Function cxrps() As Double
   cxrps = RPS
End Function
Public Function cxrph() As Double
   cxrph = RPH
End Function
Public Function cxlbfin() As Double
   cxlbfin = lbfin
End Function
Public Function cxnm_per_lbfin() As Double
   cxnm_per_lbfin = Nm_PER_lbfin
End Function
Public Function cxkgfm_per_lbfin() As Double
   cxkgfm_per_lbfin = kgfm_PER_lbfin
End Function
Public Function cxlbfinrpm_per_hp() As Double
   cxlbfinrpm_per_hp = lbfinrpm_PER_Hp
End Function
...
End Function
Public Function cxfinestructureconst() As Double
   cxfinestructureconst = FINESTRUCTURECONST
End Function
Public Function cxfinestructureconst_err() As Double
   cxfinestructureconst_err = FINESTRUCTURECONST_ERR
End Function
...
End Function
Public Function cxelectricconst() As Double
   cxelectricconst = ELECTRICCONST
End Function
Public Function cxelectricconst_err() As Double
   cxelectricconst_err = ELECTRICCONST_ERR
End Function
...
End Function
Public Function cxmolarmassconst() As Double
   cxmolarmassconst = MOLARMASSCONST
End Function
Public Function cxmolarmassconst_err() As Double
   cxmolarmassconst_err = MOLARMASSCONST_ERR
End Function
...
End Function
Public Function cxdeuteronenergy() As Double
   cxdeuteronenergy = DEUTERONENERGY
End Function
Public Function cxdeuteronhz() As Double
   cxdeuteronhz = DEUTERONHz
End Function
Public Function cxdeuteronwavelength() As Double
   cxdeuteronwavelength = DEUTERONWAVELENGTH
End Function
...
End Function
Public Function cxhelionenergy() As Double
   cxhelionenergy = HELIONENERGY
End Function
Public Function cxhelionhz() As Double
   cxhelionhz = HELIONHz
End Function
Public Function cxhelionwavelength() As Double
   cxhelionwavelength = HELIONWAVELENGTH
End Function
...
End Function
Public Function cxtritonenergy() As Double
   cxtritonenergy = TRITONENERGY
End Function
Public Function cxtritonhz() As Double
   cxtritonhz = TRITONHz
End Function
Public Function cxtritonwavelength() As Double
   cxtritonwavelength = TRITONWAVELENGTH
End Function
...
End Function
Public Function cxalphaenergy() As Double
   cxalphaenergy = ALPHAENERGY
End Function
Public Function cxalphahz() As Double
   cxalphahz = ALPHAHz
End Function
Public Function cxalphawavelength() As Double
   cxalphawavelength = ALPHAWAVELENGTH
End Function
...
End Function
Public Function cxstefanconst() As Double
   cxstefanconst = STEFANCONST
End Function
Public Function cxstefanconst_err() As Double
   cxstefanconst_err = STEFANCONST_ERR
End Function
...
End Function
Public Function cxhubbleconst() As Double
   cxhubbleconst = HUBBLECONST
End Function
Public Function cxhubbleconst_err() As Double
   cxhubbleconst_err = HUBBLECONST_ERR
End Function
Public Function cxhubbleradius() As Double
   cxhubbleradius = HUBBLERADIUS
End Function
...
End Function
Public Function cxbhmass2_entropy() As Double
   cxbhmass2_entropy = BHMASS2_ENTROPY
End Function
Public Function cxbhentropy_area() As Double
   cxbhentropy_area = BHENTROPY_AREA
End Function
Public Function cxbhmass2power() As Double
   cxbhmass2power = BHMASS2POWER
End Function
Public Function cxbhmass2_area() As Double
   cxbhmass2_area = BHMASS2_AREA
End Function
Public Function cxbhmass2_density() As Double
   cxbhmass2_density = BHMASS2_DENSITY
End Function
...
End Function
Public Function logx(ByVal xx As Double, ByVal bb As Double) _
      As Double
...
Public Function iseq(ByVal xx As Double, ByVal yy As Double) _
   ...
   iseq = (retvalu_bol)
End Function
Public Function isneq(ByVal xx As Double, ByVal yy As Double) _
      As Boolean
...
Public Function sincinv(ByVal yy As Double) As Double
   ...
   ElseIf (yy < 1.0) Then
      aa = yy - min_yy
      bb = 1.0 - yy
      start_xx = (aa * Sqr(5.0 * bb) + bb * (min_xx - Sqr(6.0 _
            * aa))) / (1.0 - min_yy)
      nrvx_xx = start_xx
      For nrvx_ii = 0 To 20
         nrvx_xp = nrvx_xx
...
Public Function num2char_print(ByVal num As Long) As Long
   ...
   num2char_print = (num)
End Function
Public Function base_out_print(ByVal num As Long, ByVal baset _
      As Long) As Long
...
End Function
Public Function base_out_fix_print(ByVal num As Long, ByVal _
      baset As Long, ByVal min_sz As Long) As Long
   Dim digits_arr(65) As Long
   Dim dig_count As Long
   Dim ii As Long
   If (num < 0) Then
...
End Function
Public Function base_outf_print(ByVal num As Double, ByVal _
...
Public Function repetendlen(ByVal denm As Long, ByVal basee _
   ...
   repetendlen = (retvalu)
End Function
Public Function kevy_print(ByVal den As Long, ByVal start As _
...
Public Function bessK0(ByVal arg As Double) As Double
   ...
   ElseIf (xx <= 1.0) Then
      ...
      sumq = ((1.0) * xz - 2.4994418972832303646E+02) * xz + _
   ...
   ElseIf (xx < Kw.DBLMAXLN) Then
      ...
      sumq = ((((((((((1.0) * xz + 2.0013443064949242491E+02) _
            * xz + 4.4329628889746408858E+03) * xz + _
...
Public Function bessK1(ByVal arg As Double) As Double
   ...
   ElseIf (xx < Kw.DBLMAXLN) Then
      ...
      sumq = (((((((((1.0) * xz + 3.6001069306861518855E+1) * _
            xz + 3.3031020088765390854E+2) * xz + _
...
Private Function lf_besucjy0_xge5(ByVal xx As Double, ByVal _
   ...
   numer = (((((((7.96936729297347051624E-4) * zz + _
         ...
         1.23953371646414299388E+0) * zz + _
         5.44725003058768775090E+0) * zz + _
         8.74716500199817011941E+0) * zz + _
         5.30324038235394892183E+0) * zz + _
         9.99999999999999997821E-1)
   denom = (((((((9.24408810558863637013E-4) * zz + _
         8.56288474354474431428E-2) * zz + _
         1.25352743901058953537E+0) * zz + _
         5.47097740330417105182E+0) * zz + _
         8.76190883237069594232E+0) * zz + _
         5.30605288235394617618E+0) * zz + _
         1.00000000000000000218E+0)
   pp = numer / denom
   numer = ((((((((-1.13663838898469149931E-2) * zz - _
         1.28252718670509318512E+0) * zz - _
         1.95539544257735972385E+1) * zz - _
         9.32060152123768231369E+1) * zz - _
         1.77681167980488050595E+2) * zz - _
         1.47077505154951170175E+2) * zz - _
         5.14105326766599330220E+1) * zz - _
         6.05014350600728481186E+0)
   denom = ((((((((1.0) * zz + 6.43178256118178023184E+1) * _
         zz + 8.56430025976980587198E+2) * zz + _
         3.88240183605401609683E+3) * zz + _
         7.24046774195652478189E+3) * zz + _
         5.93072701187316984827E+3) * zz + _
         2.06209331660327847417E+3) * zz + _
...
Private Function lf_besucjy1_xge5(ByVal xx As Double, ByVal _
   ...
   numer = (((((((7.62125616208173112003E-4) * zz + _
         ...
         1.12719608129684925192E+0) * zz + _
         5.11207951146807644818E+0) * zz + _
         8.42404590141772420927E+0) * zz + _
         5.21451598682361504063E+0) * zz + _
         1.00000000000000000254E+0)
   denom = (((((((5.71323128072548699714E-4) * zz + _
         6.88455908754495404082E-2) * zz + _
         1.10514232634061696926E+0) * zz + _
         5.07386386128601488557E+0) * zz + _
         8.39985554327604159757E+0) * zz + _
         5.20982848682361821619E+0) * zz + _
         9.99999999999999997461E-1)
   pp = numer / denom
   numer = ((((((((5.10862594750176621635E-2) * zz + _
         4.98213872951233449420E+0) * zz + _
         7.58238284132545283818E+1) * zz + _
         3.66779609360150777800E+2) * zz + _
         7.10856304998926107277E+2) * zz + _
         5.97489612400613639965E+2) * zz + _
         2.11688757100572135698E+2) * zz + _
         2.52070205858023719784E+1)
   denom = ((((((((1.0) * zz + 7.42373277035675149943E+1) * _
         zz + 1.05644886038262816351E+3) * zz + _
         4.98641058337653607651E+3) * zz + _
         9.56231892404756170795E+3) * zz + _
         7.99704160447350683650E+3) * zz + _
         2.82619278517639096600E+3) * zz + _
...
Public Function bessJ0(ByVal xx As Double) As Double
   ...
   ElseIf (xx < 5.0) Then
      numer = ((((-4.79443220978201773821E+9) * zz + _
            ...
            2.49248344360967716204E+14) * zz + _
            9.70862251047306323952E+15)
      denom = (((((((((1.0) * zz + 4.99563147152651017219E+2) _
            * zz + 1.73785401676374683123E+5) * zz + _
            ...
            3.18121955943204943306E+16) * zz + _
...
Public Function bessJ1(ByVal xx As Double) As Double
   ...
   If (ww < 5.0) Then
      ...
      numer = ((((-8.99971225705559398224E+8) * zz + _
            ...
            7.27494245221818276015E+13) * zz + _
            3.68295732863852883286E+15)
      denom = (((((((((1.0) * zz + 6.20836478118054335476E+2) _
            * zz + 2.56987256757748830383E+5) * zz + _
            ...
            8.95222336184627338078E+16) * zz + _
...
Public Function bessY0(ByVal xx As Double) As Double
   ...
   Else
      ...
      ElseIf (xx < 5.0) Then
         ...
         denom = ((((((((1.0) * zz + _
               1.04128353664259848412E+3) * zz + _
...
Public Function bessY1(ByVal xx As Double) As Double
   ...
   ElseIf (ww <= 5.0) Then
      ...
      numer = ((((((1.2632047479017802640E+9) * zz - _
            ...
            2.02439475713594898196E+17) * zz - _
            7.78877196265950026825E+17)
      denom = (((((((((1.0) * zz + 5.94301592346128195359E+2) _
            * zz + 2.35564092943068577943E+5) * zz + _
            ...
            6.87141087355300489866E+16) * zz + _
...
Private Function lf_calerf(ByVal xx As Double, ByVal jint As _
   ...
   If (absx <= 0.46875) Then
      ...
      denom = (((((1.0) * xsq + 2.36012909523441209E+1) * xsq _
   ...
   ElseIf (absx <= 4.0) Then
      ...
      denom = (((((((((1.0) * absx + 1.57449261107098347E+1) _
            * absx + 1.17693950891312499E+2) * absx + _
...
Public Function n2perm(ByVal mm As Long, ByVal nn As Long, _
   ...
   n2perm = (mmm)
End Function
Public Function n2perm_print(ByVal mm As Long, ByVal nn As _
      Long) As Long
...
Public Function n2comb(ByVal mm As Long, ByVal nn As Long, _
   ...
   n2comb = (mmm)
End Function
Public Function n2comb_print(ByVal mm As Long, ByVal nn As _
      Long, ByVal rr As Long) As Long
   ...
   n2comb_print = (jj)
End Function
Public Function pochhammer(ByVal nn As Long, ByVal xx As _
      Double) As Double
   Dim retvalu As Double
   Dim ii As Long
   If (0 < nn) Then
...
End Function
Public Function ymd2j2k__1(ByVal g_year As Double, ByVal _
   ...
   jtwok = (floor(yr * Kw.JYEAR / Kw.JDAY) + floor(30.6001 * _
...
Public Function ymd2j2k(ByVal g_year As Double, ByVal g_month _
   ...
   mn = g_month
   jcen = floor((yr - 1) / 100)
   jtwok = (floor(yr * Kw.JYEAR / Kw.JDAY - 0.0001) + _
         floor(jcen / 4.0) - jcen + ymd2doy(yr, mn, g_day) - 0.5)
   ymd2j2k = (jtwok)
End Function
...
Public Function date_easter(ByVal g_year As Long) As Long
   ...
   Dim lps As Long
   cc = floor(g_year / 100)
   nn = g_year Mod LC_YEAR_PER_METONIC
   lps = floor(g_year / 4)
   kk = floor((cc + 8) / 25 - 1)
   ff = floor((cc - kk) / 3)
   mm = cc - floor(cc / 4)
   ii = (mm - ff + LC_YEAR_PER_METONIC * nn + 15) Mod 30
...
Public Function date_rosh_hashanah(ByVal g_year As Long) As _
   ...
   dos = (floor(g_year / 100) - floor(g_year / 400) - 2 + _
         ...
         Kw.LC_HALAQIM) * aa - 1565.0 * g_year - 445405) / _
         (LC_YEAR_PER_METONIC * Kw.EARTHSOLARDAY / LC_HALAQIM))
   day_of_september = floor(dos)
   parts = Kw.frac(dos) * Kw.EARTHSOLARDAY / LC_HALAQIM
...
Public Function jewish2jdx(ByVal hebyear As Long) As Double
   ...
   months_el = (235 * floor((hebyear - 1) / _
         LC_YEAR_PER_METONIC) + 12 * hymod + floor((7 * hymod _
         + 1) / LC_YEAR_PER_METONIC))
   halaqims_el = 204 + (793 * (months_el Mod _
         LC_HALAQIM_PER_HOUR))
   hour_el = (5 + (12 * months_el) + 793 * floor(months_el / _
         LC_HALAQIM_PER_HOUR) + floor(halaqims_el / _
         LC_HALAQIM_PER_HOUR))
   ...
   hq_left = hq_left + Kw.EARTHSOLARDAY * Kw.frac(hour_el * _
         Kw.HOUR / Kw.EARTHSOLARDAY) / LC_HALAQIM
...
Public Function isjewishleap(ByVal hebyear As Long) As Boolean
   isjewishleap = (((7 * hebyear + 1) Mod LC_YEAR_PER_METONIC _
         < 7))
End Function
...
Public Function jewish_yearlength(ByVal hebyear As Long) As Long
   ...
   retvalu = (floor(Kw.JYEAR / Kw.JDAY) + _
...
Public Function agmean(ByVal xa As Double, ByVal xb As _
   ...
   Else
      ...
      Do While (True)
         ...
         xb = xtmp
         cdifff = xa - xb
         If (Kw.iswithin(xa, xb, 10.0 * Kw.DBLEPS) Or pdifff _
               < cdifff) Then
            Exit Do' loop AWAIT
         End If
         ii = ii + 1
...
Public Function agmean__1(ByVal xa As Double, ByVal xb As _
   ...
   agmean__1 = (retvalu)
End Function
Public Function agcmean(xa_arr_pc() As Double, xb_arr_pc() As _
   ...
   Do While (True)
      ...
      aa_arr(0) = tt_arr(0)
      aa_arr(1) = tt_arr(1)
      If (aa_arr(0) * gg_arr(0) + aa_arr(1) * gg_arr(1) < _
            0.0) Then
         gg_arr(0) = -gg_arr(0)
         gg_arr(1) = -gg_arr(1)
      End If
...
End Function
Public Function agmeaninva(ByVal aa As Double, ByVal gg As _
      Double) As Double
   ...
   agmeaninva = (new_aa)
End Function
Public Function agmeaninvg(ByVal aa As Double, ByVal gg As _
      Double) As Double
   ...
   Else
      ...
      ElseIf (yy < 2.1) Then
         ...
         x_init = ((((((((-0.09765625) * cc + 0.12890625) * _
               cc + 0.1875) * cc + 0.1875) * cc - 0.25) * cc _
               + 0.5) * cc + 2.0) * cc + 1.0)
      Else
         cc = -(Kw.PI / 2.0)
...
Public Function blackbodyl_pdf(ByVal wavelen As Double) As Double
   ...
   If (wavelen <= 0.0) Then
      retvalu = 0.0
   Else
      retvalu = Pow(wavelen, -5.0) / (Math.Exp(1.0 / wavelen) _
            - 1.0)
   End If
...
End Function
Public Function circle_solve(ByVal x_a As Double, ByVal y_a _
      As Double, ByVal x_b As Double, ByVal y_b As Double, _
      ByVal x_c As Double, ByVal y_c As Double) As Double
...
End Function
Private Function lf_signcrossproduct(ByVal x_a As Double, _
...
Public Function sqrt__1(ByVal xx As Double) As Double
   ...
   Else
      ...
      Do While (zz < 1.0)
         ...
         power_of_two = power_of_two / 2.0
      Loop
      yy_curr = (((-0.0353026277858345) * zz + _
...
Public Function ln__1(ByVal xx As Double) As Double
   ...
   Do While (tmp < yy_curr)
      logshift = logshift - 16.0 * Kw.LNTWO
...
Public Function euler__1() As Double
   ...
   Do While (True)
      ...
      mm = mm + 1
   Loop
   euler__1 = ((twon * sum_curr / exptwon - (nn + 1.0) * _
         Kw.LNTWO))
End Function
...
End Function
Public Function ellipsoid_surface(ByVal aa As Double, ByVal _
      bb As Double, ByVal cc As Double) As Double
   Dim absaa As Double
   Dim absbb As Double
   Dim abscc As Double
   Dim phi As Double
   ...
   If (abscc < absaa) Then
      ...
      sinphi = Math.Sin(phi)
      kk = Sqr((1.0 - abscc * abscc / (absbb * absbb)) / (1.0 _
...
End Function
Public Function ellipsoid_volume(ByVal aa As Double, ByVal bb _
      As Double, ByVal cc As Double) As Double
...
Public Function k2m(ByVal kk As Double) As Double
   k2m = (kk * kk)
End Function
Public Function a2k(ByVal aa As Double) As Double
   a2k = ((Math.Sin(aa)))
End Function
Public Function k2a(ByVal kk As Double) As Double
   k2a = ((Kw.sininv(kk)))
End Function
Public Function a2m(ByVal aa As Double) As Double
   a2m = ((Kw.square(Math.Sin(aa))))
End Function
Public Function m2a(ByVal mm As Double) As Double
   m2a = ((Kw.sininv(Sqr(mm))))
End Function
...
Public Function b2m(ByVal bb As Double) As Double
   b2m = ((1.0 - 1.0 / (bb * bb)))
End Function
Public Function elliptic1m(ByVal mm As Double) As Double
   Dim retvalu As Double
   Dim aa_arr(2) As Double
   ...
   Dim dd As Double
   If (mm < 1.0) Then
      retvalu = Kw.PI / (2.0 * Kw.agmean(1.0, Sqr(1.0 - mm)))
   ElseIf (1.0 < mm) Then
      aa_arr(0) = 1.0
      aa_arr(1) = 0.0
      bb_arr(0) = 0.0
...
End Function
Public Function elliptic1m__1(ByVal mm As Double) As Double
   elliptic1m__1 = ((Kw.carlsonRF(0.0, 1.0 - mm, 1.0)))
End Function
Public Function elliptic1m__2(ByVal mm As Double) As Double
   Dim term As Double
   Dim sum_curr As Double
   Dim sum_prev As Double
   Dim ii As Long
   ...
   ii = 1
   Do While (True)
      term = term * Kw.square(mm * ii / (ii + 1.0))
      sum_prev = sum_curr
      sum_curr = sum_curr + term
      If (Kw.iseq(sum_curr, sum_prev)) Then
         Exit Do' loop AWAIT
...
End Function
Public Function elliptic1im(ByVal mm As Double, ByVal _
   ...
   ElseIf (Kw.iszero(mc)) Then
      If (Kw.PI / 2.0 <= Math.Abs(circ_rad)) Then
         retvalu = Kw.DBLMAX
      Else
         retvalu = Math.Log(Math.Tan((Kw.PI / 2.0 + circ_rad) _
               / 2.0))
      End If
   Else
      ...
      If (10.0 < Math.Abs(tt)) Then
         ...
         If (Math.Abs(ee) < 10.0) Then
            If (npihlfs = 0) Then
               kresult = Kw.elliptic1m(mm)
            End If
            temp = kresult - Kw.elliptic1im(mm, Atan2(ee, 1.0))
            skip_flag = True
         End If
      End If
      If (Not skip_flag) Then
         ...
         Do While (True)
            temp = ggn / aan
            lcirc_rad = lcirc_rad + Atan2(tt * temp, 1.0) + _
                  modd * Kw.PI
...
End Function
Public Function elliptic1ik(ByVal kk As Double, ByVal _
      circ_rad As Double) As Double
...
End Function
Public Function elliptic1ia(ByVal aa As Double, ByVal _
      circ_rad As Double) As Double
...
End Function
Public Function elliptic1im__1(ByVal mm As Double, ByVal _
      circ_rad As Double) As Double
   Dim sinphi As Double
   Dim npihlfs As Long
   Dim lcirc_rad As Double
   ...
   elliptic1im__1 _
         = ((sinphi * _
               Kw.carlsonRF(Kw.square(Math.Cos(lcirc_rad)), _
               1.0 - mm * Kw.square(sinphi), 1.0) + kresult))
End Function
Public Function elliptic2m(ByVal mm As Double) As Double
   Dim aan As Double
   Dim ggn As Double
   Dim tn As Double
   Dim twon As Double
...
End Function
Public Function elliptic2m__1(ByVal mm As Double) As Double
   elliptic2m__1 _
         = ((Kw.carlsonRF(0.0, 1.0 - mm, 1.0) - mm * _
               Kw.carlsonRD(0.0, 1.0 - mm, 1.0) / 3.0))
End Function
Public Function elliptic2im(ByVal mm As Double, ByVal _
      circ_rad As Double) As Double
   Dim aan As Double
   Dim ggn As Double
   Dim cc As Double
   ...
   Else
      ...
      Else
         ...
         Else
            ...
            Loop
            temp = eliptwomm * (Atan2(tt, 1.0) + modd * _
...
End Function
Public Function elliptic2im__1(ByVal mm As Double, ByVal _
      circ_rad As Double) As Double
   Dim sinphi As Double
   ...
   elliptic2im__1 _
         = ((sinphi * carlsonRF(cosphisq, argb, 1.0) - mm * _
               sinphicu * carlsonRD(cosphisq, argb, 1.0) / _
               3.0 + kresult))
End Function
Public Function elliptic2k(ByVal kk As Double) As Double
   elliptic2k = ((elliptic2m(k2m(kk))))
End Function
Public Function elliptic2ik(ByVal kk As Double, ByVal _
      circ_rad As Double) As Double
...
End Function
Public Function elliptic2ia(ByVal aa As Double, ByVal _
      circ_rad As Double) As Double
   elliptic2ia = ((elliptic2im(a2m(aa), circ_rad)))
End Function
Public Function elliptic3m(ByVal nn As Double, ByVal mm As _
      Double) As Double
   Dim aan As Double
   Dim ggn As Double
   Dim ppn As Double
   ...
   elliptic3m = (retvalu)
End Function
Public Function elliptic3k(ByVal nn As Double, ByVal kk As _
      Double) As Double
   elliptic3k = ((elliptic3m(nn, k2m(kk))))
End Function
Public Function elliptic3ik(ByVal nn As Double, ByVal kk As _
      Double, ByVal circ_rad As Double) As Double
   elliptic3ik = ((elliptic3im(nn, k2m(kk), circ_rad)))
End Function
Public Function elliptic3a(ByVal nn As Double, ByVal aa As _
      Double) As Double
   elliptic3a = ((elliptic3m(nn, a2m(aa))))
End Function
Public Function elliptic3ia(ByVal nn As Double, ByVal aa As _
      Double, ByVal circ_rad As Double) As Double
...
End Function
Public Function elliptic3im(ByVal nn As Double, ByVal mm As _
      Double, ByVal circ_rad As Double) As Double
   Dim sp As Double
   Dim cp As Double
   Dim spq As Double
   ...
   spn = 1.0 - nn * spq
   elliptic3im _
         = ((sp * (Kw.carlsonRF(cpq, spm, 1.0) + spq * nn * _
...
End Function
Public Function elliptic3m__1(ByVal nn As Double, ByVal mm As _
      Double) As Double
   elliptic3m__1 _
         = ((Kw.carlsonRF(0.0, 1.0 - mm, 1.0) + nn * _
               Kw.carlsonRJ(0.0, 1.0 - mm, 1.0, 1.0 - nn) / 3.0))
End Function
Public Function elliptic3im__1(ByVal nn As Double, ByVal mm _
      As Double, ByVal circ_rad As Double) As Double
   Dim sinphi As Double
   Dim sinphisq As Double
   Dim sinphicu As Double
   Dim cosphisq As Double
   ...
   argb = 1.0 - mm * sinphisq
   elliptic3im__1 _
         = ((sinphi * carlsonRF(cosphisq, argb, 1.0) + (nn / _
               3.0) * sinphicu * carlsonRJ(cosphisq, argb, _
               1.0, 1.0 - nn * sinphisq) + kresult))
End Function
...
Public Function jacobicir(ByVal mm As Double) As Double
   jacobicir = ((4.0 * elliptic2m(mm) / Sqr(1.0 - mm)))
End Function
Public Function lv_ellpj(ByVal mm As Double, ByVal elli_rad _
   ...
   If (mm < 0.0) Then
      snx = -sn(1.0 - mm, elli_rad) + Math.Sin(elli_rad) + _
            Kw.tanh(elli_rad)
      csx = -cn(-mm, elli_rad) + 2.0 * Math.Cos(elli_rad)
      ret_arr_p(0) = snx
      ret_arr_p(1) = csx
      ret_arr_p(2) = -dn(-mm, elli_rad) + 2.0
      ret_arr_p(3) = Atan2(snx, csx)
   ElseIf (mm < LC_ELLIPTIC_EPS) Then
   ...
   ElseIf (mm < 1.0 - LC_ELLIPTIC_EPS) Then
...
Public Function cn(ByVal mm As Double, ByVal elli_rad As _
   ...
   Dim retvalu As Double
   If (mm < 0.0) Then
      retvalu = -cn(-mm, elli_rad) + 2.0 * Math.Cos(elli_rad)
   ElseIf (mm < LC_ELLIPTIC_EPS) Then
      snx = Math.Sin(elli_rad)
      csx = Math.Cos(elli_rad)
      ai = mm * (elli_rad - snx * csx) / 4.0
...
Public Function nevillethetac(ByVal mm As Double, ByVal zz As _
   ...
   Do While (True)
      ...
      nn = nn + 1
   Loop
   nevillethetac = ((sum_curr * Sqr(Kw.TAU * Sqr(qq / mm) / km)))
End Function
Public Function nevillethetad(ByVal mm As Double, ByVal zz As _
   ...
   Loop
   nevillethetad _
         = (((1.0 + 2.0 * sum_curr) * Sqr(Kw.TAU / km) / 2.0))
End Function
Public Function nevillethetan(ByVal mm As Double, ByVal zz As _
   ...
   Loop
   nevillethetan _
         = (((1.0 + 2.0 * sum_curr) * Sqr(Kw.TAU / (km * _
               Sqr(1.0 - mm))) / 2.0))
End Function
Public Function nevillethetas(ByVal mm As Double, ByVal zz As _
   ...
   Loop
   nevillethetas _
         = ((sum_curr * Sqr(Kw.TAU * Sqr(qq / (mm * (1.0 - _
               mm))) / km)))
End Function
...
End Function
Public Function ellipsen(ByVal aa As Double, ByVal bb As _
      Double) As Double
...
End Function
Public Function ellipsecir(ByVal aa_orig As Double, ByVal _
      bb_orig As Double) As Double
   Dim aaa As Double
   Dim bbb As Double
   Dim mm As Double
   Dim suma As Double
...
Public Function search_search(ByVal gg As Long) As Long
   ...
   search_search = (curr)
End Function
Public Function blackbodyl(ByVal wavelen As Double, ByVal _
   ...
   Else
      ...
      retvalu = 2.0 * hc * Kw.LIGHT / (Pow(wavelen, 5) * _
            (Math.Exp(hc_kt / wavelen) - 1.0))
   End If
   blackbodyl = (retvalu)
End Function
Public Function blackbodyf(ByVal wavefreq As Double, ByVal _
      temperk As Double) As Double
   Dim h_kt As Double
   Dim retvalu As Double
   ...
   Else
      ...
      retvalu = 2.0 * Kw.PLANCK * Pow(wavefreq, 3) / _
            (Pow(Kw.LIGHT, 2) * (Math.Exp(h_kt * wavefreq) - _
            1.0))
   End If
   blackbodyf = (retvalu)
End Function
...
End Function
Public Function bhmass_to_temp(ByVal bhmass As Double) As Double
   bhmass_to_temp = (BHMASSTEMP / bhmass)
End Function
Public Function bhtemp_to_mass(ByVal bhtempk As Double) As Double
   bhtemp_to_mass = (BHMASSTEMP / bhtempk)
End Function
Public Function bhmass_to_radius(ByVal bhmass As Double) As _
      Double
   bhmass_to_radius = (bhmass / BHMASS_RADIUS)
End Function
Public Function bhradius_to_mass(ByVal bhradius As Double) As _
      Double
   bhradius_to_mass = (BHMASS_RADIUS * bhradius)
End Function
Public Function bhmass_to_entropy(ByVal bhmass As Double) As _
      Double
   bhmass_to_entropy = (((bhmass * bhmass) / BHMASS2_ENTROPY))
End Function
Public Function bhentropy_to_mass(ByVal bhentropy As Double) _
      As Double
   bhentropy_to_mass = ((Sqr(bhentropy * BHMASS2_ENTROPY)))
End Function
Public Function bhmass_to_power(ByVal bhmass As Double) As Double
   bhmass_to_power = ((BHMASS2POWER / (bhmass * bhmass)))
End Function
Public Function bhpower_to_mass(ByVal bhpower As Double) As _
      Double
   bhpower_to_mass = ((Sqr(BHMASS2POWER / bhpower)))
End Function
Public Function bhmass_to_area(ByVal bhmass As Double) As Double
   bhmass_to_area = (((bhmass * bhmass) / BHMASS2_AREA))
End Function
Public Function bharea_to_mass(ByVal bharea As Double) As Double
   bharea_to_mass = ((Sqr(BHMASS2_AREA * bharea)))
End Function
Public Function bhmass_to_lifetime(ByVal bhmass As Double) As _
      Double
   bhmass_to_lifetime _
         = (((bhmass * bhmass * bhmass) / BHMASS3_LIFETIME))
End Function
Public Function bhlifetime_to_mass(ByVal bhlifetime As _
      Double) As Double
   bhlifetime_to_mass = ((cbrt(BHMASS3_LIFETIME * bhlifetime)))
End Function
Public Function bhmass_to_density(ByVal bhmass As Double) As _
      Double
   bhmass_to_density = (((bhmass * bhmass) / BHMASS2_DENSITY))
End Function
Public Function bhdensity_to_mass(ByVal bhdensity As Double) _
      As Double
...
Public Function pendulum(ByVal len_m As Double, ByVal _
   ...
   pendulum _
         = ((Kw.TAU * Sqr(len_m / grav) / Kw.agmean(1.0, _
...
Public Function kw_description() As String
   ...
   desc_string = desc_string _
         ...
         & "REC ALPHA_REC_ERR ALPHAENERGY ALPHAHz ALPHAMASS AL" _
         & "PHAMASS_ERR ALPHAWAVELENGTH AMP AMPERE AMU AMU_ERR" _
         & " ANGLE APERY ARCDEG ARCDEGREE ARCMIN ARCMINUTE ARC"
   desc_string = desc_string _
         & "SEC ARCSECOND ARE AREA ASTRONOMICALUNIT ATM ATMOSP" _
         & "HERE ATMOSPHERE_ERR ATOMICMASSUNIT ATOMICMASSUNIT_" _
         & "ERR ATTO AU AU_ERR AVOGADRO AVOGADRO_ERR AVOGADRO_" _
         & "REC AVOGADROCONST AVOGADROCONST_ERR BAN BAR BARN B"
   desc_string = desc_string _
         & "ARREL BASEBALLMASS BASEBALLRADIUS BASKETBALLMASS B" _
         & "ASKETBALLRADIUS BEKENSTEINENTROPY BEKENSTEININFORM" _
         & "ATION BHENTROPY_AREA BHMASS2_AREA BHMASS2_DENSITY " _
         & "BHMASS2_ENTROPY BHMASS2POWER BHMASS3_LIFETIME BHMA"
   desc_string = desc_string _
         & "SS_RADIUS BHMASSTEMP BHRADIUS2DENSITY BIT BLUEENER" _
         & "GY BLUEHz BLUEWAVELENGTH BLUEWAVENUMBER BOHRRADIUS" _
         & " BOHRRADIUS_ERR BOLTZMANN BOLTZMANN_ERR BOLTZMANN_" _
         & "REC BOLTZMANNCONST BOLTZMANNCONST_ERR BTU BTU_IT B"
   desc_string = desc_string _
         & "TU_TH CAL CAL_IT CAL_TH CALORIE CANDELA CAPACITANC" _
         & "E CARAT CARBATTERY CARBATTERYCHARGE CARBATTERYENER" _
         & "GY CARBATTERYVOLTAGE CBATTERY CBATTERYCHARGE CBATT" _
         & "ERYENERGY CBATTERYVOLTAGE CENTI CHARGE CONDUCTANCE"
   desc_string = desc_string _
         & " COULOMB COULOMBCONST COULOMBCONST_ERR CUP CURRENT" _
         & " DALTON DALTON_ERR DBATTERY DBATTERYCHARGE DBATTER" _
         & "YENERGY DBATTERYVOLTAGE DBLEPS DBLMAN DBLMAX DBLMA" _
         & "XLN DBLSIG DECA DECI DEGC DEGF DEGR DENSITY DEUTER"
   desc_string = desc_string _
         & "ON_MASS DEUTERONENERGY DEUTERONHz DEUTERONMASS DEU" _
         & "TERONMASS_ERR DEUTERONWAVELENGTH DIMEMASS DIMERADI" _
         & "US DIMETHICKNESS DISTANCE DIT DOWNQUARK_MASS DOWNQ" _
         & "UARKENERGY DOWNQUARKHz DOWNQUARKMASS DOWNQUARKMASS"
   desc_string = desc_string _
         & "_ERR DOWNQUARKWAVELENGTH dvCs_ERR dvCsENERGY dvCsH" _
         & "z dvCsHz_REC dvCsWAVELENGTH dvCsWAVENUMBER E EARTH" _
         & "AGE EARTHGRAVITYA EARTHGRAVITYB EARTHSOLARDAY EHFE" _
         & "NERGY EHFHz EHFWAVELENGTH EHFWAVENUMBER EINSTEIN E"
   desc_string = desc_string _
         & "INSTEIN_ERR ELECTRIC ELECTRIC_ERR ELECTRICCONST EL" _
         & "ECTRICCONST_ERR ELECTRON_MASS ELECTRONCHARGE ELECT" _
         & "RONCHARGE_ERR ELECTRONCHARGE_REC ELECTRONENERGY EL" _
         & "ECTRONHz ELECTRONMASS ELECTRONMASS_ERR ELECTRONMAS"
   desc_string = desc_string _
         & "SA ELECTRONMASSA_ERR ELECTRONVOLT ELECTRONWAVELENG" _
         & "TH ELEMENTARYCHARGE ELEMENTARYCHARGE_ERR ELEVATION" _
         & "MAX ELEVATIONMIN ENERGY EPSILON0 EPSILON0_ERR ERG " _
         & "EULER EUVENERGY EUVHz EUVWAVELENGTH EUVWAVENUMBER "
   desc_string = desc_string _
         & "EXA EXBI FARAD FARADAY FARADAY_ERR FARADAYCONST FA" _
         & "RADAYCONST_ERR FEET FEIGENBAUM FEMTO FINESTRUCTURE" _
         & " FINESTRUCTURE_ERR FINESTRUCTURE_REC FINESTRUCTURE" _
         & "_REC_ERR FINESTRUCTURECONST FINESTRUCTURECONST_ERR"
   desc_string = desc_string _
         & " FIRENERGY FIRHz FIRWAVELENGTH FIRWAVENUMBER FLICK" _
         & " FLOZ FLTEPS FLTMAN FLTMAX FLTSIG FOE FOOT FORCE F" _
         & "REQUENCY FT G G_ERR GALACTICCENTER GALLON GAMMAENE" _
         & "RGY GAMMAHz GAMMAMIN GAMMAWAVELENGTH GAMMAWAVENUMB"
   desc_string = desc_string _
         & "ER GAS GAS_ERR GASCONST GASCONST_ERR GIANTIMPACT G" _
         & "IBI GIGA GOLFBALLMASS GOLFBALLRADIUS GRAD GRAIN GR" _
         & "AM GRAVITATION GRAVITATION_ERR GRAVITATIONCONST GR" _
         & "AVITATIONCONST_ERR GRAVITATIONIERS GRAVITATIONNASA"
   desc_string = desc_string _
         & " GRAVITY GRAVITY_ERR GREENENERGY GREENHz GREENWAVE" _
         & "LENGTH GREENWAVENUMBER GREGORIANYEAR GYEAR h h_ERR" _
         & " h_REC HAND HART HARTLEY HBAR HBAR_ERR HECTARE HEC" _
         & "TO HELION_MASS HELIONENERGY HELIONHz HELIONMASS HE"
   desc_string = desc_string _
         & "LIONMASS_ERR HELIONWAVELENGTH HENRY HERTZ HFENERGY" _
         & " HFHz HFWAVELENGTH HFWAVENUMBER HI HI_ERR HIENERGY" _
         & " HIHz HIWAVELENGTH HIWAVENUMBER HORSE HORSEPOWER H" _
         & "OUR HP HPMETRIC HUBBLE HUBBLE_ERR HUBBLECONST HUBB"
   desc_string = desc_string _
         & "LECONST_ERR HUBBLERADIUS HXENERGY HXHz HXWAVELENGT" _
         & "H HXWAVENUMBER HYDROGENLINE HYDROGENLINE_ERR HYDRO" _
         & "GENRADIUS HYDROGENRADIUS_ERR INCH INDUCTANCE INHG " _
         & "INTMAX J2K J2KDAY JC JCENTURY JD JDAY JDSEC_AT_0GR"
   desc_string = desc_string _
         & "EGORIAN JDSEC_AT_0J2K JDSEC_AT_0KWT JDSEC_AT_0UET " _
         & "JEWISH_FIRST_MONTH JM JMILLENNIUM JOSEPHSON JOSEPH" _
         & "SON_ERR JOULE JULIANCENTURY JULIANDAY JULIANMILLEN" _
         & "NIUM JULIANYEAR JY JYEAR K_AT_0TEMPC K_AT_0TEMPF K"
   desc_string = desc_string _
         & "_AT_0TEMPR kB kB_ERR kB_REC KCD KCD_ERR KELVIN KG " _
         & "KGENERGY KGF kgfm_PER_lbfin KGHz KGWAVELENGTH KGWA" _
         & "VENUMBER KIBI KILO KILOGRAM KPGBOUNDARY KWT LB LBF" _
         & " lbfin lbfinrpm_PER_Hp LC_CIRCLE_RADIUS LC_CIRCLE_"
   desc_string = desc_string _
         & "X LC_CIRCLE_Y LC_ELLIPTIC_EPS LC_EPS LC_GD_COUNT L" _
         & "C_GD_DAY LC_GD_DOW LC_GD_HOUR LC_GD_JD LC_GD_MIN L" _
         & "C_GD_MONTH LC_GD_NUM LC_GD_SEC LC_GD_YEAR LC_HALAQ" _
         & "IM LC_HALAQIM_PER_HOUR LC_KNUTH_ARRAY_COUNT LC_KNU"
   desc_string = desc_string _
         & "TH_KK_COUNT LC_KNUTH_LL LC_KNUTH_MAX LC_KNUTH_QUAL" _
         & "ITY_COUNT LC_KNUTH_TT LC_MAGIC_INT LC_MAGIC_INT_NO" _
         & "T LC_MAX_FACTORS LC_PARABOLA_COEFF_A LC_PARABOLA_C" _
         & "OEFF_B LC_PARABOLA_COEFF_C LC_PARABOLA_REAL_ROOTS "
   desc_string = desc_string _
         & "LC_PARABOLA_ROOT_A LC_PARABOLA_ROOT_B LC_PARABOLA_" _
         & "X_AXIS LC_PARABOLA_Y_EXTREMA LC_SHC_SHIFTSHIFT LC_" _
         & "SHC_STEPSHIFT LC_STTS_COUNT LC_STTS_MAGIC LC_STTS_" _
         & "MAX LC_STTS_MAX_KEY LC_STTS_MEAN LC_STTS_MIN LC_ST"
   desc_string = desc_string _
         & "TS_MSQ LC_STTS_NUM LC_STTSC_COUNT LC_STTSC_COVAR L" _
         & "C_STTSC_MAGIC LC_STTSC_XX LC_STTSC_YY LC_YEAR_PER_" _
         & "METONIC LENGTH LFENERGY LFHz LFWAVELENGTH LFWAVENU" _
         & "MBER LIFEAGE LIGHT LIGHT_ERR LIGHT_REC LIGHTCONST "
   desc_string = desc_string _
         & "LIGHTCONST_ERR LIGHTYEAR LITER LITRE LNTEN LNTWO L" _
         & "UMEN LUX MACH MAGNETIC MAGNETIC_ERR MAGNETICFLUX M" _
         & "AGNETICINDUCTION MARATHON MASS MEBI MEGA METER MET" _
         & "RE MFENERGY MFHz MFWAVELENGTH MFWAVENUMBER MICRO M"
   desc_string = desc_string _
         & "ICROWAVEENERGY MICROWAVEHz MICROWAVEWAVELENGTH MIC" _
         & "ROWAVEWAVENUMBER MILE MILKYWAYAGE MILKYWAYDAY MILK" _
         & "YWAYMASS MILKYWAYRADIUS MILLI MINUTE MIRENERGY MIR" _
         & "Hz MIRWAVELENGTH MIRWAVENUMBER MM MMHG MOLARMASS M"
   desc_string = desc_string _
         & "OLARMASS_ERR MOLARMASSCONST MOLARMASSCONST_ERR MOL" _
         & "ARMASSX MOLARMASSX_ERR MOLE MONTHS_IN_YEAR MOONMON" _
         & "TH MPH MPS MPSS MU0 MU0_ERR MUON_MASS MUONENERGY M" _
         & "UONHz MUONMASS MUONMASS_ERR MUONWAVELENGTH NANO NA"
   desc_string = desc_string _
         & "T NEUTRON_MASS NEUTRONENERGY NEUTRONHz NEUTRONMASS" _
         & " NEUTRONMASS_ERR NEUTRONWAVELENGTH NEWTON NICKELMA" _
         & "SS NICKELRADIUS NICKELTHICKNESS NINEVOLT NINEVOLTB" _
         & "ATTERY NINEVOLTBATTERYCHARGE NINEVOLTBATTERYENERGY"
   desc_string = desc_string _
         & " NINEVOLTBATTERYVOLTAGE NINEVOLTCHARGE NINEVOLTENE" _
         & "RGY NINEVOLTVOLTAGE NIRENERGY NIRHz NIRWAVELENGTH " _
         & "NIRWAVENUMBER Nm_PER_lbfin NPBOUNDARY NUVENERGY NU" _
         & "VHz NUVWAVELENGTH NUVWAVENUMBER OHM ORANGEENERGY O"
   desc_string = desc_string _
         & "RANGEHz ORANGEWAVELENGTH ORANGEWAVENUMBER OUNCE OZ" _
         & " PA PARSEC PEBI PENNYMASS PENNYRADIUS PENNYTHICKNE" _
         & "SS PETA PHI PHONEBATTERY PHONEBATTERYCHARGE PHONEB" _
         & "ATTERYENERGY PHONEBATTERYVOLTAGE PI PICO PINGPONGB"
   desc_string = desc_string _
         & "ALLMASS PINGPONGBALLRADIUS PINT PLANCK PLANCK_ERR " _
         & "PLANCK_REC PLANCKBAR PLANCKBAR_ERR PLANCKCHARGE PL" _
         & "ANCKCONST PLANCKCONST_ERR PLANCKENERGY PLANCKLENGT" _
         & "H PLANCKMASS PLANCKTEMP PLANCKTIME POINT POUND POW"
   desc_string = desc_string _
         & "ER PPB PPM PPQ PPT PRESSURE PROTON_MASS PROTONENER" _
         & "GY PROTONHz PROTONMASS PROTONMASS_ERR PROTONRADIUS" _
         & " PROTONRADIUS_ERR PROTONWAVELENGTH PROXIMACENTAURI" _
         & " PSI PTRBOUNDARY QUART QUARTERMASS QUARTERRADIUS Q"
   desc_string = desc_string _
         & "UARTERTHICKNESS RAD RADIAN REDENERGY REDHz REDWAVE" _
         & "LENGTH REDWAVENUMBER RESISTANCE RPH RPM RPS RYDBER" _
         & "G RYDBERG_ERR RYDBERGCONST RYDBERGCONST_ERR RYDBER" _
         & "GENERGY RYDBERGHz RYDBERGWAVELENGTH RYDBERGWAVENUM"
   desc_string = desc_string _
         & "BER SECOND SHANNON SHFENERGY SHFHz SHFWAVELENGTH S" _
         & "HFWAVENUMBER SI16MAX SI16MIN SI32MAX SI32MIN SIEME" _
         & "NS SOCCERBALLMASS SOCCERBALLRADIUS SOFTBALLMASS SO" _
         & "FTBALLRADIUS SOLARCONSTANT SOLID_ANGLE SPECIFICHEA"
   desc_string = desc_string _
         & "T SPEED SPHERESURFACE SQDEG SQRTTWO SQUAREDEGREE S" _
         & "TANDARDATMOSPHERE STANDARDGRAVITY STEFAN STEFAN_ER" _
         & "R STEFANCONST STEFANCONST_ERR STERADIAN SXENERGY S" _
         & "XHz SXWAVELENGTH SXWAVENUMBER SXXENERGY SXXHz SXXW"
   desc_string = desc_string _
         & "AVELENGTH SXXWAVENUMBER TAU TAU_MASS TAUENERGY TAU" _
         & "Hz TAUMASS TAUMASS_ERR TAUWAVELENGTH TBLSP TEBI TE" _
         & "MPERATURE TENNISBALLMASS TENNISBALLRADIUS TERA TES" _
         & "LA THFENERGY THFHz THFWAVELENGTH THFWAVENUMBER TIM"
   desc_string = desc_string _
         & "E TON TONNE TORR TRITON_MASS TRITONENERGY TRITONHz" _
         & " TRITONMASS TRITONMASS_ERR TRITONWAVELENGTH TRJBOU" _
         & "NDARY TSP TT_AT_0TAI TT_AT_0UT1 TURN UAMU UAMU_ERR" _
         & " UET UHFENERGY UHFHz UHFWAVELENGTH UHFWAVENUMBER U"
   desc_string = desc_string _
         & "I16MAX UI32MAX UNITGM UNIVERSEAGE UNIVERSEMASS UNI" _
         & "VERSERADIUS UPQUARK_MASS UPQUARKENERGY UPQUARKHz U" _
         & "PQUARKMASS UPQUARKMASS_ERR UPQUARKWAVELENGTH UVAEN" _
         & "ERGY UVAHz UVAWAVELENGTH UVAWAVENUMBER UVBENERGY U"
   desc_string = desc_string _
         & "VBHz UVBWAVELENGTH UVBWAVENUMBER UVCENERGY UVCHz U" _
         & "VCWAVELENGTH UVCWAVENUMBER VHFENERGY VHFHz VHFWAVE" _
         & "LENGTH VHFWAVENUMBER VIOLETENERGY VIOLETHz VIOLETW" _
         & "AVELENGTH VIOLETWAVENUMBER VISCOSITY VOLT VOLTAGE "
   desc_string = desc_string _
         & "VOLUME VONKLITZING VONKLITZING_ERR WATT WEBER WEEK" _
         & " YARD YELLOWENERGY YELLOWGREENENERGY YELLOWGREENHz" _
         & " YELLOWGREENWAVELENGTH YELLOWGREENWAVENUMBER YELLO" _
         & "WHz YELLOWWAVELENGTH YELLOWWAVENUMBER YOBI YOCTO Y" _
         & "OTTA ZEBI ZEPTO ZETTA" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "0 bessY1 bessY1__1 bharea_to_mass bhdensity_to_mas" _
         & "s bhentropy_to_mass bhlifetime_to_mass bhmass_to_a" _
         & "rea bhmass_to_density bhmass_to_entropy bhmass_to_"
   desc_string = desc_string _
         & "lifetime bhmass_to_power bhmass_to_radius bhmass_t" _
         & "o_temp bhpower_to_mass bhradius_to_mass bhtemp_to_" _
         & "mass bits_count bits_ls1b bits_ls1bpos bits_ms1b b" _
         & "its_ms1bpos bitwisenot blackbodyf_pdf blackbodyl_p"
   desc_string = desc_string _
         & "df blackbodymode blackbodypower cantor carmichael " _
         & "catalan cauchy_rand cbrt ceil chisqr_rand circle_g" _
         & "et clip clockhour clockmin clocksec clockstdev clo" _
         & "cksum cnv_dbl2si16 contfra_print contfrac_print co"
   desc_string = desc_string _
         & "ntfracd_print corr_delete corr_handlex corr_handle" _
         & "y corrcorr corrslope corrstderr corry0 cos cos__1 " _
         & "cosd cosdinv cosh coshinv cosintegral cosinv cosm1" _
         & " cosm1__1 cost costinv cot cotd cotdinv coth cothi"
   desc_string = desc_string _
         & "nv cotinv cott cottinv csc cscd cscdinv csch cschi" _
         & "nv cscinv csct csctinv ctz cu cube cuberoot date_e" _
         & "aster date_pesach date_rosh_hashanah day2dhms day2" _
         & "dhmsinv day2hour deg2dms deg2rad deg2sec deg2tur d"
   desc_string = desc_string _
         & "igamma digamma__1 digammainv dms2deg dms2rad dooms" _
         & "day ecc2flat Ein elliptic1a elliptic1k elliptic1m " _
         & "elliptic1m__1 elliptic1m__2 elliptic2a elliptic2k " _
         & "elliptic2m elliptic2m__1 ellipticE ellipticK erf e"
   desc_string = desc_string _
         & "rf__1 erf__2 erf__3 erf__4 erfc erfcinv erfcx erfc" _
         & "xinv erfinv eta eulerphi eulerx exp exp__1 exp__2 " _
         & "exp_rand expintegral1 expintegrali expm1 fac facin" _
         & "v factor_print factorial factorialdouble factorial"
   desc_string = desc_string _
         & "inv fibo fibox fiboxinv flat2ecc floor frac fubini" _
         & " fusc g2jewish gamma_rand gau_cdf gau_pdf gau_quan" _
         & "tile gd gdinv geo_rand ghmean1 ghmean1inv glmean1 " _
         & "gmean1 gqmean1 has_primitiveroot haversin heavisid"
   desc_string = desc_string _
         & "e heronianmean1 hlmean1 hmean1 hn hn__1 hour2day h" _
         & "qmean1 hypot1 hypot1inv id int_rand iround iseven " _
         & "isint isjewish8short isjewish9short isjewishleap i" _
         & "sleapyear ismagic isneg isnegint isodd ispos ispos"
   desc_string = desc_string _
         & "int ispractical isprime issquarefree iszero j2k2do" _
         & "w j2k2jd j2k2kwt j2k2uet j2k2ymdhms jacobicir jd26" _
         & "4_print jd2dow jd2j2k jd2kwt jd2uet jd2ymdhms jewi" _
         & "sh2g jewish2jd jewish2jdx jewish_months_in_year je"
   desc_string = desc_string _
         & "wish_yearlength k2a k2m k2tempc k2tempf k2tempr ks" _
         & "_a_cdf kwt2j2k kwt2jd kwt2uet l10 lg lgamma lgamma" _
         & "__1 lgamma__2 lgammainv lix lixinv lmean1 ln ln1p " _
         & "ln__1 ln__2 lns log log10 log2 logintegral logisti"
   desc_string = desc_string _
         & "c_rand lqmean1 lucas lucasx m2a m2b m2k mertens mi" _
         & "nkowski mobius n2primish neg nom normal_cdf normal" _
         & "_cdf__1 normal_pdf normal_quantile normal_quantile" _
         & "__1 num2char_print parabola_get parabola_roots par"
   desc_string = desc_string _
         & "tition partitionq pcf pfg pfl poi_rand pow10 pow2 " _
         & "prime0 prime1 primecount primecountx primenext pri" _
         & "menth primenth__1 primeprev primitiveroot qmean1 r" _
         & "ad2deg rad2dms rad2dms2 rad2sec rad2tur rand_init "
   desc_string = desc_string _
         & "randl reci reimann round rto125 rtoi rtoz sba sear" _
         & "ch_check search_search sec sec2deg sec2dhms sec2ra" _
         & "d sec2tur secd secdinv sech sechinv secinv sect se" _
         & "ctinv shc_shft2steps shc_steps2shft shc_steps2shft"
   desc_string = desc_string _
         & "inv sigma0 sigma1 sigmoid sigmoidinv sign sin sin_" _
         & "_1 sinc sinc__1 sincc sincinv sind sindinv sinh si" _
         & "nhinv sinintegral sininv sint sintinv smooth sq sq" _
         & "inv sqrt sqrt__1 sqrt__2 sqrt__3 sqrtx square squa"
   desc_string = desc_string _
         & "reroot stair stts_delete stts_reset sttscount stts" _
         & "magic sttsmax sttsmean sttsmin sttsmsq sttsspread " _
         & "sttsstdev sttsstdevmean sttssum sttsvar supercatal" _
         & "an tan tand tandinv tanh tanhinv taninv tant tanti"
   desc_string = desc_string _
         & "nv tempc tempc2k tempc2tempf tempf tempf2k tempf2t" _
         & "empc tempr tempr2k tgamma tgamma__1 tgamma__2 tgam" _
         & "ma__3 tgamma__stirling tgammadouble tgammainv topy" _
         & "h1 totient tri tri_rand triinv trip trunc tur2deg " _
         & "tur2rad tur2sec uet2j2k uet2jd uet2kwt uni_rand ve" _
         & "rsin wm wminv wp wpinv ymdhms2j2k ymdhms2jd ymdhms" _
         & "_get zag zeta zig zigzag" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "uantile ymd2dow ymd2doy ymd2j2k ymd2j2k__1 ymd2jd " _
         & "zipf_cdf zipf_pdf zipf_quantile" _
         & vbNewLine
   desc_string = desc_string _
...
Public Function kw_desc() As String
   ...
   desc_string = desc_string _
         ...
         & "CRE ACTION AH ALPHA(|_(ERR|MASS|REC(_ERR)?)|ENERGY"
   desc_string = desc_string _
         & "|Hz|MASS(_ERR)?|WAVELENGTH) AMP(ERE)? AMU(_ERR)? A" _
         & "NGLE APERY ARC(DEG(REE)?|MIN(UTE)?|SEC(OND)?) AREA" _
         & "? ASTRONOMICALUNIT ATM(OSPHERE(_ERR)?)? ATOMICMASS" _
         & "UNIT(_ERR)? ATTO AU AU_ERR AVOGADRO(|_(ERR|REC)|CO"
   desc_string = desc_string _
         & "NST(_ERR)?) BAN BAR(N|REL)? BAS(EBALL(MASS|RADIUS)" _
         & "|KETBALL(MASS|RADIUS)) BEKENSTEIN(ENTROPY|INFORMAT" _
         & "ION) BHENTROPY_AREA BHMASS(2(_(AREA|DENSITY|ENTROP" _
         & "Y)|POWER)|3_LIFETIME|_RADIUS|TEMP) BHRADIUS2DENSIT"
   desc_string = desc_string _
         & "Y BIT BLUE(ENERGY|Hz|WAVE(LENGTH|NUMBER)) BOHRRADI" _
         & "US(_ERR)? BOLTZMANN(|_(ERR|REC)|CONST(_ERR)?) BTU(" _
         & "_(IT|TH))? CAL(_(IT|TH)|ORIE)? CANDELA CAPACITANCE" _
         & " CAR(AT|BATTERY(CHARGE|ENERGY|VOLTAGE)?) CBATTERY("
   desc_string = desc_string _
         & "CHARGE|ENERGY|VOLTAGE)? CENTI CHARGE CONDUCTANCE C" _
         & "OULOMB(CONST(_ERR)?)? CUP CURRENT DALTON(_ERR)? DB" _
         & "ATTERY(CHARGE|ENERGY|VOLTAGE)? DBL(EPS|MA(N|X(LN)?" _
         & ")|SIG) DEC[AI] DEG[CFR] DENSITY DEUTERON(_MASS|ENE"
   desc_string = desc_string _
         & "RGY|Hz|MASS(_ERR)?|WAVELENGTH) DIME(MAS|RADIU|THIC" _
         & "KNES)S DISTANCE DIT DOWNQUARK(_MASS|ENERGY|Hz|MASS" _
         & "(_ERR)?|WAVELENGTH) dvCs(_ERR|ENERGY|Hz(_REC)?|WAV" _
         & "E(LENGTH|NUMBER)) E EARTH(AGE|GRAVITY[AB]|SOLARDAY"
   desc_string = desc_string _
         & ") EHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) EINSTEIN(_ERR" _
         & ")? ELE(CTR(IC(_ERR|CONST(_ERR)?)?|ON(_MASS|CHARGE(" _
         & "_(ERR|REC))?|ENERGY|Hz|MASS(_ERR|A(_ERR)?)?|VOLT|W" _
         & "AVELENGTH))|MENTARYCHARGE(_ERR)?|VATIONM(AX|IN)) E"
   desc_string = desc_string _
         & "NERGY EPSILON0(_ERR)? ERG EULER EUV(ENERGY|Hz|WAVE" _
         & "(LENGTH|NUMBER)) EXA EXBI FARAD(|AY(_ERR|CONST(_ER" _
         & "R)?)?) FEET FEIGENBAUM FEMTO FINESTRUCTURE(|_(ERR|" _
         & "REC(_ERR)?)|CONST(_ERR)?) FIR(ENERGY|Hz|WAVE(LENGT"
   desc_string = desc_string _
         & "H|NUMBER)) FLICK FLOZ FLT(EPS|MA[NX]|SIG) FOE FOOT" _
         & " FORCE FREQUENCY FT G G_ERR GAL(ACTICCENTER|LON) G" _
         & "AMMA(ENERGY|Hz|MIN|WAVE(LENGTH|NUMBER)) GAS(_ERR|C" _
         & "ONST(_ERR)?)? GIANTIMPACT GIBI GIGA GOLFBALL(MASS|"
   desc_string = desc_string _
         & "RADIUS) GRA(D|IN|M|VIT(ATION(_ERR|CONST(_ERR)?|IER" _
         & "S|NASA)?|Y(_ERR)?)) GRE(EN(ENERGY|Hz|WAVE(LENGTH|N" _
         & "UMBER))|GORIANYEAR) GYEAR h h_ERR h_REC HAND HART(" _
         & "LEY)? HBAR(_ERR)? HECT(ARE|O) HELION(_MASS|ENERGY|"
   desc_string = desc_string _
         & "Hz|MASS(_ERR)?|WAVELENGTH) HENRY HERTZ HFENERGY HF" _
         & "Hz HFWAVE(LENGTH|NUMBER) HI HI_ERR HIENERGY HIHz H" _
         & "IWAVE(LENGTH|NUMBER) HORSE(POWER)? HOUR HP HPMETRI" _
         & "C HUBBLE(_ERR|CONST(_ERR)?|RADIUS)? HXENERGY HXHz "
   desc_string = desc_string _
         & "HXWAVE(LENGTH|NUMBER) HYDROGEN(LINE(_ERR)?|RADIUS(" _
         & "_ERR)?) INCH INDUCTANCE INHG INTMAX J2K(DAY)? JC J" _
         & "CENTURY JD JDAY JDSEC_AT_0(GREGORIAN|J2K|KWT|UET) " _
         & "JEWISH_FIRST_MONTH JM JMILLENNIUM JOSEPHSON(_ERR)?"
   desc_string = desc_string _
         & " JOULE JULIAN(CENTURY|DAY|MILLENNIUM|YEAR) JY JYEA" _
         & "R K_AT_0TEMP[CFR] kB kB_(ERR|REC) KCD(_ERR)? KELVI" _
         & "N KG KGENERGY KGF kgfm_PER_lbfin KGHz KGWAVE(LENGT" _
         & "H|NUMBER) KIBI KILO(GRAM)? KPGBOUNDARY KWT LB LBF "
   desc_string = desc_string _
         & "lbfin(rpm_PER_Hp)? LC_(CIRCLE_(RADIUS|[XY])|E(LLIP" _
         & "TIC_EPS|PS)|GD_(COUNT|D(AY|OW)|HOUR|JD|M(IN|ONTH)|" _
         & "NUM|SEC|YEAR)|HALAQIM(_PER_HOUR)?|KNUTH_(ARRAY_COU" _
         & "NT|KK_COUNT|LL|MAX|QUALITY_COUNT|TT)|MA(GIC_INT(_N"
   desc_string = desc_string _
         & "OT)?|X_FACTORS)|PARABOLA_(COEFF_[ABC]|R(EAL_ROOTS|" _
         & "OOT_[AB])|X_AXIS|Y_EXTREMA)|S(HC_S(HIFTSHIFT|TEPSH" _
         & "IFT)|TTS(_(COUNT|M(A(GIC|X(_KEY)?)|EAN|IN|SQ)|NUM)" _
         & "|C_(CO(UNT|VAR)|MAGIC|XX|YY)))|YEAR_PER_METONIC) L"
   desc_string = desc_string _
         & "ENGTH LFENERGY LFHz LFWAVE(LENGTH|NUMBER) LIFEAGE " _
         & "LIGHT(|_(ERR|REC)|CONST(_ERR)?|YEAR) LIT(ER|RE) LN" _
         & "T(EN|WO) LUMEN LUX MACH MAGNETIC(_ERR|FLUX|INDUCTI" _
         & "ON)? MARATHON MASS MEBI MEGA MET(ER|RE) MFENERGY M"
   desc_string = desc_string _
         & "FHz MFWAVE(LENGTH|NUMBER) MICRO(|WAVE(ENERGY|Hz|WA" _
         & "VE(LENGTH|NUMBER))) MIL(E|KYWAY(AGE|DAY|MASS|RADIU" _
         & "S)|LI) MINUTE MIR(ENERGY|Hz|WAVE(LENGTH|NUMBER)) M" _
         & "M MMHG MOL(ARMASS(|_ERR|CONST(_ERR)?|X(_ERR)?)|E) "
   desc_string = desc_string _
         & "MONTHS_IN_YEAR MOONMONTH MPH MPSS? MU0(_ERR)? MUON" _
         & "(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) NANO NAT " _
         & "NEUTRON(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) NE" _
         & "WTON NICKEL(MAS|RADIU|THICKNES)S NINEVOLT(BATTERY("
   desc_string = desc_string _
         & "CHARGE|ENERGY|VOLTAGE)?|CHARGE|ENERGY|VOLTAGE)? NI" _
         & "R(ENERGY|Hz|WAVE(LENGTH|NUMBER)) Nm_PER_lbfin NPBO" _
         & "UNDARY NUV(ENERGY|Hz|WAVE(LENGTH|NUMBER)) OHM ORAN" _
         & "GE(ENERGY|Hz|WAVE(LENGTH|NUMBER)) OUNCE OZ PA PARS"
   desc_string = desc_string _
         & "EC PEBI PENNY(MAS|RADIU|THICKNES)S PETA PHI PHONEB" _
         & "ATTERY(CHARGE|ENERGY|VOLTAGE)? PI PICO PIN(GPONGBA" _
         & "LL(MASS|RADIUS)|T) PLANCK(|_(ERR|REC)|BAR(_ERR)?|C" _
         & "(HARGE|ONST(_ERR)?)|ENERGY|LENGTH|MASS|T(EMP|IME))"
   desc_string = desc_string _
         & " POINT POUND POWER PPB PPM PPQ PPT PRESSURE PRO(TO" _
         & "N(_MASS|ENERGY|Hz|MASS(_ERR)?|RADIUS(_ERR)?|WAVELE" _
         & "NGTH)|XIMACENTAURI) PSI PTRBOUNDARY QUART(ER(MAS|R" _
         & "ADIU|THICKNES)S)? RAD(IAN)? RED(ENERGY|Hz|WAVE(LEN"
   desc_string = desc_string _
         & "GTH|NUMBER)) RESISTANCE RPH RPM RPS RYDBERG(|_ERR|" _
         & "CONST(_ERR)?|ENERGY|Hz|WAVE(LENGTH|NUMBER)) SECOND" _
         & " SHANNON SHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) SI16M(" _
         & "AX|IN) SI32M(AX|IN) SIEMENS SOCCERBALL(MASS|RADIUS"
   desc_string = desc_string _
         & ") SOFTBALL(MASS|RADIUS) SOL(ARCONSTANT|ID_ANGLE) S" _
         & "PE(CIFICHEAT|ED) SPHERESURFACE SQDEG SQRTTWO SQUAR" _
         & "EDEGREE STANDARD(ATMOSPHERE|GRAVITY) STE(FAN(_ERR|" _
         & "CONST(_ERR)?)?|RADIAN) SXENERGY SXHz SXWAVE(LENGTH"
   desc_string = desc_string _
         & "|NUMBER) SXX(ENERGY|Hz|WAVE(LENGTH|NUMBER)) TAU(_M" _
         & "ASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH)? TBLSP TEBI " _
         & "TEMPERATURE TENNISBALL(MASS|RADIUS) TERA TESLA THF" _
         & "(ENERGY|Hz|WAVE(LENGTH|NUMBER)) TIME TON(NE)? TORR"
   desc_string = desc_string _
         & " TRITON(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) TR" _
         & "JBOUNDARY TSP TT_AT_0(TAI|UT1) TURN UAMU(_ERR)? UE" _
         & "T UHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) UI16MAX UI32M" _
         & "AX UNI(TGM|VERSE(AGE|MASS|RADIUS)) UPQUARK(_MASS|E"
   desc_string = desc_string _
         & "NERGY|Hz|MASS(_ERR)?|WAVELENGTH) UVA(ENERGY|Hz|WAV" _
         & "E(LENGTH|NUMBER)) UVB(ENERGY|Hz|WAVE(LENGTH|NUMBER" _
         & ")) UVC(ENERGY|Hz|WAVE(LENGTH|NUMBER)) VHF(ENERGY|H" _
         & "z|WAVE(LENGTH|NUMBER)) VIOLET(ENERGY|Hz|WAVE(LENGT"
   desc_string = desc_string _
         & "H|NUMBER)) VISCOSITY VOL(T(AGE)?|UME) VONKLITZING(" _
         & "_ERR)? WATT WEBER WEEK YARD YELLOW(ENERGY|GREEN(EN" _
         & "ERGY|Hz|WAVE(LENGTH|NUMBER))|Hz|WAVE(LENGTH|NUMBER" _
   ...
   desc_string = desc_string _
         & "Y(0|1(__1)?)) bharea_to_mass bhdensity_to_mass bhe" _
         & "ntropy_to_mass bhlifetime_to_mass bhmass_to_(area|" _
         & "density|entropy|lifetime|power|radius|temp) bhpowe" _
         & "r_to_mass bhradius_to_mass bhtemp_to_mass bit(s_(c"
   desc_string = desc_string _
         & "ount|ls1b(pos)?|ms1b(pos)?)|wisenot) blackbody(f_p" _
         & "df|l_pdf|mode|power) cantor carmichael catalan cau" _
         & "chy_rand cbrt ceil chisqr_rand circle_get clip clo" _
         & "ck(hour|min|s(ec|tdev|um)) cnv_dbl2si16 contfra(_p"
   desc_string = desc_string _
         & "rint|c(_print|d_print)) corr(_(delete|handle[xy])|" _
         & "corr|s(lope|tderr)|y0) cos(|__1|d(inv)?|h(inv)?|in" _
         & "(tegral|v)|m1(__1)?|t(inv)?) cot([dht]?(inv)?)? cs" _
         & "c([dht]?(inv)?)? ctz cu cube(root)? date_(easter|p"
   desc_string = desc_string _
         & "esach|rosh_hashanah) day2(dhms(inv)?|hour) deg2(dm" _
         & "s|rad|sec|tur) digamma(__1|inv)? dms2(deg|rad) doo" _
         & "msday ecc2flat Ein elliptic(1([ak]|m(__[12])?)|2([" _
         & "ak]|m(__1)?)|[EK]) erf(|__([12]|[34])|c(inv|x(inv)"
   desc_string = desc_string _
         & "?)?|inv) eta euler(phi|x) exp(_(_[12]|rand)|integr" _
         & "al[1i]|m1)? fac(|inv|tor(_print|ial(double|inv)?))" _
         & " fibo(x(inv)?)? flat2ecc floor frac fubini fusc g2" _
         & "jewish gamma_rand gau_(cdf|pdf|quantile) gd gdinv "
   desc_string = desc_string _
         & "geo_rand ghmean1(inv)? glmean1 gmean1 gqmean1 has_" _
         & "primitiveroot haversin heaviside heronianmean1 hlm" _
         & "ean1 hmean1 hn hn__1 hour2day hqmean1 hypot1(inv)?" _
         & " id int_rand iround is(even|int|jewish8short|jewis"
   desc_string = desc_string _
         & "h9short|jewishleap|leapyear|magic|neg|negint|odd|p" _
         & "os|posint|practical|prime|squarefree|zero) j2k2(do" _
         & "w|jd|kwt|uet|ymdhms) jacobicir jd2(64_print|dow|j2" _
         & "k|kwt|uet|ymdhms) jewish(2(g|jdx?)|_(months_in_yea"
   desc_string = desc_string _
         & "r|yearlength)) k2a k2m k2temp[cfr] ks_a_cdf kwt2(j" _
         & "(2k|d)|uet) l10 lg lgamma(__[12]|inv)? lix(inv)? l" _
         & "mean1 ln ln1p ln__[12] lns log(10|2|i(ntegral|stic" _
         & "_rand))? lqmean1 lucasx? m2a m2b m2k mertens minko"
   desc_string = desc_string _
         & "wski mobius n2primish neg nom normal_(cdf(__1)?|pd" _
         & "f|quantile(__1)?) num2char_print par(abola_(get|ro" _
         & "ots)|titionq?) pcf pfg pfl poi_rand pow(10|2) prim" _
         & "(e([01]|countx?|n(ext|th(__1)?)|prev)|itiveroot) q"
   desc_string = desc_string _
         & "mean1 rad2(d(eg|ms2?)|sec|tur) rand(_init|l) reci " _
         & "reimann round rto(125|[iz]) sba search_(check|sear" _
         & "ch) sec(|2(d(eg|hms)|rad|tur)|[dht]?(inv)?) shc_s(" _
         & "hft2steps|teps2shft(inv)?) sig(m(a[01]|oid(inv)?)|"
   desc_string = desc_string _
         & "n) sin(|__1|c(__1|c|inv)?|d(inv)?|h(inv)?|in(tegra" _
         & "l|v)|t(inv)?) smooth sq sqinv sqrt(__[123]|x)? squ" _
         & "are(root)? stair stts(_(delete|reset)|count|m(a(gi" _
         & "c|x)|ean|in|sq)|s(pread|tdev(mean)?|um)|var) super"
   desc_string = desc_string _
         & "catalan tan([dht]?(inv)?)? temp(c(2(k|tempf))?|f(2" _
         & "(k|tempc))?|r(2k)?) tgamma(__([123]|stirling)|doub" _
         & "le|inv)? topyh1 totient tri(_rand|inv|p)? trunc tu" _
         & "r2(deg|rad|sec) uet2(j(2k|d)|kwt) uni_rand versin " _
         & "wm(inv)? wp(inv)? ymdhms(2j(2k|d)|_get) zag zeta z" _
         & "ig(zag)?" _
         & vbNewLine
   desc_string = desc_string _
   ...
   desc_string = desc_string _
         ...
         & "[wy]|j(2k(__1)?|d)) zipf_(cdf|pdf|quantile)" _
         & vbNewLine
   desc_string = desc_string _
         & "   Kw 4-ary Functions: carlsonRJ f21 great_circle_" _
         & "distance n2comb sphere_d" _
