...
# 2020-08-03 kdw  For Changelog, See File varylog
import sys
import math
import time
KILOGRAM = 1.0
...
def kw_description():
   ...
   " doomsday ecc2flat Ein elliptic1 elliptic1__1 elliptic1_" + \
   "_2 elliptic2 elliptic2__1 ellipticE ellipticK erf erf__1" + \
   " erf__2 erf__3 erf__4 erfc erfcinv erfcx erfcxinv erfinv" + \
   " eta eulerphi eulerx exp exp__1 exp__2 exp_rand expinteg" + \
   "ral1 expintegrali expm1 fac facinv factorial factorialdo" + \
   "uble factorialinv fibo fibox fiboxinv flat2ecc floor fra" + \
   "c fubini fusc g2jewish gamma_rand gau_cdf gau_pdf gau_qu" + \
   "antile gd gdinv geo_rand ghmean1 ghmean1inv glmean1 gmea" + \
   "n1 gqmean1 has_primitiveroot haversin heaviside heronian" + \
   "mean1 hlmean1 hmean1 hn hn__1 hour2day hqmean1 hypot1 hy" + \
   "pot1inv id int_rand iround iseven isint isjewish8short i" + \
   "sjewish9short isjewishleap isleapyear ismagic isneg isne" + \
   "gint isodd ispos isposint ispractical isprime issquarefr" + \
   "ee iszero j2k2dow j2k2jd j2k2kwt j2k2uet j2k2ymdhms jaco" + \
   "bicir jd2dow jd2j2k jd2kwt jd2uet jd2ymdhms jewish2g jew" + \
   "ish2jd jewish2jdx jewish_months_in_year jewish_yearlengt" + \
   "h k2m k2tempc k2tempf k2tempr ks_a_cdf kwt2j2k kwt2jd kw" + \
   "t2uet l10 lg lgamma lgamma__1 lgamma__2 lgammainv lix li" + \
   "xinv lmean1 ln ln1p ln__1 ln__2 lns log log10 log2 login" + \
   "tegral logistic_rand lqmean1 lucas lucasx m2b m2k merten" + \
   "s minkowski mobius neg nom normal_cdf normal_cdf__1 norm" + \
   "al_pdf normal_quantile normal_quantile__1 parabola_get p" + \
   "arabola_roots partition partitionq pcf pfg pfl poi_rand " + \
   "pow10 pow2 prime0 prime1 primecount primecountx primenex" + \
   "t primenth primenth__1 primeprev primitiveroot print_con" + \
   "tfra print_contfrac print_contfracd print_factor print_j" + \
   "d264 print_num2char qmean1 rad2deg rad2dms rad2dms2 rad2" + \
   "sec rad2tur rand_init randl reci reimann round rto125 rt" + \
   "oi rtoz sba search_check search_search sec sec2deg sec2d" + \
   "hms sec2rad sec2tur secd secdinv sech sechinv secinv sec" + \
   "t sectinv shc_shft2steps shc_steps2shft shc_steps2shftin" + \
   "v sigma0 sigma1 sigmoid sigmoidinv sign sin sin__1 sinc " + \
   "sinc__1 sincc sincinv sind sindinv sinh sinhinv sininteg" + \
   "ral sininv sint sintinv smooth sq sqinv sqrt sqrt__1 sqr" + \
   "t__2 sqrt__3 sqrtx square squareroot stair stts_delete s" + \
   "tts_reset sttscount sttsmagic sttsmax sttsmean sttsmin s" + \
   "ttsmsq sttsspread sttsstdev sttsstdevmean sttssum sttsva" + \
   "r supercatalan tan tand tandinv tanh tanhinv taninv tant" + \
   " tantinv tempc tempc2k tempc2tempf tempf tempf2k tempf2t" + \
   "empc tempr tempr2k tgamma tgamma__1 tgamma__2 tgamma__3 " + \
   "tgamma__stirling tgammadouble tgammainv topyh1 totient t" + \
   "ri tri_rand triinv trip trunc tur2deg tur2rad tur2sec ue" + \
   "t2j2k uet2jd uet2kwt uni_rand versin wm wminv wp wpinv y" + \
   "mdhms2j2k ymdhms2jd ymdhms_get zag zeta zig zigzag" + \
   "\n" + \
   ...
   " isgt isle islt ismult ismultl isneq ispractical_f ispri" + \
   "mitiveroot issignsame issquarefree_f isss jacobiphi jewi" + \
   "sh_monthbegin jewish_monthlength ks_b_cdf ks_cdf ks_quan" + \
   "tile lbeta lcam lcams lcantim lcm lcombx ligamma ligamma" + \
   "inv lmean logistic_cdf logistic_pdf logistic_quantile lp" + \
   "ermx lqmean max2 maxabs maxabs2 maxi mb_cdf mb_pdf mean " + \
   "min2 minabs minabs2 mini mobius_f mods modulo nbd_rand n" + \
   "c nd nevillethetac nevillethetad nevillethetan nevilleth" + \
   "etas ns nthroot__1 pareto_rand perm permutation permx po" + \
   "i_cdf poi_pdf poi_quantile polya_rand polycnum polynum p" + \
   "ow powi prime0_f print_base_out print_base_outf qmean ra" + \
   "nd_init_array ratio repetendlen rligamma rms round2 rtom" + \
   "sd ruigamma sc sd shc_shftpoles2steps sigma sigma0_f sig" + \
   "ma1_f sn sn__1 spread2stdev stdev2spread stirling1 stirl" + \
   "ing2 stts_copy stts_downdate stts_newx stts_scale stts_s" + \
   "hift stts_update stts_update2 t_cdf t_cdf__1 t_pdf t_qua" + \
   "ntile t_quantile__1 tetracnum tetranum theta1 theta2 the" + \
   "ta3 theta4 Tn topyh totient_f trap_rand tri_cdf tri_pdf " + \
   "tri_quantile uigamma uigamma__1 uigammainv Un uni_cdf un" + \
   "i_pdf uni_quantile variance zeta_cdf zeta_pdf zipf_rand " + \
   "znorder znorder__1" + \
   "\n" + \
...
def kw_desc():
   ...
   "2(deg|rad) doomsday ecc2flat Ein elliptic(1(__[12])?|2(_" + \
   "_1)?|[EK]) erf(|__([12]|[34])|c(inv|x(inv)?)?|inv) eta e" + \
   "uler(phi|x) exp(_(_[12]|rand)|integral[1i]|m1)? fac(inv|" + \
   "torial(double|inv)?)? fibo(x(inv)?)? flat2ecc floor frac" + \
   " fubini fusc g2jewish gamma_rand gau_(cdf|pdf|quantile) " + \
   "gd gdinv geo_rand ghmean1(inv)? glmean1 gmean1 gqmean1 h" + \
   "as_primitiveroot haversin heaviside heronianmean1 hlmean" + \
   "1 hmean1 hn hn__1 hour2day hqmean1 hypot1(inv)? id int_r" + \
   "and iround is(even|int|jewish8short|jewish9short|jewishl" + \
   "eap|leapyear|magic|neg|negint|odd|pos|posint|practical|p" + \
   "rime|squarefree|zero) j2k2(dow|jd|kwt|uet|ymdhms) jacobi" + \
   "cir jd2(dow|j2k|kwt|uet|ymdhms) jewish(2(g|jdx?)|_(month" + \
   "s_in_year|yearlength)) k2m k2temp[cfr] ks_a_cdf kwt2(j(2" + \
   "k|d)|uet) l10 lg lgamma(__[12]|inv)? lix(inv)? lmean1 ln" + \
   " ln1p ln__[12] lns log(10|2|i(ntegral|stic_rand))? lqmea" + \
   "n1 lucasx? m2b m2k mertens minkowski mobius neg nom norm" + \
   "al_(cdf(__1)?|pdf|quantile(__1)?) par(abola_(get|roots)|" + \
   "titionq?) pcf pfg pfl poi_rand pow(10|2) pri(m(e([01]|co" + \
   "untx?|n(ext|th(__1)?)|prev)|itiveroot)|nt_(contfra(cd?)?" + \
   "|factor|jd264|num2char)) qmean1 rad2(d(eg|ms2?)|sec|tur)" + \
   ...
   "lt|mult|multl|neq|practical_f|primitiveroot|signsame|squ" + \
   "arefree_f|ss) jacobiphi jewish_month(begin|length) ks_(b" + \
   "_cdf|cdf|quantile) lbeta lca(ms?|ntim) lcm lcombx ligamm" + \
   "a(inv)? lmean logistic_(cdf|pdf|quantile) lpermx lqmean " + \
   "max(2|abs2?|i) mb_(cdf|pdf) mean min(2|abs2?|i) mobius_f" + \
   " mod(s|ulo) nbd_rand nc nd nevilletheta([cd]|[ns]) ns nt" + \
   "hroot__1 pareto_rand perm(utation|x)? poi_(cdf|pdf|quant" + \
   "ile) poly(a_rand|cnum|num) powi? pri(me0_f|nt_base_outf?" + \
   ") qmean rand_init_array ratio repetendlen rligamma rms r" + \
   "ound2 rtomsd ruigamma sc sd shc_shftpoles2steps sigma(0_" + \
   "f|1_f)? sn sn__1 spread2stdev stdev2spread stirling[12] " + \
   "stts_(copy|downdate|newx|s(cale|hift)|update2?) t_cdf(__" + \
   "1)? t_pdf t_quantile(__1)? tetra(cnum|num) theta([12]|[3" + \
   "4]) Tn topyh totient_f trap_rand tri_(cdf|pdf|quantile) " + \
   "uigamma(__1|inv)? Un uni_(cdf|pdf|quantile) variance zet" + \
   "a_(cdf|pdf) zipf_rand znorder(__1)?" + \
   "\n" + \
