...
# 2020-10-05 kdw  For Changelog, See File varylog
import sys
import math
import time
KILOGRAM = 1.0
...
BOLTZMANN_ERR = 0
BOLTZMANNCONST = 1.380649E-23 * JOULE / KELVIN
BOLTZMANNCONST_ERR = 0
ELECTRONCHARGE = 1.602176634E-19 * COULOMB
ELECTRONCHARGE_ERR = 0
LIGHT = 2.99792458E+8 * METER / SECOND
LIGHT_ERR = 0
LIGHTCONST = 2.99792458E+8 * METER / SECOND
LIGHTCONST_ERR = 0
AVOGADRO = 6.02214076E+23 / MOLE
AVOGADRO_ERR = 0
AVOGADROCONST = 6.02214076E+23 / MOLE
AVOGADROCONST_ERR = 0
PLANCK = 6.62607015E-34 * JOULE * SECOND
PLANCK_ERR = 0
PLANCKCONST = 6.62607015E-34 * JOULE * SECOND
PLANCKCONST_ERR = 0
...
FARADAY_ERR = 0
FARADAYCONST = ELECTRONCHARGE * AVOGADRO
FARADAYCONST_ERR = 0
GAS = BOLTZMANN * AVOGADRO
GAS_ERR = 0
GASCONST = BOLTZMANN * AVOGADRO
GASCONST_ERR = 0
GRAVITATION = 6.67430E-11 * UNITGM / KILOGRAM
GRAVITATION_ERR = 2.2E-5
GRAVITATIONCONST = 6.67430E-11 * UNITGM / KILOGRAM
GRAVITATIONCONST_ERR = 2.2E-5
RYDBERG = 1.0973731568160E+7 * LIGHT / METER
RYDBERG_ERR = 1.9E-12
RYDBERGCONST = 1.0973731568160E+7 * LIGHT / METER
...
ELECTRICCONST = 1.0 / (LIGHT * LIGHT * MU0)
...
MOLARMASSCONST = 0.99999999965 * GRAM / MOLE
...
STEFAN_ERR = 0
STEFANCONST = (TAU * TAU * LIGHT * BOLTZMANN) * ((BOLTZMANN /\
      (LIGHT * HBAR)) * (BOLTZMANN / (LIGHT * HBAR)) *\
      (BOLTZMANN / (LIGHT * HBAR))) / 240.0
STEFANCONST_ERR = 0
...
HUBBLE_ERR = 6E-3
HUBBLECONST = 67.66E-3 * METER / (SECOND * PARSEC)
HUBBLECONST_ERR = 6E-3
...
def kw_description():
   ...
   "RO_ERR AVOGADRO_REC AVOGADROCONST AVOGADROCONST_ERR BAN " + \
   "BAR BARN BARREL BASEBALLMASS BASEBALLRADIUS BASKETBALLMA" + \
   "SS BASKETBALLRADIUS BEKENSTEINENTROPY BEKENSTEININFORMAT" + \
   "ION BHENTROPY_AREA BHMASS3_LIFETIME BHMASS_RADIUS BHMASS" + \
   "TEMP BHRADIUS2DENSITY BIT BLUEENERGY BLUEHz BLUEWAVELENG" + \
   "TH BLUEWAVENUMBER BOHRRADIUS BOHRRADIUS_ERR BOLTZMANN BO" + \
   "LTZMANN_ERR BOLTZMANN_REC BOLTZMANNCONST BOLTZMANNCONST_" + \
   "ERR BTU BTU_IT BTU_TH CAL CAL_IT CAL_TH CALORIE CANDELA " + \
   "CAPACITANCE CARAT CARBATTERY CARBATTERYCHARGE CARBATTERY" + \
   "ENERGY CARBATTERYVOLTAGE CBATTERY CBATTERYCHARGE CBATTER" + \
   "YENERGY CBATTERYVOLTAGE CENTI CHARGE CONDUCTANCE COULOMB" + \
   " COULOMBCONST COULOMBCONST_ERR CUP CURRENT DALTON DALTON" + \
   "_ERR DAY DBATTERY DBATTERYCHARGE DBATTERYENERGY DBATTERY" + \
   "VOLTAGE DBLEPS DBLMAN DBLMAX DBLMAXLN DBLSIG DECA DECI D" + \
   "EGC DEGF DEGR DENSITY DEUTERON_MASS DEUTERONMASS DEUTERO" + \
   "NMASS_ERR DIMEMASS DIMERADIUS DIMETHICKNESS DISTANCE DIT" + \
   " DOWNQUARK_MASS DOWNQUARKENERGY DOWNQUARKHz DOWNQUARKMAS" + \
   "S DOWNQUARKMASS_ERR DOWNQUARKWAVELENGTH dvCs_ERR dvCsENE" + \
   "RGY dvCsHz dvCsHz_REC dvCsWAVELENGTH dvCsWAVENUMBER E EA" + \
   "RTHAGE EARTHGRAVITYA EARTHGRAVITYB EARTHSOLARDAY EHFENER" + \
   "GY EHFHz EHFWAVELENGTH EHFWAVENUMBER EINSTEIN EINSTEIN_E" + \
   "RR ELECTRIC ELECTRIC_ERR ELECTRICCONST ELECTRICCONST_ERR" + \
   " ELECTRON_MASS ELECTRONCHARGE ELECTRONCHARGE_ERR ELECTRO" + \
   "NCHARGE_REC ELECTRONENERGY ELECTRONHz ELECTRONMASS ELECT" + \
   "RONMASS_ERR ELECTRONMASSA ELECTRONMASSA_ERR ELECTRONVOLT" + \
   " ELECTRONWAVELENGTH ELEMENTARYCHARGE ELEMENTARYCHARGE_ER" + \
   "R ELEVATIONMAX ELEVATIONMIN ENERGY EPSILON0 EPSILON0_ERR" + \
   " ERG EULER EUVENERGY EUVHz EUVWAVELENGTH EUVWAVENUMBER E" + \
   "XA EXBI FARAD FARADAY FARADAY_ERR FARADAYCONST FARADAYCO" + \
   "NST_ERR FEIGENBAUM FEMTO FINESTRUCTURE FINESTRUCTURE_ERR" + \
   " FINESTRUCTURE_REC FINESTRUCTURE_REC_ERR FINESTRUCTURECO" + \
   "NST FINESTRUCTURECONST_ERR FIRENERGY FIRHz FIRWAVELENGTH" + \
   " FIRWAVENUMBER FLICK FLOZ FLTEPS FLTMAN FLTMAX FLTSIG FO" + \
   "E FOOT FORCE FREQUENCY FT G G_ERR GALACTICCENTER GALLON " + \
   "GAMMAENERGY GAMMAHz GAMMAWAVELENGTH GAMMAWAVENUMBER GAS " + \
   "GAS_ERR GASCONST GASCONST_ERR GIANTIMPACT GIBI GIGA GOLF" + \
   "BALLMASS GOLFBALLRADIUS GRAD GRAIN GRAM GRAVITATION GRAV" + \
   "ITATION_ERR GRAVITATIONCONST GRAVITATIONCONST_ERR GRAVIT" + \
   "ATIONIERS GRAVITATIONNASA GRAVITY GRAVITY_ERR GREENENERG" + \
   "Y GREENHz GREENWAVELENGTH GREENWAVENUMBER GREGORIANYEAR " + \
   "GYEAR h h_ERR h_REC HALAQIM HALAQIM_PER_HOUR HART HARTLE" + \
   "Y HBAR HBAR_ERR HECTARE HECTO HELION_MASS HELIONMASS HEL" + \
   "IONMASS_ERR HENRY HERTZ HFENERGY HFHz HFWAVELENGTH HFWAV" + \
   "ENUMBER HI HI_ERR HIENERGY HIHz HIWAVELENGTH HIWAVENUMBE" + \
   "R HORSEPOWER HOUR HP HPMETRIC HUBBLE HUBBLE_ERR HUBBLECO" + \
   "NST HUBBLECONST_ERR HXENERGY HXHz HXWAVELENGTH HXWAVENUM" + \
   "BER HYDROGENLINE HYDROGENLINE_ERR HYDROGENRADIUS HYDROGE" + \
   "NRADIUS_ERR INCH INDUCTANCE INHG INTMAX J2K J2KDAY JC JC" + \
   "ENTURY JD JDAY JDSEC_AT_0GREGORIAN JDSEC_AT_0J2K JDSEC_A" + \
   "T_0KWT JDSEC_AT_0UET JEWISH_FIRST_MONTH JM JMILLENNIUM J" + \
   "OSEPHSON JOSEPHSON_ERR JOULE JULIANCENTURY JULIANDAY JUL" + \
   "IANMILLENNIUM JULIANYEAR JY JYEAR K_AT_0TEMPC K_AT_0TEMP" + \
   "F K_AT_0TEMPR kB kB_ERR kB_REC KCD KCD_ERR KELVIN KG KGE" + \
   "NERGY KGF KGHz KGWAVELENGTH KGWAVENUMBER KIBI KILO KILOG" + \
   "RAM KPGBOUNDARY KWT LB LBF LC_CIRCLE_RADIUS LC_CIRCLE_X " + \
   "LC_CIRCLE_Y LC_EPS LC_GD_COUNT LC_GD_DAY LC_GD_DOW LC_GD" + \
   "_HOUR LC_GD_JD LC_GD_MIN LC_GD_MONTH LC_GD_NUM LC_GD_SEC" + \
   " LC_GD_YEAR LC_KNUTH_ARRAY_COUNT LC_KNUTH_KK_COUNT LC_KN" + \
   "UTH_LL LC_KNUTH_MAX LC_KNUTH_QUALITY_COUNT LC_KNUTH_TT L" + \
   "C_MAGIC_INT LC_MAGIC_INT_NOT LC_MAX_FACTORS LC_PARABOLA_" + \
   "COEFF_A LC_PARABOLA_COEFF_B LC_PARABOLA_COEFF_C LC_PARAB" + \
   "OLA_REAL_ROOTS LC_PARABOLA_ROOT_A LC_PARABOLA_ROOT_B LC_" + \
   "PARABOLA_X_AXIS LC_PARABOLA_Y_EXTREMA LC_STTS_COUNT LC_S" + \
   "TTS_MAGIC LC_STTS_MAX LC_STTS_MAX_KEY LC_STTS_MEAN LC_ST" + \
   "TS_MIN LC_STTS_MSQ LC_STTS_NUM LC_STTSC_COUNT LC_STTSC_C" + \
   "OVAR LC_STTSC_MAGIC LC_STTSC_XX LC_STTSC_YY LENGTH LFENE" + \
   "RGY LFHz LFWAVELENGTH LFWAVENUMBER LIFEAGE LIGHT LIGHT_E" + \
   "RR LIGHT_REC LIGHTCONST LIGHTCONST_ERR LIGHTYEAR LITER L" + \
   "ITRE LNTEN LNTWO LUMEN LUX MACH MAGNETIC MAGNETIC_ERR MA" + \
   "GNETICFLUX MAGNETICINDUCTION MARATHON MASS MEBI MEGA MET" + \
   "ER METRE MFENERGY MFHz MFWAVELENGTH MFWAVENUMBER MICRO M" + \
   "ICROWAVEENERGY MICROWAVEHz MICROWAVEWAVELENGTH MICROWAVE" + \
   "WAVENUMBER MILE MILKYWAYAGE MILKYWAYDAY MILKYWAYMASS MIL" + \
   "KYWAYRADIUS MILLI MINUTE MIRENERGY MIRHz MIRWAVELENGTH M" + \
   "IRWAVENUMBER MM MMHG MOLARMASS MOLARMASS_ERR MOLARMASSCO" + \
   "NST MOLARMASSCONST_ERR MOLARMASSX MOLARMASSX_ERR MOLE MO" + \
   "NTHS_IN_YEAR MOONMONTH MPH MPS MPSS MU0 MU0_ERR MUON_MAS" + \
   "S MUONENERGY MUONHz MUONMASS MUONMASS_ERR MUONWAVELENGTH" + \
   " NANO NAT NEUTRON_MASS NEUTRONENERGY NEUTRONHz NEUTRONMA" + \
   "SS NEUTRONMASS_ERR NEUTRONWAVELENGTH NEWTON NICKELMASS N" + \
   "ICKELRADIUS NICKELTHICKNESS NINEVOLT NINEVOLTBATTERY NIN" + \
   "EVOLTBATTERYCHARGE NINEVOLTBATTERYENERGY NINEVOLTBATTERY" + \
   "VOLTAGE NINEVOLTCHARGE NINEVOLTENERGY NINEVOLTVOLTAGE NI" + \
   "RENERGY NIRHz NIRWAVELENGTH NIRWAVENUMBER NPBOUNDARY NUV" + \
   "ENERGY NUVHz NUVWAVELENGTH NUVWAVENUMBER OHM ORANGEENERG" + \
   "Y ORANGEHz ORANGEWAVELENGTH ORANGEWAVENUMBER OUNCE PA PA" + \
   "RSEC PEBI PENNYMASS PENNYRADIUS PENNYTHICKNESS PETA PHI " + \
   "PHONEBATTERY PHONEBATTERYCHARGE PHONEBATTERYENERGY PHONE" + \
   "BATTERYVOLTAGE PI PICO PINGPONGBALLMASS PINGPONGBALLRADI" + \
   "US PINT PLANCK PLANCK_ERR PLANCK_REC PLANCKBAR PLANCKBAR" + \
   "_ERR PLANCKCHARGE PLANCKCONST PLANCKCONST_ERR PLANCKENER" + \
   "GY PLANCKLENGTH PLANCKMASS PLANCKTEMP PLANCKTIME POINT P" + \
   "OUND POWER PPB PPM PPQ PPT PRESSURE PROTON_MASS PROTONEN" + \
   "ERGY PROTONHz PROTONMASS PROTONMASS_ERR PROTONRADIUS PRO" + \
   "TONRADIUS_ERR PROTONWAVELENGTH PROXIMACENTAURI PSI PTRBO" + \
   "UNDARY QUART QUARTERMASS QUARTERRADIUS QUARTERTHICKNESS " + \
   "RAD RADIAN REDENERGY REDHz REDWAVELENGTH REDWAVENUMBER R" + \
   "ESISTANCE RPM RPS RYDBERG RYDBERG_ERR RYDBERGCONST RYDBE" + \
   "RGCONST_ERR RYDBERGENERGY RYDBERGHz RYDBERGWAVELENGTH RY" + \
   "DBERGWAVENUMBER SECOND SHANNON SHFENERGY SHFHz SHFWAVELE" + \
   "NGTH SHFWAVENUMBER SI16MAX SI16MIN SI32MAX SI32MIN SIEME" + \
   "NS SOCCERBALLMASS SOCCERBALLRADIUS SOFTBALLMASS SOFTBALL" + \
   "RADIUS SOLARCONSTANT SOLID_ANGLE SPECIFICHEAT SPEED SPHE" + \
   "RESURFACE SQDEG SQRTTWO SQUAREDEGREE STANDARDATMOSPHERE " + \
   "STANDARDGRAVITY STEFAN STEFAN_ERR STEFANCONST STEFANCONS" + \
   "T_ERR STERADIAN SXENERGY SXHz SXWAVELENGTH SXWAVENUMBER " + \
   "SXXENERGY SXXHz SXXWAVELENGTH SXXWAVENUMBER TAU TAU_MASS" + \
   " TAUENERGY TAUHz TAUMASS TAUMASS_ERR TAUWAVELENGTH TBLSP" + \
   " TEBI TEMPERATURE TENNISBALLMASS TENNISBALLRADIUS TERA T" + \
   "ESLA THFENERGY THFHz THFWAVELENGTH THFWAVENUMBER TIME TO" + \
   "N TONNE TORR TRITON_MASS TRITONMASS TRITONMASS_ERR TRJBO" + \
   "UNDARY TSP TT_AT_0TAI TT_AT_0UT1 TURN UAMU UAMU_ERR UET " + \
   "UHFENERGY UHFHz UHFWAVELENGTH UHFWAVENUMBER UI16MAX UI32" + \
   "MAX UNITGM UNIVERSEAGE UNIVERSEMASS UNIVERSERADIUS UPQUA" + \
   "RK_MASS UPQUARKENERGY UPQUARKHz UPQUARKMASS UPQUARKMASS_" + \
   "ERR UPQUARKWAVELENGTH UVAENERGY UVAHz UVAWAVELENGTH UVAW" + \
   "AVENUMBER UVBENERGY UVBHz UVBWAVELENGTH UVBWAVENUMBER UV" + \
   "CENERGY UVCHz UVCWAVELENGTH UVCWAVENUMBER VHFENERGY VHFH" + \
   "z VHFWAVELENGTH VHFWAVENUMBER VIOLETENERGY VIOLETHz VIOL" + \
   "ETWAVELENGTH VIOLETWAVENUMBER VISCOSITY VOLT VOLTAGE VOL" + \
   "UME VONKLITZING VONKLITZING_ERR WATT WEBER WEEK YARD YEL" + \
   "LOWENERGY YELLOWGREENENERGY YELLOWGREENHz YELLOWGREENWAV" + \
   "ELENGTH YELLOWGREENWAVENUMBER YELLOWHz YELLOWWAVELENGTH " + \
   "YELLOWWAVENUMBER YOBI YOCTO YOTTA ZEBI ZEPTO ZETTA" + \
   "\n" + \
   ...
   "essJ0 bessJ1 bessK0 bessK1 bessY0 bessY1 bessY1__1 bits_" + \
   "count bits_ls1b bits_ls1bpos bits_ms1b bits_ms1bpos bitw" + \
   "isenot blackbodyf_pdf blackbodyl_pdf blackbodymode black" + \
   "bodypower cantor carmichael catalan cauchy_rand cbrt cei" + \
   "l chisqr_rand circle_get clip clockhour clockmin clockse" + \
   "c clockstdev clocksum cnv_dbl2si16 contfra_print contfra" + \
   "c_print contfracd_print corr_delete corr_handlex corr_ha" + \
   "ndley corrcorr corrslope corrstderr corry0 cos cos__1 co" + \
   "sd cosdinv cosh coshinv cosintegral cosinv cosm1 cosm1__" + \
   "1 cost costinv cot cotd cotdinv coth cothinv cotinv cott" + \
   " cottinv csc cscd cscdinv csch cschinv cscinv csct cscti" + \
   "nv ctz cu cube cuberoot date_easter date_pesach date_ros" + \
   "h_hashanah day2dhms day2dhmsinv day2hour deg2dms deg2rad" + \
   " deg2sec deg2tur digamma digamma__1 digammainv dms2deg d" + \
   "ms2rad doomsday ecc2flat Ein elliptic1a elliptic1k ellip" + \
   "tic1m elliptic1m__1 elliptic1m__2 elliptic2a elliptic2k " + \
   "elliptic2m elliptic2m__1 ellipticE ellipticK erf erf__1 " + \
   "erf__2 erf__3 erf__4 erfc erfcinv erfcx erfcxinv erfinv " + \
   "eta eulerphi eulerx exp exp__1 exp__2 exp_rand expintegr" + \
   "al1 expintegrali expm1 fac facinv factor_print factorial" + \
   " factorialdouble factorialinv fibo fibox fiboxinv flat2e" + \
   "cc floor frac fubini fusc g2jewish gamma_rand gau_cdf ga" + \
   "u_pdf gau_quantile gd gdinv geo_rand ghmean1 ghmean1inv " + \
   "glmean1 gmean1 gqmean1 has_primitiveroot haversin heavis" + \
   "ide heronianmean1 hlmean1 hmean1 hn hn__1 hour2day hqmea" + \
   "n1 hypot1 hypot1inv id int_rand iround iseven isint isje" + \
   "wish8short isjewish9short isjewishleap isleapyear ismagi" + \
   "c isneg isnegint isodd ispos isposint ispractical isprim" + \
   "e issquarefree iszero j2k2dow j2k2jd j2k2kwt j2k2uet j2k" + \
   "2ymdhms jacobicir jd264_print jd2dow jd2j2k jd2kwt jd2ue" + \
   "t jd2ymdhms jewish2g jewish2jd jewish2jdx jewish_months_" + \
   "in_year jewish_yearlength k2a k2m k2tempc k2tempf k2temp" + \
   "r ks_a_cdf kwt2j2k kwt2jd kwt2uet l10 lg lgamma lgamma__" + \
   "1 lgamma__2 lgammainv lix lixinv lmean1 ln ln1p ln__1 ln" + \
   "__2 lns log log10 log2 logintegral logistic_rand lqmean1" + \
   " lucas lucasx m2a m2b m2k mertens minkowski mobius neg n" + \
   "om normal_cdf normal_cdf__1 normal_pdf normal_quantile n" + \
   "ormal_quantile__1 num2char_print parabola_get parabola_r" + \
   "oots partition partitionq pcf pfg pfl poi_rand pow10 pow" + \
   "2 prime0 prime1 primecount primecountx primenext priment" + \
   "h primenth__1 primeprev primitiveroot qmean1 rad2deg rad" + \
   "2dms rad2dms2 rad2sec rad2tur rand_init randl reci reima" + \
   "nn round rto125 rtoi rtoz sba search_check search_search" + \
   " sec sec2deg sec2dhms sec2rad sec2tur secd secdinv sech " + \
   "sechinv secinv sect sectinv shc_shft2steps shc_steps2shf" + \
   "t shc_steps2shftinv sigma0 sigma1 sigmoid sigmoidinv sig" + \
   "n sin sin__1 sinc sinc__1 sincc sincinv sind sindinv sin" + \
   "h sinhinv sinintegral sininv sint sintinv smooth sq sqin" + \
   "v sqrt sqrt__1 sqrt__2 sqrt__3 sqrtx square squareroot s" + \
   "tair stts_delete stts_reset sttscount sttsmagic sttsmax " + \
   "sttsmean sttsmin sttsmsq sttsspread sttsstdev sttsstdevm" + \
   "ean sttssum sttsvar supercatalan tan tand tandinv tanh t" + \
   "anhinv taninv tant tantinv tempc tempc2k tempc2tempf tem" + \
   "pf tempf2k tempf2tempc tempr tempr2k tgamma tgamma__1 tg" + \
   "amma__2 tgamma__3 tgamma__stirling tgammadouble tgammain" + \
   "v topyh1 totient tri tri_rand triinv trip trunc tur2deg " + \
   "tur2rad tur2sec uet2j2k uet2jd uet2kwt uni_rand versin w" + \
   "m wminv wp wpinv ymdhms2j2k ymdhms2jd ymdhms_get zag zet" + \
   "a zig zigzag" + \
   "\n" + \
   ...
   "ntile ymd2dow ymd2doy ymd2j2k ymd2jd zipf_cdf zipf_pdf z" + \
   "ipf_quantile" + \
   "\n" + \
...
def kw_desc():
   ...
   "NIT(_ERR)? ATTO AU AU_ERR AVOGADRO(|_(ERR|REC)|CONST(_ER" + \
   "R)?) BAN BAR(N|REL)? BAS(EBALL(MASS|RADIUS)|KETBALL(MASS" + \
   "|RADIUS)) BEKENSTEIN(ENTROPY|INFORMATION) BHENTROPY_AREA" + \
   " BHMASS(3_LIFETIME|_RADIUS|TEMP) BHRADIUS2DENSITY BIT BL" + \
   "UE(ENERGY|Hz|WAVE(LENGTH|NUMBER)) BOHRRADIUS(_ERR)? BOLT" + \
   "ZMANN(|_(ERR|REC)|CONST(_ERR)?) BTU(_(IT|TH))? CAL(_(IT|" + \
   "TH)|ORIE)? CANDELA CAPACITANCE CAR(AT|BATTERY(CHARGE|ENE" + \
   "RGY|VOLTAGE)?) CBATTERY(CHARGE|ENERGY|VOLTAGE)? CENTI CH" + \
   "ARGE CONDUCTANCE COULOMB(CONST(_ERR)?)? CUP CURRENT DALT" + \
   "ON(_ERR)? DAY DBATTERY(CHARGE|ENERGY|VOLTAGE)? DBL(EPS|M" + \
   "A(N|X(LN)?)|SIG) DEC[AI] DEG[CFR] DENSITY DEUTERON(_MASS" + \
   "|MASS(_ERR)?) DIME(MAS|RADIU|THICKNES)S DISTANCE DIT DOW" + \
   "NQUARK(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) dvCs(_ERR" + \
   "|ENERGY|Hz(_REC)?|WAVE(LENGTH|NUMBER)) E EARTH(AGE|GRAVI" + \
   "TY[AB]|SOLARDAY) EHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) EINS" + \
   "TEIN(_ERR)? ELE(CTR(IC(_ERR|CONST(_ERR)?)?|ON(_MASS|CHAR" + \
   "GE(_(ERR|REC))?|ENERGY|Hz|MASS(_ERR|A(_ERR)?)?|VOLT|WAVE" + \
   "LENGTH))|MENTARYCHARGE(_ERR)?|VATIONM(AX|IN)) ENERGY EPS" + \
   "ILON0(_ERR)? ERG EULER EUV(ENERGY|Hz|WAVE(LENGTH|NUMBER)" + \
   ") EXA EXBI FARAD(|AY(_ERR|CONST(_ERR)?)?) FEIGENBAUM FEM" + \
   "TO FINESTRUCTURE(|_(ERR|REC(_ERR)?)|CONST(_ERR)?) FIR(EN" + \
   "ERGY|Hz|WAVE(LENGTH|NUMBER)) FLICK FLOZ FLT(EPS|MA[NX]|S" + \
   "IG) FOE FOOT FORCE FREQUENCY FT G G_ERR GAL(ACTICCENTER|" + \
   "LON) GAMMA(ENERGY|Hz|WAVE(LENGTH|NUMBER)) GAS(_ERR|CONST" + \
   "(_ERR)?)? GIANTIMPACT GIBI GIGA GOLFBALL(MASS|RADIUS) GR" + \
   "A(D|IN|M|VIT(ATION(_ERR|CONST(_ERR)?|IERS|NASA)?|Y(_ERR)" + \
   "?)) GRE(EN(ENERGY|Hz|WAVE(LENGTH|NUMBER))|GORIANYEAR) GY" + \
   "EAR h h_ERR h_REC HALAQIM(_PER_HOUR)? HART(LEY)? HBAR(_E" + \
   "RR)? HECT(ARE|O) HELION(_MASS|MASS(_ERR)?) HENRY HERTZ H" + \
   "FENERGY HFHz HFWAVE(LENGTH|NUMBER) HI HI_ERR HIENERGY HI" + \
   "Hz HIWAVE(LENGTH|NUMBER) HORSEPOWER HOUR HP HPMETRIC HUB" + \
   "BLE(_ERR|CONST(_ERR)?)? HXENERGY HXHz HXWAVE(LENGTH|NUMB" + \
   "ER) HYDROGEN(LINE(_ERR)?|RADIUS(_ERR)?) INCH INDUCTANCE " + \
   "INHG INTMAX J2K(DAY)? JC JCENTURY JD JDAY JDSEC_AT_0(GRE" + \
   "GORIAN|J2K|KWT|UET) JEWISH_FIRST_MONTH JM JMILLENNIUM JO" + \
   "SEPHSON(_ERR)? JOULE JULIAN(CENTURY|DAY|MILLENNIUM|YEAR)" + \
   " JY JYEAR K_AT_0TEMP[CFR] kB kB_(ERR|REC) KCD(_ERR)? KEL" + \
   "VIN KG KGENERGY KGF KGHz KGWAVE(LENGTH|NUMBER) KIBI KILO" + \
   "(GRAM)? KPGBOUNDARY KWT LB LBF LC_(CIRCLE_(RADIUS|[XY])|" + \
   "EPS|GD_(COUNT|D(AY|OW)|HOUR|JD|M(IN|ONTH)|NUM|SEC|YEAR)|" + \
   "KNUTH_(ARRAY_COUNT|KK_COUNT|LL|MAX|QUALITY_COUNT|TT)|MA(" + \
   "GIC_INT(_NOT)?|X_FACTORS)|PARABOLA_(COEFF_[ABC]|R(EAL_RO" + \
   "OTS|OOT_[AB])|X_AXIS|Y_EXTREMA)|STTS(_(COUNT|M(A(GIC|X(_" + \
   "KEY)?)|EAN|IN|SQ)|NUM)|C_(CO(UNT|VAR)|MAGIC|XX|YY))) LEN" + \
   "GTH LFENERGY LFHz LFWAVE(LENGTH|NUMBER) LIFEAGE LIGHT(|_" + \
   "(ERR|REC)|CONST(_ERR)?|YEAR) LIT(ER|RE) LNT(EN|WO) LUMEN" + \
   " LUX MACH MAGNETIC(_ERR|FLUX|INDUCTION)? MARATHON MASS M" + \
   "EBI MEGA MET(ER|RE) MFENERGY MFHz MFWAVE(LENGTH|NUMBER) " + \
   "MICRO(|WAVE(ENERGY|Hz|WAVE(LENGTH|NUMBER))) MIL(E|KYWAY(" + \
   "AGE|DAY|MASS|RADIUS)|LI) MINUTE MIR(ENERGY|Hz|WAVE(LENGT" + \
   "H|NUMBER)) MM MMHG MOL(ARMASS(|_ERR|CONST(_ERR)?|X(_ERR)" + \
   "?)|E) MONTHS_IN_YEAR MOONMONTH MPH MPSS? MU0(_ERR)? MUON" + \
   "(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) NANO NAT NEUTRO" + \
   "N(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) NEWTON NICKEL(" + \
   "MAS|RADIU|THICKNES)S NINEVOLT(BATTERY(CHARGE|ENERGY|VOLT" + \
   "AGE)?|CHARGE|ENERGY|VOLTAGE)? NIR(ENERGY|Hz|WAVE(LENGTH|" + \
   "NUMBER)) NPBOUNDARY NUV(ENERGY|Hz|WAVE(LENGTH|NUMBER)) O" + \
   "HM ORANGE(ENERGY|Hz|WAVE(LENGTH|NUMBER)) OUNCE PA PARSEC" + \
   " PEBI PENNY(MAS|RADIU|THICKNES)S PETA PHI PHONEBATTERY(C" + \
   "HARGE|ENERGY|VOLTAGE)? PI PICO PIN(GPONGBALL(MASS|RADIUS" + \
   ")|T) PLANCK(|_(ERR|REC)|BAR(_ERR)?|C(HARGE|ONST(_ERR)?)|" + \
   "ENERGY|LENGTH|MASS|T(EMP|IME)) POINT POUND POWER PPB PPM" + \
   " PPQ PPT PRESSURE PRO(TON(_MASS|ENERGY|Hz|MASS(_ERR)?|RA" + \
   "DIUS(_ERR)?|WAVELENGTH)|XIMACENTAURI) PSI PTRBOUNDARY QU" + \
   "ART(ER(MAS|RADIU|THICKNES)S)? RAD(IAN)? RED(ENERGY|Hz|WA" + \
   "VE(LENGTH|NUMBER)) RESISTANCE RPM RPS RYDBERG(|_ERR|CONS" + \
   "T(_ERR)?|ENERGY|Hz|WAVE(LENGTH|NUMBER)) SECOND SHANNON S" + \
   "HF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) SI16M(AX|IN) SI32M(AX|" + \
   "IN) SIEMENS SOCCERBALL(MASS|RADIUS) SOFTBALL(MASS|RADIUS" + \
   ") SOL(ARCONSTANT|ID_ANGLE) SPE(CIFICHEAT|ED) SPHERESURFA" + \
   "CE SQDEG SQRTTWO SQUAREDEGREE STANDARD(ATMOSPHERE|GRAVIT" + \
   "Y) STE(FAN(_ERR|CONST(_ERR)?)?|RADIAN) SXENERGY SXHz SXW" + \
   "AVE(LENGTH|NUMBER) SXX(ENERGY|Hz|WAVE(LENGTH|NUMBER)) TA" + \
   "U(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH)? TBLSP TEBI TE" + \
   "MPERATURE TENNISBALL(MASS|RADIUS) TERA TESLA THF(ENERGY|" + \
   "Hz|WAVE(LENGTH|NUMBER)) TIME TON(NE)? TORR TRITON(_MASS|" + \
   "MASS(_ERR)?) TRJBOUNDARY TSP TT_AT_0(TAI|UT1) TURN UAMU(" + \
   "_ERR)? UET UHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) UI16MAX UI" + \
   "32MAX UNI(TGM|VERSE(AGE|MASS|RADIUS)) UPQUARK(_MASS|ENER" + \
   "GY|Hz|MASS(_ERR)?|WAVELENGTH) UVA(ENERGY|Hz|WAVE(LENGTH|" + \
   "NUMBER)) UVB(ENERGY|Hz|WAVE(LENGTH|NUMBER)) UVC(ENERGY|H" + \
   "z|WAVE(LENGTH|NUMBER)) VHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)" + \
   ") VIOLET(ENERGY|Hz|WAVE(LENGTH|NUMBER)) VISCOSITY VOL(T(" + \
   "AGE)?|UME) VONKLITZING(_ERR)? WATT WEBER WEEK YARD YELLO" + \
   "W(ENERGY|GREEN(ENERGY|Hz|WAVE(LENGTH|NUMBER))|Hz|WAVE(LE" + \
   "NGTH|NUMBER)) YOBI YOCTO YOTTA ZEBI ZEPTO ZETTA" + \
   "\n" + \
   "   Kw 0-ary Functions: corr_new euler__1 gau_rand jd2ymd" + \
   ...
   "bernoulli[dn]? bess(I[01]|J[01]|K[01]|Y(0|1(__1)?)) bit(" + \
   "s_(count|ls1b(pos)?|ms1b(pos)?)|wisenot) blackbody(f_pdf" + \
   "|l_pdf|mode|power) cantor carmichael catalan cauchy_rand" + \
   " cbrt ceil chisqr_rand circle_get clip clock(hour|min|s(" + \
   "ec|tdev|um)) cnv_dbl2si16 contfra(_print|c(_print|d_prin" + \
   "t)) corr(_(delete|handle[xy])|corr|s(lope|tderr)|y0) cos" + \
   "(|__1|d(inv)?|h(inv)?|in(tegral|v)|m1(__1)?|t(inv)?) cot" + \
   "([dht]?(inv)?)? csc([dht]?(inv)?)? ctz cu cube(root)? da" + \
   "te_(easter|pesach|rosh_hashanah) day2(dhms(inv)?|hour) d" + \
   "eg2(dms|rad|sec|tur) digamma(__1|inv)? dms2(deg|rad) doo" + \
   "msday ecc2flat Ein elliptic(1([ak]|m(__[12])?)|2([ak]|m(" + \
   "__1)?)|[EK]) erf(|__([12]|[34])|c(inv|x(inv)?)?|inv) eta" + \
   " euler(phi|x) exp(_(_[12]|rand)|integral[1i]|m1)? fac(|i" + \
   "nv|tor(_print|ial(double|inv)?)) fibo(x(inv)?)? flat2ecc" + \
   " floor frac fubini fusc g2jewish gamma_rand gau_(cdf|pdf" + \
   "|quantile) gd gdinv geo_rand ghmean1(inv)? glmean1 gmean" + \
   "1 gqmean1 has_primitiveroot haversin heaviside heronianm" + \
   "ean1 hlmean1 hmean1 hn hn__1 hour2day hqmean1 hypot1(inv" + \
   ")? id int_rand iround is(even|int|jewish8short|jewish9sh" + \
   "ort|jewishleap|leapyear|magic|neg|negint|odd|pos|posint|" + \
   "practical|prime|squarefree|zero) j2k2(dow|jd|kwt|uet|ymd" + \
   "hms) jacobicir jd2(64_print|dow|j2k|kwt|uet|ymdhms) jewi" + \
   "sh(2(g|jdx?)|_(months_in_year|yearlength)) k2a k2m k2tem" + \
   "p[cfr] ks_a_cdf kwt2(j(2k|d)|uet) l10 lg lgamma(__[12]|i" + \
   "nv)? lix(inv)? lmean1 ln ln1p ln__[12] lns log(10|2|i(nt" + \
   "egral|stic_rand))? lqmean1 lucasx? m2a m2b m2k mertens m" + \
   "inkowski mobius neg nom normal_(cdf(__1)?|pdf|quantile(_" + \
   "_1)?) num2char_print par(abola_(get|roots)|titionq?) pcf" + \
   " pfg pfl poi_rand pow(10|2) prim(e([01]|countx?|n(ext|th" + \
   "(__1)?)|prev)|itiveroot) qmean1 rad2(d(eg|ms2?)|sec|tur)" + \
   " rand(_init|l) reci reimann round rto(125|[iz]) sba sear" + \
   "ch_(check|search) sec(|2(d(eg|hms)|rad|tur)|[dht]?(inv)?" + \
   ") shc_s(hft2steps|teps2shft(inv)?) sig(m(a[01]|oid(inv)?" + \
   ")|n) sin(|__1|c(__1|c|inv)?|d(inv)?|h(inv)?|in(tegral|v)" + \
   "|t(inv)?) smooth sq sqinv sqrt(__[123]|x)? square(root)?" + \
   " stair stts(_(delete|reset)|count|m(a(gic|x)|ean|in|sq)|" + \
   "s(pread|tdev(mean)?|um)|var) supercatalan tan([dht]?(inv" + \
   ")?)? temp(c(2(k|tempf))?|f(2(k|tempc))?|r(2k)?) tgamma(_" + \
   "_([123]|stirling)|double|inv)? topyh1 totient tri(_rand|" + \
   "inv|p)? trunc tur2(deg|rad|sec) uet2(j(2k|d)|kwt) uni_ra" + \
   "nd versin wm(inv)? wp(inv)? ymdhms(2j(2k|d)|_get) zag ze" + \
   "ta zig(zag)?" + \
   "\n" + \
