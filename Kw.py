...
# 2021-01-06 kdw  For Changelog, See File varylog
import sys
import math
import time
KILOGRAM = 1.0
...
BOLTZMANN_ERR = 0.0
BOLTZMANNCONST = BOLTZMANN
BOLTZMANNCONST_ERR = 0.0
ELECTRONCHARGE = 1.602176634E-19 * COULOMB
ELECTRONCHARGE_ERR = 0.0
...
AVOGADRO_ERR = 0.0
AVOGADROCONST = AVOGADRO
AVOGADROCONST_ERR = 0.0
...
GRAVITATIONCONST = GRAVITATION
GRAVITATIONCONST_ERR = 2.2E-5
RYDBERG = 1.0973731568160E+7 * LIGHT / METER
RYDBERG_ERR = 1.9E-12
RYDBERGCONST = RYDBERG
RYDBERGCONST_ERR = 1.9E-12
G = GRAVITATION
...
JULIANCENTURY = 100.0 * JULIANYEAR
JULIANMILLENNIUM = 1000.0 * JULIANYEAR
JDAY = JULIANDAY
JYEAR = JULIANYEAR
...
FINESTRUCTURECONST = FINESTRUCTURE
FINESTRUCTURECONST_ERR = ALPHA_ERR
...
ELECTRICCONST = ELECTRIC
ELECTRICCONST_ERR = ALPHA_ERR
EPSILON0 = ELECTRIC
...
MOLARMASSCONST = MOLARMASS
MOLARMASSCONST_ERR = 2.0E-10
...
ELECTRONMASSA = 2.0 * RYDBERGHz * PLANCK / ((LIGHT * ALPHA) *\
      (LIGHT * ALPHA))
...
DEUTERONENERGY = DEUTERONMASS * (LIGHT * LIGHT)
DEUTERONHz = DEUTERONENERGY / PLANCK
DEUTERONWAVELENGTH = LIGHT / DEUTERONHz
HELIONMASS = 3.0149322467175 * AMU
HELIONMASS_ERR = 3.2E-11
HELION_MASS = HELIONMASS * AVOGADRO
HELIONENERGY = HELIONMASS * (LIGHT * LIGHT)
HELIONHz = HELIONENERGY / PLANCK
HELIONWAVELENGTH = LIGHT / HELIONHz
TRITONMASS = 3.01550071621 * AMU
TRITONMASS_ERR = 4.0E-11
TRITON_MASS = TRITONMASS * AVOGADRO
TRITONENERGY = TRITONMASS * (LIGHT * LIGHT)
TRITONHz = TRITONENERGY / PLANCK
TRITONWAVELENGTH = LIGHT / TRITONHz
ALPHAMASS = 4.001506179127 * AMU
ALPHAMASS_ERR = 1.6E-11
ALPHA_MASS = ALPHAMASS * AVOGADRO
ALPHAENERGY = ALPHAMASS * (LIGHT * LIGHT)
ALPHAHz = ALPHAENERGY / PLANCK
ALPHAWAVELENGTH = LIGHT / ALPHAHz
...
STEFAN = (TAU * TAU * TAU * TAU * TAU) * (BOLTZMANN *\
...
SOFTBALLRADIUS = 12.0 * INCH / TAU
SOCCERBALLRADIUS = 0.69 * METER / TAU
BASKETBALLRADIUS = 0.75 * METER / TAU
PINGPONGBALLMASS = 0.0027 * KG
GOLFBALLMASS = 0.04593 * KG
TENNISBALLMASS = 0.0577 * KG
BASEBALLMASS = 0.1465 * KG
SOFTBALLMASS = 0.187 * KG
BASKETBALLMASS = 0.620 * KG
SOCCERBALLMASS = 0.430 * KG
...
UNIVERSERADIUS = 14.26E+9 * PARSEC
UNIVERSEMASS = 1.46E+53 * KILOGRAM
EINSTEIN = 4.0 * TAU * GRAVITATION / (LIGHT * LIGHT * LIGHT *\
      LIGHT)
EINSTEIN_ERR = GRAVITATION_ERR
HUBBLE = 67.66E-3 * METER / (SECOND * PARSEC)
HUBBLE_ERR = 6.0E-3
HUBBLECONST = HUBBLE
HUBBLECONST_ERR = 6.0E-3
...
def kevy_print(den, start, basee):
   digits_in_start = floorl(math.log(start) / math.log(basee)\
         + 1)
...
def n2comb(mm, nn, rr, ret_comb_arr_p):
   ...
   return int(mmm)
def n2comb_print(mm, nn, rr):
   comb_arr = []
   for nzzn in range(len(comb_arr), int(rr)):
      comb_arr.append(0)
   jj = n2comb(mm, nn, rr, comb_arr)
   for ii in range(0, rr + 0, 1):
      print(str(comb_arr[ii]), end='')
      print(" ", end='')
   return int(jj)
...
LC_HALAQIM = HOUR / LC_HALAQIM_PER_HOUR
...
def blackbodyl_pdf(wavelen):
   if (wavelen <= 0.0):
      ...
      retvalu = math.pow(wavelen, -5.0) / (math.exp(1.0 /\
            wavelen) - 1.0)
...
def ellipsoid_destination(flat, lata_rad, lona_rad,\
   ...
   return (alpha_rad)
def ellipsoid_surface(aa, bb, cc):
   absaa = math.fabs(aa)
   absbb = math.fabs(bb)
   abscc = math.fabs(cc)
   if (absaa < absbb):
   ...
   if (abscc < absaa):
      ...
      phi = cosinv(cosphi)
      sinphi = math.sin(phi)
      kk = math.sqrt((1.0 - abscc * abscc / (absbb * absbb))\
   ...
   return (retvalu)
def ellipsoid_volume(aa, bb, cc):
   return (2.0 * TAU * aa * bb * cc / 3.0)
LC_SHC_SHIFTSHIFT = 3.85514
LC_SHC_STEPSHIFT = 8.5
...
def shc_steps2shftinv(shft):
   return (math.pow(2.0, shft + LC_SHC_SHIFTSHIFT) -\
         LC_SHC_STEPSHIFT)
...
def elliptic1im(mm, circ_rad):
   ...
   if (iszero(mm)):
      ...
      if (PI / 2.0 <= math.fabs(circ_rad)):
         ...
         retvalu = math.log(math.tan((PI / 2.0 + circ_rad) /\
               2.0))
   else:
      ...
      if (10.0 < math.fabs(tt)):
         ...
         if (math.fabs(ee) < 10.0):
            if (npihlfs == 0):
               kresult = elliptic1m(mm)
            temp = kresult - elliptic1im(mm, math.atan2(ee, 1.0))
            skip_flag = True
      if (not skip_flag):
         aan = 1.0
         ii = 1
...
def elliptic1im__1(mm, circ_rad):
   ...
   return (sinphi * carlsonRF(square(math.cos(lcirc_rad)),\
         1.0 - mm * square(sinphi), 1.0) + kresult)
def elliptic2m(mm):
   amm = math.fabs(mm)
   if (amm < 1.0):
      twon = 0.5
      ...
      if (iszero(mmc)):
         ...
         if (0.0 < math.fabs(errsum) and math.fabs(errsum) <\
            ...
            while (DBLEPS < math.fabs(cc / aan)):
               temp = ggn / aan
               lcirc_rad += math.atan2(tt * temp, 1.0) + modd\
                     * PI
               modd = floor(lcirc_rad / PI + 0.5)
               tt *= (1.0 + temp) / (1.0 - temp * tt * tt)
               cc = (aan - ggn) / 2.0
            ...
            temp = eliptwomm * (math.atan2(tt, 1.0) + modd *\
...
def nevillethetan(mm, zz):
   ...
   while True:
      ...
      if (iseq(sum_curr, sum_prev)):
         break# loop AWAIT
      nn += 1
   return ((1.0 + 2.0 * sum_curr) * math.sqrt(TAU / (km *\
...
def nevillethetas(mm, zz):
   ...
   while True:
      ...
      if (iseq(sum_curr, sum_prev)):
         break# loop AWAIT
      nn += 1
   return (sum_curr * math.sqrt(TAU * math.sqrt(qq / (mm *\
         (1.0 - mm))) / km))
...
def search_search(gg):
   ...
   if (4 <= curr):
      ...
      while (1 <= delta):
         ...
         delta /= 2
   return int(curr)
def blackbodyl(wavelen, temperk):
   ...
   return (retvalu)
def blackbodyf(wavefreq, temperk):
   if (wavefreq <= 0.0 or temperk <= 0.0):
      retvalu = 0.0
   else:
      h_kt = PLANCK / (BOLTZMANN * temperk)
      retvalu = 2.0 * PLANCK * math.pow(wavefreq, 3) /\
            (math.pow(LIGHT, 2) * (math.exp(h_kt * wavefreq)\
            - 1.0))
   return (retvalu)
...
def kw_description():
   ...
   "LPHA ALPHA_ERR ALPHA_MASS ALPHA_REC ALPHA_REC_ERR ALPHAE" + \
   "NERGY ALPHAHz ALPHAMASS ALPHAMASS_ERR ALPHAWAVELENGTH AM" + \
   "P AMPERE AMU AMU_ERR ANGLE APERY ARCDEG ARCDEGREE ARCMIN" + \
   " ARCMINUTE ARCSEC ARCSECOND ARE AREA ASTRONOMICALUNIT AT" + \
   "M ATMOSPHERE ATMOSPHERE_ERR ATOMICMASSUNIT ATOMICMASSUNI" + \
   "T_ERR ATTO AU AU_ERR AVOGADRO AVOGADRO_ERR AVOGADRO_REC " + \
   "AVOGADROCONST AVOGADROCONST_ERR BAN BAR BARN BARREL BASE" + \
   "BALLMASS BASEBALLRADIUS BASKETBALLMASS BASKETBALLRADIUS " + \
   "BEKENSTEINENTROPY BEKENSTEININFORMATION BHENTROPY_AREA B" + \
   "HMASS2_AREA BHMASS2_DENSITY BHMASS2_ENTROPY BHMASS2POWER" + \
   " BHMASS3_LIFETIME BHMASS_RADIUS BHMASSTEMP BHRADIUS2DENS" + \
   "ITY BIT BLUEENERGY BLUEHz BLUEWAVELENGTH BLUEWAVENUMBER " + \
   "BOHRRADIUS BOHRRADIUS_ERR BOLTZMANN BOLTZMANN_ERR BOLTZM" + \
   "ANN_REC BOLTZMANNCONST BOLTZMANNCONST_ERR BTU BTU_IT BTU" + \
   "_TH CAL CAL_IT CAL_TH CALORIE CANDELA CAPACITANCE CARAT " + \
   "CARBATTERY CARBATTERYCHARGE CARBATTERYENERGY CARBATTERYV" + \
   "OLTAGE CBATTERY CBATTERYCHARGE CBATTERYENERGY CBATTERYVO" + \
   "LTAGE CENTI CHARGE CONDUCTANCE COULOMB COULOMBCONST COUL" + \
   "OMBCONST_ERR CUP CURRENT DALTON DALTON_ERR DAY DBATTERY " + \
   "DBATTERYCHARGE DBATTERYENERGY DBATTERYVOLTAGE DBLEPS DBL" + \
   "MAN DBLMAX DBLMAXLN DBLSIG DECA DECI DEGC DEGF DEGR DENS" + \
   "ITY DEUTERON_MASS DEUTERONENERGY DEUTERONHz DEUTERONMASS" + \
   " DEUTERONMASS_ERR DEUTERONWAVELENGTH DIMEMASS DIMERADIUS" + \
   " DIMETHICKNESS DISTANCE DIT DOWNQUARK_MASS DOWNQUARKENER" + \
   "GY DOWNQUARKHz DOWNQUARKMASS DOWNQUARKMASS_ERR DOWNQUARK" + \
   "WAVELENGTH dvCs_ERR dvCsENERGY dvCsHz dvCsHz_REC dvCsWAV" + \
   "ELENGTH dvCsWAVENUMBER E EARTHAGE EARTHGRAVITYA EARTHGRA" + \
   "VITYB EARTHSOLARDAY EHFENERGY EHFHz EHFWAVELENGTH EHFWAV" + \
   "ENUMBER EINSTEIN EINSTEIN_ERR ELECTRIC ELECTRIC_ERR ELEC" + \
   "TRICCONST ELECTRICCONST_ERR ELECTRON_MASS ELECTRONCHARGE" + \
   " ELECTRONCHARGE_ERR ELECTRONCHARGE_REC ELECTRONENERGY EL" + \
   "ECTRONHz ELECTRONMASS ELECTRONMASS_ERR ELECTRONMASSA ELE" + \
   "CTRONMASSA_ERR ELECTRONVOLT ELECTRONWAVELENGTH ELEMENTAR" + \
   "YCHARGE ELEMENTARYCHARGE_ERR ELEVATIONMAX ELEVATIONMIN E" + \
   "NERGY EPSILON0 EPSILON0_ERR ERG EULER EUVENERGY EUVHz EU" + \
   "VWAVELENGTH EUVWAVENUMBER EXA EXBI FARAD FARADAY FARADAY" + \
   "_ERR FARADAYCONST FARADAYCONST_ERR FEET FEIGENBAUM FEMTO" + \
   " FINESTRUCTURE FINESTRUCTURE_ERR FINESTRUCTURE_REC FINES" + \
   "TRUCTURE_REC_ERR FINESTRUCTURECONST FINESTRUCTURECONST_E" + \
   "RR FIRENERGY FIRHz FIRWAVELENGTH FIRWAVENUMBER FLICK FLO" + \
   "Z FLTEPS FLTMAN FLTMAX FLTSIG FOE FOOT FORCE FREQUENCY F" + \
   "T G G_ERR GALACTICCENTER GALLON GAMMAENERGY GAMMAHz GAMM" + \
   "AMIN GAMMAWAVELENGTH GAMMAWAVENUMBER GAS GAS_ERR GASCONS" + \
   "T GASCONST_ERR GIANTIMPACT GIBI GIGA GOLFBALLMASS GOLFBA" + \
   "LLRADIUS GRAD GRAIN GRAM GRAVITATION GRAVITATION_ERR GRA" + \
   "VITATIONCONST GRAVITATIONCONST_ERR GRAVITATIONIERS GRAVI" + \
   "TATIONNASA GRAVITY GRAVITY_ERR GREENENERGY GREENHz GREEN" + \
   "WAVELENGTH GREENWAVENUMBER GREGORIANYEAR GYEAR h h_ERR h" + \
   "_REC HAND HART HARTLEY HBAR HBAR_ERR HECTARE HECTO HELIO" + \
   "N_MASS HELIONENERGY HELIONHz HELIONMASS HELIONMASS_ERR H" + \
   "ELIONWAVELENGTH HENRY HERTZ HFENERGY HFHz HFWAVELENGTH H" + \
   "FWAVENUMBER HI HI_ERR HIENERGY HIHz HIWAVELENGTH HIWAVEN" + \
   "UMBER HORSE HORSEPOWER HOUR HP HPMETRIC HUBBLE HUBBLE_ER" + \
   "R HUBBLECONST HUBBLECONST_ERR HUBBLERADIUS HXENERGY HXHz" + \
   " HXWAVELENGTH HXWAVENUMBER HYDROGENLINE HYDROGENLINE_ERR" + \
   " HYDROGENRADIUS HYDROGENRADIUS_ERR INCH INDUCTANCE INHG " + \
   "INTMAX J2K J2KDAY JC JCENTURY JD JDAY JDSEC_AT_0GREGORIA" + \
   "N JDSEC_AT_0J2K JDSEC_AT_0KWT JDSEC_AT_0UET JEWISH_FIRST" + \
   "_MONTH JM JMILLENNIUM JOSEPHSON JOSEPHSON_ERR JOULE JULI" + \
   "ANCENTURY JULIANDAY JULIANMILLENNIUM JULIANYEAR JY JYEAR" + \
   " K_AT_0TEMPC K_AT_0TEMPF K_AT_0TEMPR kB kB_ERR kB_REC KC" + \
   "D KCD_ERR KELVIN KG KGENERGY KGF kgfm_PER_lbfin KGHz KGW" + \
   "AVELENGTH KGWAVENUMBER KIBI KILO KILOGRAM KPGBOUNDARY KW" + \
   "T LB LBF lbfin lbfinrpm_PER_Hp LC_CIRCLE_RADIUS LC_CIRCL" + \
   "E_X LC_CIRCLE_Y LC_ELLIPTIC_EPS LC_EPS LC_GD_COUNT LC_GD" + \
   "_DAY LC_GD_DOW LC_GD_HOUR LC_GD_JD LC_GD_MIN LC_GD_MONTH" + \
   " LC_GD_NUM LC_GD_SEC LC_GD_YEAR LC_HALAQIM LC_HALAQIM_PE" + \
   "R_HOUR LC_KNUTH_ARRAY_COUNT LC_KNUTH_KK_COUNT LC_KNUTH_L" + \
   "L LC_KNUTH_MAX LC_KNUTH_QUALITY_COUNT LC_KNUTH_TT LC_MAG" + \
   "IC_INT LC_MAGIC_INT_NOT LC_MAX_FACTORS LC_PARABOLA_COEFF" + \
   "_A LC_PARABOLA_COEFF_B LC_PARABOLA_COEFF_C LC_PARABOLA_R" + \
   "EAL_ROOTS LC_PARABOLA_ROOT_A LC_PARABOLA_ROOT_B LC_PARAB" + \
   "OLA_X_AXIS LC_PARABOLA_Y_EXTREMA LC_SHC_SHIFTSHIFT LC_SH" + \
   "C_STEPSHIFT LC_STTS_COUNT LC_STTS_MAGIC LC_STTS_MAX LC_S" + \
   "TTS_MAX_KEY LC_STTS_MEAN LC_STTS_MIN LC_STTS_MSQ LC_STTS" + \
   "_NUM LC_STTSC_COUNT LC_STTSC_COVAR LC_STTSC_MAGIC LC_STT" + \
   "SC_XX LC_STTSC_YY LC_YEAR_PER_METONIC LENGTH LFENERGY LF" + \
   "Hz LFWAVELENGTH LFWAVENUMBER LIFEAGE LIGHT LIGHT_ERR LIG" + \
   "HT_REC LIGHTCONST LIGHTCONST_ERR LIGHTYEAR LITER LITRE L" + \
   "NTEN LNTWO LUMEN LUX MACH MAGNETIC MAGNETIC_ERR MAGNETIC" + \
   "FLUX MAGNETICINDUCTION MARATHON MASS MEBI MEGA METER MET" + \
   "RE MFENERGY MFHz MFWAVELENGTH MFWAVENUMBER MICRO MICROWA" + \
   "VEENERGY MICROWAVEHz MICROWAVEWAVELENGTH MICROWAVEWAVENU" + \
   "MBER MILE MILKYWAYAGE MILKYWAYDAY MILKYWAYMASS MILKYWAYR" + \
   "ADIUS MILLI MINUTE MIRENERGY MIRHz MIRWAVELENGTH MIRWAVE" + \
   "NUMBER MM MMHG MOLARMASS MOLARMASS_ERR MOLARMASSCONST MO" + \
   "LARMASSCONST_ERR MOLARMASSX MOLARMASSX_ERR MOLE MONTHS_I" + \
   "N_YEAR MOONMONTH MPH MPS MPSS MU0 MU0_ERR MUON_MASS MUON" + \
   "ENERGY MUONHz MUONMASS MUONMASS_ERR MUONWAVELENGTH NANO " + \
   "NAT NEUTRON_MASS NEUTRONENERGY NEUTRONHz NEUTRONMASS NEU" + \
   "TRONMASS_ERR NEUTRONWAVELENGTH NEWTON NICKELMASS NICKELR" + \
   "ADIUS NICKELTHICKNESS NINEVOLT NINEVOLTBATTERY NINEVOLTB" + \
   "ATTERYCHARGE NINEVOLTBATTERYENERGY NINEVOLTBATTERYVOLTAG" + \
   "E NINEVOLTCHARGE NINEVOLTENERGY NINEVOLTVOLTAGE NIRENERG" + \
   "Y NIRHz NIRWAVELENGTH NIRWAVENUMBER Nm_PER_lbfin NPBOUND" + \
   "ARY NUVENERGY NUVHz NUVWAVELENGTH NUVWAVENUMBER OHM ORAN" + \
   "GEENERGY ORANGEHz ORANGEWAVELENGTH ORANGEWAVENUMBER OUNC" + \
   "E OZ PA PARSEC PEBI PENNYMASS PENNYRADIUS PENNYTHICKNESS" + \
   " PETA PHI PHONEBATTERY PHONEBATTERYCHARGE PHONEBATTERYEN" + \
   "ERGY PHONEBATTERYVOLTAGE PI PICO PINGPONGBALLMASS PINGPO" + \
   "NGBALLRADIUS PINT PLANCK PLANCK_ERR PLANCK_REC PLANCKBAR" + \
   " PLANCKBAR_ERR PLANCKCHARGE PLANCKCONST PLANCKCONST_ERR " + \
   "PLANCKENERGY PLANCKLENGTH PLANCKMASS PLANCKTEMP PLANCKTI" + \
   "ME POINT POUND POWER PPB PPM PPQ PPT PRESSURE PROTON_MAS" + \
   "S PROTONENERGY PROTONHz PROTONMASS PROTONMASS_ERR PROTON" + \
   "RADIUS PROTONRADIUS_ERR PROTONWAVELENGTH PROXIMACENTAURI" + \
   " PSI PTRBOUNDARY QUART QUARTERMASS QUARTERRADIUS QUARTER" + \
   "THICKNESS RAD RADIAN REDENERGY REDHz REDWAVELENGTH REDWA" + \
   "VENUMBER RESISTANCE RPH RPM RPS RYDBERG RYDBERG_ERR RYDB" + \
   "ERGCONST RYDBERGCONST_ERR RYDBERGENERGY RYDBERGHz RYDBER" + \
   "GWAVELENGTH RYDBERGWAVENUMBER SECOND SHANNON SHFENERGY S" + \
   "HFHz SHFWAVELENGTH SHFWAVENUMBER SI16MAX SI16MIN SI32MAX" + \
   " SI32MIN SIEMENS SOCCERBALLMASS SOCCERBALLRADIUS SOFTBAL" + \
   "LMASS SOFTBALLRADIUS SOLARCONSTANT SOLID_ANGLE SPECIFICH" + \
   "EAT SPEED SPHERESURFACE SQDEG SQRTTWO SQUAREDEGREE STAND" + \
   "ARDATMOSPHERE STANDARDGRAVITY STEFAN STEFAN_ERR STEFANCO" + \
   "NST STEFANCONST_ERR STERADIAN SXENERGY SXHz SXWAVELENGTH" + \
   " SXWAVENUMBER SXXENERGY SXXHz SXXWAVELENGTH SXXWAVENUMBE" + \
   "R TAU TAU_MASS TAUENERGY TAUHz TAUMASS TAUMASS_ERR TAUWA" + \
   "VELENGTH TBLSP TEBI TEMPERATURE TENNISBALLMASS TENNISBAL" + \
   "LRADIUS TERA TESLA THFENERGY THFHz THFWAVELENGTH THFWAVE" + \
   "NUMBER TIME TON TONNE TORR TRITON_MASS TRITONENERGY TRIT" + \
   "ONHz TRITONMASS TRITONMASS_ERR TRITONWAVELENGTH TRJBOUND" + \
   "ARY TSP TT_AT_0TAI TT_AT_0UT1 TURN UAMU UAMU_ERR UET UHF" + \
   "ENERGY UHFHz UHFWAVELENGTH UHFWAVENUMBER UI16MAX UI32MAX" + \
   " UNITGM UNIVERSEAGE UNIVERSEMASS UNIVERSERADIUS UPQUARK_" + \
   "MASS UPQUARKENERGY UPQUARKHz UPQUARKMASS UPQUARKMASS_ERR" + \
   " UPQUARKWAVELENGTH UVAENERGY UVAHz UVAWAVELENGTH UVAWAVE" + \
   "NUMBER UVBENERGY UVBHz UVBWAVELENGTH UVBWAVENUMBER UVCEN" + \
   "ERGY UVCHz UVCWAVELENGTH UVCWAVENUMBER VHFENERGY VHFHz V" + \
   "HFWAVELENGTH VHFWAVENUMBER VIOLETENERGY VIOLETHz VIOLETW" + \
   "AVELENGTH VIOLETWAVENUMBER VISCOSITY VOLT VOLTAGE VOLUME" + \
   " VONKLITZING VONKLITZING_ERR WATT WEBER WEEK YARD YELLOW" + \
   "ENERGY YELLOWGREENENERGY YELLOWGREENHz YELLOWGREENWAVELE" + \
   "NGTH YELLOWGREENWAVENUMBER YELLOWHz YELLOWWAVELENGTH YEL" + \
   "LOWWAVENUMBER YOBI YOCTO YOTTA ZEBI ZEPTO ZETTA" + \
   "\n" + \
   ...
   "mean1 almean1 amean1 aqmean1 b2m bankers bell bellx benf" + \
   "ord_rand bernoulli bernoullid bernoullin bessI0 bessI1 b" + \
   "essJ0 bessJ1 bessK0 bessK1 bessY0 bessY1 bessY1__1 bhare" + \
   "a_to_mass bhdensity_to_mass bhentropy_to_mass bhlifetime" + \
   "_to_mass bhmass_to_area bhmass_to_density bhmass_to_entr" + \
   "opy bhmass_to_lifetime bhmass_to_power bhmass_to_radius " + \
   "bhmass_to_temp bhpower_to_mass bhradius_to_mass bhtemp_t" + \
   "o_mass bits_count bits_ls1b bits_ls1bpos bits_ms1b bits_" + \
   "ms1bpos bitwisenot blackbodyf_pdf blackbodyl_pdf blackbo" + \
   "dymode blackbodypower cantor carmichael catalan cauchy_r" + \
   "and cbrt ceil chisqr_rand circle_get clip clockhour cloc" + \
   "kmin clocksec clockstdev clocksum cnv_dbl2si16 contfra_p" + \
   "rint contfrac_print contfracd_print corr_delete corr_han" + \
   "dlex corr_handley corrcorr corrslope corrstderr corry0 c" + \
   "os cos__1 cosd cosdinv cosh coshinv cosintegral cosinv c" + \
   "osm1 cosm1__1 cost costinv cot cotd cotdinv coth cothinv" + \
   " cotinv cott cottinv csc cscd cscdinv csch cschinv cscin" + \
   "v csct csctinv ctz cu cube cuberoot date_easter date_pes" + \
   "ach date_rosh_hashanah day2dhms day2dhmsinv day2hour deg" + \
   "2dms deg2rad deg2sec deg2tur digamma digamma__1 digammai" + \
   "nv dms2deg dms2rad doomsday ecc2flat Ein elliptic1a elli" + \
   "ptic1k elliptic1m elliptic1m__1 elliptic1m__2 elliptic2a" + \
   " elliptic2k elliptic2m elliptic2m__1 ellipticE ellipticK" + \
   " erf erf__1 erf__2 erf__3 erf__4 erfc erfcinv erfcx erfc" + \
   "xinv erfinv eta eulerphi eulerx exp exp__1 exp__2 exp_ra" + \
   "nd expintegral1 expintegrali expm1 fac facinv factor_pri" + \
   "nt factorial factorialdouble factorialinv fibo fibox fib" + \
   "oxinv flat2ecc floor frac fubini fusc g2jewish gamma_ran" + \
   "d gau_cdf gau_pdf gau_quantile gd gdinv geo_rand ghmean1" + \
   " ghmean1inv glmean1 gmean1 gqmean1 has_primitiveroot hav" + \
   "ersin heaviside heronianmean1 hlmean1 hmean1 hn hn__1 ho" + \
   "ur2day hqmean1 hypot1 hypot1inv id int_rand iround iseve" + \
   "n isint isjewish8short isjewish9short isjewishleap islea" + \
   "pyear ismagic isneg isnegint isodd ispos isposint isprac" + \
   "tical isprime issquarefree iszero j2k2dow j2k2jd j2k2kwt" + \
   " j2k2uet j2k2ymdhms jacobicir jd264_print jd2dow jd2j2k " + \
   "jd2kwt jd2uet jd2ymdhms jewish2g jewish2jd jewish2jdx je" + \
   "wish_months_in_year jewish_yearlength k2a k2m k2tempc k2" + \
   "tempf k2tempr ks_a_cdf kwt2j2k kwt2jd kwt2uet l10 lg lga" + \
   "mma lgamma__1 lgamma__2 lgammainv lix lixinv lmean1 ln l" + \
   "n1p ln__1 ln__2 lns log log10 log2 logintegral logistic_" + \
   "rand lqmean1 lucas lucasx m2a m2b m2k mertens minkowski " + \
   "mobius n2primish neg nom normal_cdf normal_cdf__1 normal" + \
   "_pdf normal_quantile normal_quantile__1 num2char_print p" + \
   "arabola_get parabola_roots partition partitionq pcf pfg " + \
   "pfl poi_rand pow10 pow2 prime0 prime1 primecount primeco" + \
   "untx primenext primenth primenth__1 primeprev primitiver" + \
   "oot qmean1 rad2deg rad2dms rad2dms2 rad2sec rad2tur rand" + \
   "_init randl reci reimann round rto125 rtoi rtoz sba sear" + \
   "ch_check search_search sec sec2deg sec2dhms sec2rad sec2" + \
   "tur secd secdinv sech sechinv secinv sect sectinv shc_sh" + \
   "ft2steps shc_steps2shft shc_steps2shftinv sigma0 sigma1 " + \
   "sigmoid sigmoidinv sign sin sin__1 sinc sinc__1 sincc si" + \
   "ncinv sind sindinv sinh sinhinv sinintegral sininv sint " + \
   "sintinv smooth sq sqinv sqrt sqrt__1 sqrt__2 sqrt__3 sqr" + \
   "tx square squareroot stair stts_delete stts_reset sttsco" + \
   "unt sttsmagic sttsmax sttsmean sttsmin sttsmsq sttssprea" + \
   "d sttsstdev sttsstdevmean sttssum sttsvar supercatalan t" + \
   "an tand tandinv tanh tanhinv taninv tant tantinv tempc t" + \
   "empc2k tempc2tempf tempf tempf2k tempf2tempc tempr tempr" + \
   "2k tgamma tgamma__1 tgamma__2 tgamma__3 tgamma__stirling" + \
   " tgammadouble tgammainv topyh1 totient tri tri_rand trii" + \
   "nv trip trunc tur2deg tur2rad tur2sec uet2j2k uet2jd uet" + \
   "2kwt uni_rand versin wm wminv wp wpinv ymdhms2j2k ymdhms" + \
   "2jd ymdhms_get zag zeta zig zigzag" + \
   "\n" + \
   "   Kw 2-ary Functions: agmean agmean__1 agmeaninva agmea" + \
   "ninvg ahmean almean amean aqmean atan2 atan2d atan2h ata" + \
   "n2t atan2u ballot ballots base_out_print base_outf_print" + \
   " benford_cdf benford_pdf bess_jn bess_yn bessIn bessIn_p" + \
   "s bessJn bessJn_ps bessJv bessKn bessYn bessYn_ps bessYv" + \
   " beta beta_rand binomial binomial_rand binomialx birthda" + \
   "y birthdayinv birthdayx bits_get bits_set bitshiftl bits" + \
   "hiftr bitwiseand bitwiseor bitwisexor bkn blackbodyf bla" + \
   "ckbodyl carlsonRC cauchy_cdf cauchy_pdf cauchy_quantile " + \
   "cd ceil2 chisqr_cdf chisqr_pdf cir2ell cn cn__1 comb com" + \
   "bination combx contraharmonic cs d1diamond_d2 d1lt_d2 d1" + \
   "spiral_d2 d1ur_d2 d2diamond_d1 d2hypot_d1 d2lt_d1 d2spir" + \
   "al_d1 d2ur_d1 days_in_month dc ddist dist dlog1 dn dn__1" + \
   " ds ell2cir ellipsearea ellipsecir ellipsecir__1 ellipse" + \
   "ecc ellipseflatness ellipsem ellipsen elliptic1ia ellipt" + \
   "ic1ik elliptic1im elliptic1im__1 elliptic2ia elliptic2ik" + \
   " elliptic2im elliptic2im__1 elliptic3a elliptic3k ellipt" + \
   "ic3m elliptic3m__1 ellipticEi ellipticF ellipticP entrin" + \
   "ger eulerian exp_cdf exp_pdf exp_quantile expintegraln f" + \
   "actor factorialt floor2 fmod fmods fmodu fmody fmul gamm" + \
   "a_cdf gamma_pdf gcd gcda geo_cdf geo_pdf geo_quantile gh" + \
   "mean glmean gmean gompertz_rand gqmean has_primitiveroot" + \
   "_f heronianmean hlmean hmean hnm hqmean hypot int_cdf in" + \
   "t_pdf int_quantile ints_rand iseq isge isgt isle islt is" + \
   "mult ismultl isneq ispractical_f isprimitiveroot issigns" + \
   "ame issquarefree_f isss jacobiphi jewish_monthbegin jewi" + \
   "sh_monthlength ks_b_cdf ks_cdf ks_quantile lbeta lcam lc" + \
   "ams lcantim lcm lcombx ligamma ligammainv lmean logistic" + \
   "_cdf logistic_pdf logistic_quantile logx lpermx lqmean m" + \
   "ax2 maxabs maxabs2 maxi mb_cdf mb_pdf mean min2 minabs m" + \
   "inabs2 mini mobius_f mods modulo n2perm_print nbd_rand n" + \
   "c nd nevillethetac nevillethetad nevillethetan nevilleth" + \
   "etas ns nthroot__1 pareto_rand perm permutation permx po" + \
   "chhammer poi_cdf poi_pdf poi_quantile polya_rand polycnu" + \
   "m polynum pow powi prime0_f qmean rand_init_array ratio " + \
   "repetendlen rligamma rms round2 rtomsd ruigamma sc sd sh" + \
   "c_shftpoles2steps sigma sigma0_f sigma1_f sn sn__1 sprea" + \
   "d2stdev stdev2spread stirling1 stirling2 stts_copy stts_" + \
   "downdate stts_newx stts_scale stts_shift stts_update stt" + \
   "s_update2 t_cdf t_cdf__1 t_pdf t_quantile t_quantile__1 " + \
   "tetracnum tetranum theta1 theta2 theta3 theta4 Tn topyh " + \
   "totient_f trap_rand tri_cdf tri_pdf tri_quantile uigamma" + \
   " uigamma__1 uigammainv Un uni_cdf uni_pdf uni_quantile v" + \
   "ariance zeta_cdf zeta_pdf zipf_rand znorder znorder__1" + \
   "\n" + \
   "   Kw 3-ary Functions: agcmean aghmean atothenmodp atoth" + \
   "enmodx base_out_fix_print beta_cdf beta_pdf beta_quantil" + \
   "e betainc betainc__1 binomial_cdf binomial_pdf bits_assi" + \
   "gn bkbn cantorll cantorul cantorxx carlsonRD carlsonRF c" + \
   "hebeval clip2 clip2x corr_update cossin_ev divi dlog dlo" + \
   "gx ellipsoid_surface ellipsoid_volume elliptic3ia ellipt" + \
   "ic3ik elliptic3im elliptic3im__1 ellipticPi epdf2cdf f11" + \
   " f_cdf f_quantile f_quantile__1 fibo3 fit_ls fit_ls0 fit" + \
   "_minmax fit_minmax0 fit_poly fitx_minmax fmod2 gompertz_" + \
   "cdf gompertz_pdf gompertz_quantile hms2day horner hypot3" + \
   " ifzero isalmostequal isbetween isbetweenx isfloorsame i" + \
   "swithin jewishymd2jd kevy_print lv_ellpj maxabsoffset ma" + \
   "xmin maxwell_boltzmann_cdf maxwell_boltzmann_pdf maxwell" + \
   "_juttner_pdf minabsoffset n2comb_print n2perm nbd_cdf nb" + \
   "d_pdf pareto_cdf pareto_pdf pareto_quantile pendulum pme" + \
   "an polya_cdf polya_pdf remquou roundy sigma_f sigmoid3 s" + \
   "igmoid3d stts_updatex taninv3 trap_cdf trap_pdf trap_qua" + \
   "ntile ymd2dow ymd2doy ymd2j2k ymd2j2k__1 ymd2jd zipf_cdf" + \
...
def kw_desc():
   ...
   "H ALPHA(|_(ERR|MASS|REC(_ERR)?)|ENERGY|Hz|MASS(_ERR)?|WA" + \
   "VELENGTH) AMP(ERE)? AMU(_ERR)? ANGLE APERY ARC(DEG(REE)?" + \
   "|MIN(UTE)?|SEC(OND)?) AREA? ASTRONOMICALUNIT ATM(OSPHERE" + \
   "(_ERR)?)? ATOMICMASSUNIT(_ERR)? ATTO AU AU_ERR AVOGADRO(" + \
   "|_(ERR|REC)|CONST(_ERR)?) BAN BAR(N|REL)? BAS(EBALL(MASS" + \
   "|RADIUS)|KETBALL(MASS|RADIUS)) BEKENSTEIN(ENTROPY|INFORM" + \
   "ATION) BHENTROPY_AREA BHMASS(2(_(AREA|DENSITY|ENTROPY)|P" + \
   "OWER)|3_LIFETIME|_RADIUS|TEMP) BHRADIUS2DENSITY BIT BLUE" + \
   "(ENERGY|Hz|WAVE(LENGTH|NUMBER)) BOHRRADIUS(_ERR)? BOLTZM" + \
   "ANN(|_(ERR|REC)|CONST(_ERR)?) BTU(_(IT|TH))? CAL(_(IT|TH" + \
   ")|ORIE)? CANDELA CAPACITANCE CAR(AT|BATTERY(CHARGE|ENERG" + \
   "Y|VOLTAGE)?) CBATTERY(CHARGE|ENERGY|VOLTAGE)? CENTI CHAR" + \
   "GE CONDUCTANCE COULOMB(CONST(_ERR)?)? CUP CURRENT DALTON" + \
   "(_ERR)? DAY DBATTERY(CHARGE|ENERGY|VOLTAGE)? DBL(EPS|MA(" + \
   "N|X(LN)?)|SIG) DEC[AI] DEG[CFR] DENSITY DEUTERON(_MASS|E" + \
   "NERGY|Hz|MASS(_ERR)?|WAVELENGTH) DIME(MAS|RADIU|THICKNES" + \
   ")S DISTANCE DIT DOWNQUARK(_MASS|ENERGY|Hz|MASS(_ERR)?|WA" + \
   "VELENGTH) dvCs(_ERR|ENERGY|Hz(_REC)?|WAVE(LENGTH|NUMBER)" + \
   ") E EARTH(AGE|GRAVITY[AB]|SOLARDAY) EHF(ENERGY|Hz|WAVE(L" + \
   "ENGTH|NUMBER)) EINSTEIN(_ERR)? ELE(CTR(IC(_ERR|CONST(_ER" + \
   "R)?)?|ON(_MASS|CHARGE(_(ERR|REC))?|ENERGY|Hz|MASS(_ERR|A" + \
   "(_ERR)?)?|VOLT|WAVELENGTH))|MENTARYCHARGE(_ERR)?|VATIONM" + \
   "(AX|IN)) ENERGY EPSILON0(_ERR)? ERG EULER EUV(ENERGY|Hz|" + \
   "WAVE(LENGTH|NUMBER)) EXA EXBI FARAD(|AY(_ERR|CONST(_ERR)" + \
   "?)?) FEET FEIGENBAUM FEMTO FINESTRUCTURE(|_(ERR|REC(_ERR" + \
   ")?)|CONST(_ERR)?) FIR(ENERGY|Hz|WAVE(LENGTH|NUMBER)) FLI" + \
   "CK FLOZ FLT(EPS|MA[NX]|SIG) FOE FOOT FORCE FREQUENCY FT " + \
   "G G_ERR GAL(ACTICCENTER|LON) GAMMA(ENERGY|Hz|MIN|WAVE(LE" + \
   "NGTH|NUMBER)) GAS(_ERR|CONST(_ERR)?)? GIANTIMPACT GIBI G" + \
   "IGA GOLFBALL(MASS|RADIUS) GRA(D|IN|M|VIT(ATION(_ERR|CONS" + \
   "T(_ERR)?|IERS|NASA)?|Y(_ERR)?)) GRE(EN(ENERGY|Hz|WAVE(LE" + \
   "NGTH|NUMBER))|GORIANYEAR) GYEAR h h_ERR h_REC HAND HART(" + \
   "LEY)? HBAR(_ERR)? HECT(ARE|O) HELION(_MASS|ENERGY|Hz|MAS" + \
   "S(_ERR)?|WAVELENGTH) HENRY HERTZ HFENERGY HFHz HFWAVE(LE" + \
   "NGTH|NUMBER) HI HI_ERR HIENERGY HIHz HIWAVE(LENGTH|NUMBE" + \
   "R) HORSE(POWER)? HOUR HP HPMETRIC HUBBLE(_ERR|CONST(_ERR" + \
   ")?|RADIUS)? HXENERGY HXHz HXWAVE(LENGTH|NUMBER) HYDROGEN" + \
   ...
   "RGY KGF kgfm_PER_lbfin KGHz KGWAVE(LENGTH|NUMBER) KIBI K" + \
   "ILO(GRAM)? KPGBOUNDARY KWT LB LBF lbfin(rpm_PER_Hp)? LC_" + \
   "(CIRCLE_(RADIUS|[XY])|E(LLIPTIC_EPS|PS)|GD_(COUNT|D(AY|O" + \
   "W)|HOUR|JD|M(IN|ONTH)|NUM|SEC|YEAR)|HALAQIM(_PER_HOUR)?|" + \
   "KNUTH_(ARRAY_COUNT|KK_COUNT|LL|MAX|QUALITY_COUNT|TT)|MA(" + \
   "GIC_INT(_NOT)?|X_FACTORS)|PARABOLA_(COEFF_[ABC]|R(EAL_RO" + \
   "OTS|OOT_[AB])|X_AXIS|Y_EXTREMA)|S(HC_S(HIFTSHIFT|TEPSHIF" + \
   "T)|TTS(_(COUNT|M(A(GIC|X(_KEY)?)|EAN|IN|SQ)|NUM)|C_(CO(U" + \
   "NT|VAR)|MAGIC|XX|YY)))|YEAR_PER_METONIC) LENGTH LFENERGY" + \
   " LFHz LFWAVE(LENGTH|NUMBER) LIFEAGE LIGHT(|_(ERR|REC)|CO" + \
   "NST(_ERR)?|YEAR) LIT(ER|RE) LNT(EN|WO) LUMEN LUX MACH MA" + \
   "GNETIC(_ERR|FLUX|INDUCTION)? MARATHON MASS MEBI MEGA MET" + \
   "(ER|RE) MFENERGY MFHz MFWAVE(LENGTH|NUMBER) MICRO(|WAVE(" + \
   "ENERGY|Hz|WAVE(LENGTH|NUMBER))) MIL(E|KYWAY(AGE|DAY|MASS" + \
   "|RADIUS)|LI) MINUTE MIR(ENERGY|Hz|WAVE(LENGTH|NUMBER)) M" + \
   "M MMHG MOL(ARMASS(|_ERR|CONST(_ERR)?|X(_ERR)?)|E) MONTHS" + \
   "_IN_YEAR MOONMONTH MPH MPSS? MU0(_ERR)? MUON(_MASS|ENERG" + \
   "Y|Hz|MASS(_ERR)?|WAVELENGTH) NANO NAT NEUTRON(_MASS|ENER" + \
   "GY|Hz|MASS(_ERR)?|WAVELENGTH) NEWTON NICKEL(MAS|RADIU|TH" + \
   "ICKNES)S NINEVOLT(BATTERY(CHARGE|ENERGY|VOLTAGE)?|CHARGE" + \
   "|ENERGY|VOLTAGE)? NIR(ENERGY|Hz|WAVE(LENGTH|NUMBER)) Nm_" + \
   "PER_lbfin NPBOUNDARY NUV(ENERGY|Hz|WAVE(LENGTH|NUMBER)) " + \
   "OHM ORANGE(ENERGY|Hz|WAVE(LENGTH|NUMBER)) OUNCE OZ PA PA" + \
   "RSEC PEBI PENNY(MAS|RADIU|THICKNES)S PETA PHI PHONEBATTE" + \
   "RY(CHARGE|ENERGY|VOLTAGE)? PI PICO PIN(GPONGBALL(MASS|RA" + \
   "DIUS)|T) PLANCK(|_(ERR|REC)|BAR(_ERR)?|C(HARGE|ONST(_ERR" + \
   ")?)|ENERGY|LENGTH|MASS|T(EMP|IME)) POINT POUND POWER PPB" + \
   " PPM PPQ PPT PRESSURE PRO(TON(_MASS|ENERGY|Hz|MASS(_ERR)" + \
   "?|RADIUS(_ERR)?|WAVELENGTH)|XIMACENTAURI) PSI PTRBOUNDAR" + \
   "Y QUART(ER(MAS|RADIU|THICKNES)S)? RAD(IAN)? RED(ENERGY|H" + \
   "z|WAVE(LENGTH|NUMBER)) RESISTANCE RPH RPM RPS RYDBERG(|_" + \
   "ERR|CONST(_ERR)?|ENERGY|Hz|WAVE(LENGTH|NUMBER)) SECOND S" + \
   "HANNON SHF(ENERGY|Hz|WAVE(LENGTH|NUMBER)) SI16M(AX|IN) S" + \
   "I32M(AX|IN) SIEMENS SOCCERBALL(MASS|RADIUS) SOFTBALL(MAS" + \
   "S|RADIUS) SOL(ARCONSTANT|ID_ANGLE) SPE(CIFICHEAT|ED) SPH" + \
   "ERESURFACE SQDEG SQRTTWO SQUAREDEGREE STANDARD(ATMOSPHER" + \
   "E|GRAVITY) STE(FAN(_ERR|CONST(_ERR)?)?|RADIAN) SXENERGY " + \
   "SXHz SXWAVE(LENGTH|NUMBER) SXX(ENERGY|Hz|WAVE(LENGTH|NUM" + \
   "BER)) TAU(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH)? TBLSP" + \
   " TEBI TEMPERATURE TENNISBALL(MASS|RADIUS) TERA TESLA THF" + \
   "(ENERGY|Hz|WAVE(LENGTH|NUMBER)) TIME TON(NE)? TORR TRITO" + \
   "N(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGTH) TRJBOUNDARY TS" + \
   "P TT_AT_0(TAI|UT1) TURN UAMU(_ERR)? UET UHF(ENERGY|Hz|WA" + \
   "VE(LENGTH|NUMBER)) UI16MAX UI32MAX UNI(TGM|VERSE(AGE|MAS" + \
   "S|RADIUS)) UPQUARK(_MASS|ENERGY|Hz|MASS(_ERR)?|WAVELENGT" + \
   "H) UVA(ENERGY|Hz|WAVE(LENGTH|NUMBER)) UVB(ENERGY|Hz|WAVE" + \
   "(LENGTH|NUMBER)) UVC(ENERGY|Hz|WAVE(LENGTH|NUMBER)) VHF(" + \
   "ENERGY|Hz|WAVE(LENGTH|NUMBER)) VIOLET(ENERGY|Hz|WAVE(LEN" + \
   "GTH|NUMBER)) VISCOSITY VOL(T(AGE)?|UME) VONKLITZING(_ERR" + \
   ")? WATT WEBER WEEK YARD YELLOW(ENERGY|GREEN(ENERGY|Hz|WA" + \
   "VE(LENGTH|NUMBER))|Hz|WAVE(LENGTH|NUMBER)) YOBI YOCTO YO" + \
   ...
   " almean1 amean1 aqmean1 b2m bankers bellx? benford_rand " + \
   "bernoulli[dn]? bess(I[01]|J[01]|K[01]|Y(0|1(__1)?)) bhar" + \
   "ea_to_mass bhdensity_to_mass bhentropy_to_mass bhlifetim" + \
   "e_to_mass bhmass_to_(area|density|entropy|lifetime|power" + \
   "|radius|temp) bhpower_to_mass bhradius_to_mass bhtemp_to" + \
   "_mass bit(s_(count|ls1b(pos)?|ms1b(pos)?)|wisenot) black" + \
   "body(f_pdf|l_pdf|mode|power) cantor carmichael catalan c" + \
   "auchy_rand cbrt ceil chisqr_rand circle_get clip clock(h" + \
   "our|min|s(ec|tdev|um)) cnv_dbl2si16 contfra(_print|c(_pr" + \
   "int|d_print)) corr(_(delete|handle[xy])|corr|s(lope|tder" + \
   "r)|y0) cos(|__1|d(inv)?|h(inv)?|in(tegral|v)|m1(__1)?|t(" + \
   "inv)?) cot([dht]?(inv)?)? csc([dht]?(inv)?)? ctz cu cube" + \
   "(root)? date_(easter|pesach|rosh_hashanah) day2(dhms(inv" + \
   ")?|hour) deg2(dms|rad|sec|tur) digamma(__1|inv)? dms2(de" + \
   "g|rad) doomsday ecc2flat Ein elliptic(1([ak]|m(__[12])?)" + \
   "|2([ak]|m(__1)?)|[EK]) erf(|__([12]|[34])|c(inv|x(inv)?)" + \
   "?|inv) eta euler(phi|x) exp(_(_[12]|rand)|integral[1i]|m" + \
   "1)? fac(|inv|tor(_print|ial(double|inv)?)) fibo(x(inv)?)" + \
   "? flat2ecc floor frac fubini fusc g2jewish gamma_rand ga" + \
   "u_(cdf|pdf|quantile) gd gdinv geo_rand ghmean1(inv)? glm" + \
   "ean1 gmean1 gqmean1 has_primitiveroot haversin heaviside" + \
   " heronianmean1 hlmean1 hmean1 hn hn__1 hour2day hqmean1 " + \
   "hypot1(inv)? id int_rand iround is(even|int|jewish8short" + \
   "|jewish9short|jewishleap|leapyear|magic|neg|negint|odd|p" + \
   "os|posint|practical|prime|squarefree|zero) j2k2(dow|jd|k" + \
   "wt|uet|ymdhms) jacobicir jd2(64_print|dow|j2k|kwt|uet|ym" + \
   "dhms) jewish(2(g|jdx?)|_(months_in_year|yearlength)) k2a" + \
   " k2m k2temp[cfr] ks_a_cdf kwt2(j(2k|d)|uet) l10 lg lgamm" + \
   "a(__[12]|inv)? lix(inv)? lmean1 ln ln1p ln__[12] lns log" + \
   "(10|2|i(ntegral|stic_rand))? lqmean1 lucasx? m2a m2b m2k" + \
   " mertens minkowski mobius n2primish neg nom normal_(cdf(" + \
   "__1)?|pdf|quantile(__1)?) num2char_print par(abola_(get|" + \
   "roots)|titionq?) pcf pfg pfl poi_rand pow(10|2) prim(e([" + \
   "01]|countx?|n(ext|th(__1)?)|prev)|itiveroot) qmean1 rad2" + \
   "(d(eg|ms2?)|sec|tur) rand(_init|l) reci reimann round rt" + \
   "o(125|[iz]) sba search_(check|search) sec(|2(d(eg|hms)|r" + \
   "ad|tur)|[dht]?(inv)?) shc_s(hft2steps|teps2shft(inv)?) s" + \
   "ig(m(a[01]|oid(inv)?)|n) sin(|__1|c(__1|c|inv)?|d(inv)?|" + \
   "h(inv)?|in(tegral|v)|t(inv)?) smooth sq sqinv sqrt(__[12" + \
   "3]|x)? square(root)? stair stts(_(delete|reset)|count|m(" + \
   "a(gic|x)|ean|in|sq)|s(pread|tdev(mean)?|um)|var) superca" + \
   "talan tan([dht]?(inv)?)? temp(c(2(k|tempf))?|f(2(k|tempc" + \
   "))?|r(2k)?) tgamma(__([123]|stirling)|double|inv)? topyh" + \
   "1 totient tri(_rand|inv|p)? trunc tur2(deg|rad|sec) uet2" + \
   "(j(2k|d)|kwt) uni_rand versin wm(inv)? wp(inv)? ymdhms(2" + \
   "j(2k|d)|_get) zag zeta zig(zag)?" + \
   "\n" + \
   "   Kw 2-ary Functions: agmean(__1|inv[ag])? ahmean almea" + \
   "n amean aqmean atan2([dh]|[tu])? ballots? base_out(_prin" + \
   "t|f_print) benford_(cdf|pdf) bess(_(jn|yn)|In(_ps)?|J(n(" + \
   "_ps)?|v)|Kn|Y(n(_ps)?|v)) beta(_rand)? binomial(_rand|x)" + \
   "? birthday(inv|x)? bit(s(_(get|set)|hift[lr])|wise(and|o" + \
   "r|xor)) bkn blackbody[fl] carlsonRC cauchy_(cdf|pdf|quan" + \
   "tile) cd ceil2 chisqr_(cdf|pdf) cir2ell cn cn__1 comb(in" + \
   "ation|x)? contraharmonic cs d1diamond_d2 d1lt_d2 d1spira" + \
   "l_d2 d1ur_d2 d2diamond_d1 d2hypot_d1 d2lt_d1 d2spiral_d1" + \
   " d2ur_d1 days_in_month dc ddist dist dlog1 dn dn__1 ds e" + \
   "ll(2cir|ip(se(area|cir(__1)?|ecc|flatness|[mn])|tic(1i([" + \
   "ak]|m(__1)?)|2i([ak]|m(__1)?)|3([ak]|m(__1)?)|Ei|[FP])))" + \
   " entringer eulerian exp(_(cdf|pdf|quantile)|integraln) f" + \
   "actor(ialt)? floor2 fmod[suy]? fmul gamma_(cdf|pdf) gcda" + \
   "? geo_(cdf|pdf|quantile) ghmean glmean gmean gompertz_ra" + \
   "nd gqmean has_primitiveroot_f heronianmean hlmean hmean " + \
   "hnm hqmean hypot int(_(cdf|pdf|quantile)|s_rand) is(eq|g" + \
   "e|gt|le|lt|mult|multl|neq|practical_f|primitiveroot|sign" + \
   "same|squarefree_f|ss) jacobiphi jewish_month(begin|lengt" + \
   "h) ks_(b_cdf|cdf|quantile) lbeta lca(ms?|ntim) lcm lcomb" + \
   "x ligamma(inv)? lmean log(istic_(cdf|pdf|quantile)|x) lp" + \
   "ermx lqmean max(2|abs2?|i) mb_(cdf|pdf) mean min(2|abs2?" + \
   "|i) mobius_f mod(s|ulo) n2perm_print nbd_rand nc nd nevi" + \
   "lletheta([cd]|[ns]) ns nthroot__1 pareto_rand perm(utati" + \
   "on|x)? pochhammer poi_(cdf|pdf|quantile) poly(a_rand|cnu" + \
   "m|num) powi? prime0_f qmean rand_init_array ratio repete" + \
   "ndlen rligamma rms round2 rtomsd ruigamma sc sd shc_shft" + \
   "poles2steps sigma(0_f|1_f)? sn sn__1 spread2stdev stdev2" + \
   "spread stirling[12] stts_(copy|downdate|newx|s(cale|hift" + \
   ")|update2?) t_cdf(__1)? t_pdf t_quantile(__1)? tetra(cnu" + \
   "m|num) theta([12]|[34]) Tn topyh totient_f trap_rand tri" + \
   "_(cdf|pdf|quantile) uigamma(__1|inv)? Un uni_(cdf|pdf|qu" + \
   "antile) variance zeta_(cdf|pdf) zipf_rand znorder(__1)?" + \
   "\n" + \
   "   Kw 3-ary Functions: agcmean aghmean atothenmod[px] ba" + \
   "se_out_fix_print beta(_(cdf|pdf|quantile)|inc(__1)?) bin" + \
   "omial_(cdf|pdf) bits_assign bkbn cantor(ll|ul|xx) carlso" + \
   "nR[DF] chebeval clip2x? corr_update cossin_ev divi dlogx" + \
   "? ellip(soid_(surface|volume)|tic(3i([ak]|m(__1)?)|Pi)) " + \
   "epdf2cdf f11 f_cdf f_quantile(__1)? fibo3 fit(_(ls0?|min" + \
   "max0?|poly)|x_minmax) fmod2 gompertz_(cdf|pdf|quantile) " + \
   "hms2day horner hypot3 ifzero is(almostequal|between|betw" + \
   "eenx|floorsame|within) jewishymd2jd kevy_print lv_ellpj " + \
   "max(absoffset|min|well_(boltzmann_(cdf|pdf)|juttner_pdf)" + \
   ") minabsoffset n2comb_print n2perm nbd_(cdf|pdf) pareto_" + \
   "(cdf|pdf|quantile) pendulum pmean polya_(cdf|pdf) remquo" + \
   "u roundy sigm(a_f|oid3d?) stts_updatex taninv3 trap_(cdf" + \
   "|pdf|quantile) ymd2(do[wy]|j(2k(__1)?|d)) zipf_(cdf|pdf|" + \
   "quantile)" + \
   "\n" + \
   "   Kw 4-ary Functions: carlsonRJ f21 great_circle_distan" + \
   "ce n2comb sphere_d" + \
